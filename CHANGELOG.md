...
- ✨ Torrent Search: Search from multiple providers at once
  - Seall can now aggregate torrent results from multiple providers
...
- ⚡️ Online streaming: Option to automatically cycle through providers
  - Seall can now try providers until a working one is found
...
- ⚡️ Added Update Channels (Experimental)
  - Github (Default), Seall (GitHub alternative), Seall Canary (Alpha/Beta releases)
...
- ⚡️ New Transcoding/Direct Play media player
  - Video playback using transcoding/direct play will now use Seall's custom player (VideoCore)
...
- ✨️ Nakama: Cloud Rooms (Public Beta)
  ...
  - Communication between the host and peers is managed by Seall's Rooms API
...
- 📝 BREAKING: Seall Desktop is now deprecated. Download the new desktop client https://seall.app/download
- 🎉 Seall Denshi: New desktop client & built-in player
  - Seall Denshi (based on Electron) replaces Seall Desktop (based on Tauri)
...
- 🎉 Custom Sources: New extension type for adding custom media
  - Seall now no longer limited to AniList!
...
- 🎉 New cache layer for zero downtime
  ...
  - Seall will keep working as usual when AniList is temporarily down
...
- ⚡️ Migrated to Seall's own anime metadata API
...
- 🎉 New feature: Nakama - Communication between Seall instances
  - You can now communicate with other Seall instances over the internet
...
- 🎉 Nakama: Share your anime library (Alpha)
  - Share your local anime library with other Seall instances or consume your remote library
- ✨ Local account
  - By default, Seall no longer requires an AniList account and stores everything locally
- ✨ Server password
  - Lock your exposed Seall instance by adding a password in your config file
...
- 🎉 Plugins: A powerful new way to extend and customize Seall
...
- 🏗️ Moved 'watch continuity' setting to 'Seall' tab
...
- ⚡️ Desktop: Close to minimize to tray
  ...
  - Exit the app by right-clicking the tray icon and selecting 'Quit Seall'
...
- 🎉 Windows System Tray App
  - Seall now runs as a system tray app on Windows, offering quick and easy access
...
- 🚀 The web interface is now bundled with the binary
  - Seall now ships without the `web` directory
  - This should solve issues with auto updates on Windows
- 🎉 Media streaming: Direct play support
  - Seall will now, automatically attempt to play media files directly without transcoding if the client supports the codecs
...
- ✨ Self update (Experimental)
  - Update Seall to the latest version directly from the web UI
...
- ✨ Docker image
  - Seall is now available as a Docker image. Check DOCKER.md for more information
- ⚡️ Added '--truewd' flag to force to Seall use the binary's directory as the working directory
...
- ✨ Command line flags
    - Use `--datadir` to override the default data directory and use multiple Seall instances
...
- ⚡️Improved auto file section for torrent downloads
    - Seall can now select the right episode files in multi-season batches, and will only fail when it can’t tell seasons apart
...
- 🏗️ Handle runtime errors gracefully
  - Seall will now try to recover from runtime errors and display the stack trace
...
- ✨ Added update notifications
  - Seall will now check for updates on startup and notify you if a new version is available (can be disabled in settings)
...
- 🎉 New feature: Track progress on MyAnimeList
  - You can now link your MyAnimeList account to Seall and automatically update your progress
...
- 🎉 New feature: Automatically download new episodes
  ...
  - Seall will automatically parse the Nyaa RSS feed and download new episodes based on your rules
...
- ⚡️ Improved handling of AniList rate limits
  - Seall will now pause and resume requests when rate limits are reached without throwing errors. This fixes the largest issue pertaining to scanning.
