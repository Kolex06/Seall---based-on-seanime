# Seall Development and Build Guide

- [Seall Denshi Guide](https://github.com/Kolex06/Seall---based-on-seanime/blob/main/seall-denshi/README.md)
...
* Seall Denshi: Built with a custom [Electron](https://www.electronjs.org/)/Chromium to support more codecs
...
2. After the build completes, a new `out` directory will be created inside `seall-web`.
...
1. **Windows (System Tray)**:
   ```bash
   set CGO_ENABLED=1
   go build -o seall.exe -trimpath -ldflags="-s -w -H=windowsgui -extldflags '-static'"
   ```
...
3. **Automated Type Generation**:
   ...
   - Types for the frontend are generated in:
     - `seall-web/api/generated/types.ts`
     - `seall-web/api/generated/endpoint.types.ts`
     - `seall-web/api/generated/hooks_template.ts`
...
#### SIMKL GraphQL API Integration

The project integrates with the SIMKL GraphQL API:

1. **GraphQL Queries**:
   - Queries are defined in `internal/simkl/queries/*.graphql`
...
3. **Client Implementation**:
   - Generated queries and types are in `internal/api/simkl/client_gen.go`
   - A wrapper implementation in `internal/api/simkl/client.go` provides a cleaner interface
...
1. Create a dummy SIMKL account for testing
...
SIMKL mock fixtures are read-only during normal test runs. Set `SEALL_TEST_RECORD_MEDIA_FIXTURES=true` when you intentionally want missing or refreshed fixtures written back to the repository.

To avoid remembering the environment variable and basic auth checks, use the refresh wrapper:

```bash
go run ./scripts/record_simkl_fixtures
```

Notes:

- It validates that `test/config.toml` exists, `flags.enable_simkl_tests=true`, and `provider.simkl_jwt` is set.
- It defaults to refreshing `./internal/api/simkl` and sets `SEALL_TEST_RECORD_MEDIA_FIXTURES=true` for the test process.
- Pass packages to widen the refresh scope, for example `go run ./scripts/record_simkl_fixtures ./internal/api/simkl ./internal/library/scanner`.
- Pass `-run` to target specific live refresh tests, for example `go run ./scripts/record_simkl_fixtures -run 'TestGetAnimeByIdLive|TestBaseAnime_FetchMediaTree_BaseAnimeLive'`.
