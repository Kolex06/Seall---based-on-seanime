...
import (
	...
	"seall/internal/torrent_clients/qbittorrent"
	"seall/internal/torrent_clients/torrent_client"
	"seall/internal/torrent_clients/transmission"
...
func (a *App) initModulesOnce() {
	...
	a.LocalManager.SetRefreshMediaCollectionsFunc(func() {
	...
	a.PlaybackManager = playbackmanager.New(&playbackmanager.NewPlaybackManagerOptions{
		...
		PlatformRef:         a.MediaPlatformRef,
...
func (a *App) InitOrRefreshModules() {
	...
	if settings.Simkl != nil {
		shared_platform.ShouldCache.Store(!settings.Simkl.DisableCacheLayer)
	}
...
// InitOrRefreshMediaData initializes the SIMKL media collection and the account.
// This function should be called after App.Database is initialized and after settings are updated.
func (a *App) InitOrRefreshMediaData() {
	a.Logger.Debug().Msg("app: Fetching SIMKL media data")
	...
	// Set username to active media platform.
	a.MediaPlatformRef.Get().SetUsername(currUser.Viewer.Name)
	...
	go func() {
		...
		if err != nil {
			a.Logger.Error().Err(err).Msg("app: Failed to fetch SIMKL media collection")
		}
		...
		if err != nil {
			a.Logger.Error().Err(err).Msg("app: Failed to fetch manga collection")
		}
	}()
	...
	a.Logger.Info().Msg("app: Fetched SIMKL media data")
}
