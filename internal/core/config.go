...
type Config struct {
	...
	Server  struct {
		...
		Port            int
		Offline         bool
		UseBinaryPath   bool // Makes $SEALL_WORKING_DIR point to the binary's directory
		Systray         bool
		DoHUrl          string
		Password        string
...
func NewConfig(options *ConfigOptions, logger *zerolog.Logger) (*Config, error) {
	...
	if os.Getenv("SEALL_DATA_DIR") != "" {
		definedDataDir = os.Getenv("SEALL_DATA_DIR")
	}
	...
	if os.Getenv("SEALL_SERVER_HOST") != "" {
		defaultHost = os.Getenv("SEALL_SERVER_HOST")
	}
	if os.Getenv("SEALL_SERVER_PORT") != "" {
		var err error
		defaultPort, err = strconv.Atoi(os.Getenv("SEALL_SERVER_PORT"))
		if err != nil {
			return nil, fmt.Errorf("invalid SEALL_SERVER_PORT environment variable: %s", os.Getenv("SEALL_SERVER_PORT"))
		}
	}
	...
	// Set Seall's default custom environment variables
	...
	viper.SetDefault("database.name", "seall")
	viper.SetDefault("web.assetDir", "$SEALL_DATA_DIR/assets")
	viper.SetDefault("cache.dir", "$SEALL_DATA_DIR/cache")
	viper.SetDefault("cache.transcodeDir", "$SEALL_DATA_DIR/cache/transcode")
	viper.SetDefault("manga.downloadDir", "$SEALL_DATA_DIR/reading")
	viper.SetDefault("manga.localDir", "$SEALL_DATA_DIR/reading-local")
	viper.SetDefault("logs.dir", "$SEALL_DATA_DIR/logs")
	viper.SetDefault("offline.dir", "$SEALL_DATA_DIR/offline")
	viper.SetDefault("offline.assetDir", "$SEALL_DATA_DIR/offline/assets")
	viper.SetDefault("extensions.dir", "$SEALL_DATA_DIR/extensions")
	viper.SetDefault("simkl.clientId", os.Getenv("SIMKL_CLIENT_ID"))
	viper.SetDefault("simkl.clientSecret", os.Getenv("SIMKL_CLIENT_SECRET"))
	viper.SetDefault("simkl.redirectUri", os.Getenv("SIMKL_REDIRECT_URI"))
	...
	if (flags.Host != "" || os.Getenv("SEALL_SERVER_HOST") != "") && existingHost != defaultHost {
		viper.Set("server.host", defaultHost)
		hostChanged = true
	}
	if flags.Host == "" && os.Getenv("SEALL_SERVER_HOST") == "" && existingHost == "127.0.0.1" {
		viper.Set("server.host", defaultHost)
		hostChanged = true
	}
	if (flags.Port != 0 || os.Getenv("SEALL_SERVER_PORT") != "") && existingPort != defaultPort {
		viper.Set("server.port", defaultPort)
		portChanged = true
	}
	if flags.Port == 0 && os.Getenv("SEALL_SERVER_PORT") == "" && existingPort == 43211 {
		viper.Set("server.port", defaultPort)
		portChanged = true
	}
	if flags.Password != "" {
	...
	cfg.Data.WorkingDir = os.Getenv("SEALL_WORKING_DIR")

	if cfg.Server.Tls.Enabled && (cfg.Server.Tls.CertPath == "" || cfg.Server.Tls.KeyPath == "") {
		viper.SetDefault("server.tls.certPath", "$SEALL_DATA_DIR/certs/cert.pem")
		viper.SetDefault("server.tls.keyPath", "$SEALL_DATA_DIR/certs/key.pem")
		_ = viper.WriteConfig()
...
func setDataDirEnv(dataDir string) error {
	...
	if os.Getenv("SEALL_DATA_DIR") == "" {
		if err := os.Setenv("SEALL_DATA_DIR", dataDir); err != nil {
			return err
		}
	}
...
func setWorkingDirEnv(useBinaryPath bool) error {
	...
	if err = os.Setenv("SEALL_WORKING_DIR", filepath.FromSlash(wd)); err != nil {
		return err
	}
...
func initAppDataDir(definedDataDir string, logger *zerolog.Logger) (dataDir string, configPath string, err error) {
	...
	if definedDataDir != "" {
		...
		dataDir = filepath.Join(dataDir, "Seall")
	}
...
func loadLogo(embeddedLogo []byte, dataDir string) (err error) {
	...
	logoPath := filepath.Join(dataDir, "seall-logo.png")
