...
func NewEchoApp(app *App, webFS *embed.FS) *echo.Echo {
	...
	e.Use(func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			if strings.HasPrefix(c.Request().URL.Path, "/reading-downloads/") {
	...
	e.Use(func(next echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			if strings.HasPrefix(cUrl, "/api") ||
				...
				strings.HasPrefix(cUrl, "/reading-downloads") ||
	...
	e.Use(middleware.StaticWithConfig(middleware.StaticConfig{
		...
		Skipper: func(c echo.Context) bool {
			...
			if strings.HasPrefix(cUrl.RequestURI(), "/api") ||
				...
				strings.HasPrefix(cUrl.RequestURI(), "/reading-downloads") ||
	...
	// Serve reading downloads
	if app.Config.Manga.DownloadDir != "" {
		app.Logger.Info().Msgf("app: Reading downloads path: %s", app.Config.Manga.DownloadDir)
		e.Static("/reading-downloads", app.Config.Manga.DownloadDir)
	}
...
func RunEchoServer(app *App, e *echo.Echo) {
	...
	app.Logger.Info().Msg("app: " + constants.AppName + " started at " + app.Config.GetServerURI())
}
