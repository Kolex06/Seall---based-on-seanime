...
import (
	...
	hibikemanga "seall/internal/extension/hibike/manga"
	...
	goja_runtime "seall/internal/goja/goja_runtime"
...
type (
	PlaygroundRepository struct {
		...
		baseAnimeCache      *result.Cache[int, *mediaapi.BaseAnime]
		baseMangaCache      *result.Cache[int, *mediaapi.BaseManga]
...
func NewPlaygroundRepository(logger *zerolog.Logger, platformRef *util.Ref[platform.Platform], metadataProviderRef *util.Ref[metadata_provider.Provider]) *PlaygroundRepository {
	return &PlaygroundRepository{
		...
		baseAnimeCache:      result.NewCache[int, *mediaapi.BaseAnime](),
...
func (r *PlaygroundRepository) RunPlaygroundCode(params *RunPlaygroundCodeParams) (resp *RunPlaygroundCodeResponse, err error) {
	...
	case extension.TypeMediaTorrentProvider:
		return r.runPlaygroundCodeMediaTorrentProvider(ext, params)
	default:
	}
...
func (r *PlaygroundRepository) getAnime(mediaId int) (anime *mediaapi.BaseAnime, am *metadata.AnimeMetadata, err error) {
	var ok bool
	...
	am, _ = r.metadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, mediaId)
	return anime, am, nil
}

func (r *PlaygroundRepository) getManga(mediaId int) (manga *mediaapi.BaseManga, err error) {
	var ok bool
...
func (r *PlaygroundRepository) runPlaygroundCodeMediaTorrentProvider(ext *extension.Extension, params *RunPlaygroundCodeParams) (resp *RunPlaygroundCodeResponse, err error) {
	...
	case extension.LanguageJavascript, extension.LanguageTypescript:
		_, provider, err := extension_repo.NewGojaMediaTorrentProvider(ext, params.Language, playgroundLogger.logger, r.gojaRuntimeManager, r.wsEventManager)
		if err != nil {
...
func (r *PlaygroundRepository) runPlaygroundCodeOnlinestreamProvider(ext *extension.Extension, params *RunPlaygroundCodeParams) (resp *RunPlaygroundCodeResponse, err error) {
	...
	case extension.LanguageJavascript, extension.LanguageTypescript:
		...
		case "search":
			...
			if len(ret) == 0 {
				return newPlaygroundResponse(playgroundLogger, onlinestream.ErrNoMediaFound), nil
			}
		...
		case "findMediaItems", "findEpisodes":
			// FindMediaItems - params: id: string
			res, err := provider.FindMediaItems(params.Inputs["id"].(string))
			if err != nil {
				return newPlaygroundResponse(playgroundLogger, err), nil
			}
			return newPlaygroundResponse(playgroundLogger, res), nil

		case "findMediaItemServer", "findEpisodeServer":
			// FindMediaItemServer - params: item: EpisodeDetails, server: string
			itemJSON, ok := params.Inputs["item"].(string)
			if !ok {
				itemJSON = params.Inputs["episode"].(string)
			}
			var item hibikeonlinestream.EpisodeDetails
			_ = json.Unmarshal([]byte(itemJSON), &item)

			res, err := provider.FindMediaItemServer(&item, params.Inputs["server"].(string))
			if err != nil {
				return newPlaygroundResponse(playgroundLogger, err), nil
			}
			return newPlaygroundResponse(playgroundLogger, res), nil
