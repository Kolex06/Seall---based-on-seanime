...
import (
	"encoding/json"
	"fmt"
	metadataapi "seall/internal/api/metadata"
...
class Provider {
    ...
    return [
      {
        ...
        title: options.query,
        url: "https://example.com/media/" + options.query.toLowerCase().replace(/\s+/g, "-"),
        subOrDub: options.dub ? "dub" : "sub",
      },
    ]
  }

  async findMediaItems(id) {
    return [
      {
        id: id + "-1",
        number: 1,
  ...
  async findMediaItemServer(item, server) {
    return {
      server: server,
      headers: {
        Referer: item.url,
...
class Provider {
  ...
  async findMediaItems(_id) {
    return []
  }

  async findMediaItemServer(_item, _server) {
    return {
      server: "default",
      headers: {},
      videoSources: [],
...
func TestRunPlaygroundCodeMediaTorrentProvider(t *testing.T) {
	repo, _, _ := newTestPlaygroundRepository()
	...
	t.Run("direct info helpers and settings", func(t *testing.T) {
		...
		require.NoError(t, err)
		var latest []hibiketorrent.MediaTorrent
...
func TestRunPlaygroundCodeOnlinestreamProvider(t *testing.T) {
	...
	t.Run("media items and media item server", func(t *testing.T) {
		resp, err := repo.RunPlaygroundCode(&RunPlaygroundCodeParams{
			Type:     extension.TypeOnlinestreamProvider,
			Language: extension.LanguageJavascript,
			Code:     onlinestreamProviderScript,
		...
		require.NoError(t, err)
		var items []hibikeonlinestream.EpisodeDetails
		decodeJSON(t, resp.Value, &items)
		require.Len(t, items, 1)
		require.Equal(t, "playground-extension", items[0].Provider)
