...
func (s *StreamManager) getMediaInfo(ctx context.Context, mediaId int) (media *mediaapi.CompleteAnime, animeMetadata *metadata.AnimeMetadata, err error) {
	// Get the media
	var found bool
	...
	// Get the media
	animeMetadata, err = s.repository.metadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, mediaId)
	if err != nil {
