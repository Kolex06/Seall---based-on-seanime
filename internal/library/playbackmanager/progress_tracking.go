...
var (
	ErrProgressUpdateSimkl = errors.New("playback manager: Failed to update progress on SIMKL")
...
func (pm *PlaybackManager) handleVideoCompleted(status *mediaplayer.PlaybackStatus) {
	...
	//
	// Update the progress on SIMKL if auto update progress is enabled
	//
...
func (pm *PlaybackManager) autoSyncCurrentProgress(_ps *PlaybackState) {
	...
	pm.Logger.Debug().Msg("playback manager: Updating progress on SIMKL")
	err = pm.updateProgress()

	if err != nil {
		_ps.ProgressUpdated = false
		pm.wsEventManager.SendEvent(events.ErrorToast, "Failed to update progress on SIMKL")
	} else {
...
//   - This method will return an error only if the progress update fails on SIMKL
//   - This method will refresh the simkl collection
...
// updateProgress updates the progress of the current video playback on SIMKL and MyAnimeList.
// This only returns an error if the progress update fails on SIMKL
...
func (pm *PlaybackManager) updateProgress() (err error) {
	...
	if err != nil {
		pm.Logger.Error().Err(err).Msg("playback manager: Error occurred while updating progress on SIMKL")
		return ErrProgressUpdateSimkl
	}

	pm.refreshAnimeCollectionFunc() // Refresh the SIMKL collection

	pm.Logger.Info().Msg("playback manager: Updated progress on SIMKL")
