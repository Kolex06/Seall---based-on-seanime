...
func (pm *PlaybackManager) StartRandomVideo(opts *StartRandomVideoOptions) error {
	...
	animeCollection, err := pm.platformRef.Get().GetMediaCollection(context.Background(), false)
	if err != nil {
		return err
	}
	...
	for _, e := range lfEntries {
		simklEntry, ok := animeCollection.GetListEntryFromAnimeId(e.GetMediaId())
		if !ok {
			continue
		}
		progress := 0
		if simklEntry.Progress != nil {
			progress = *simklEntry.Progress
		}
		if simklEntry.Status == nil || *simklEntry.Status == "COMPLETED" {
			continue
		}
		...
		if *simklEntry.Status == "CURRENT" || *simklEntry.Status == "REPEATING" {
