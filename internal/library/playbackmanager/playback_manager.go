...
type (
	...
	//  - Syncing progress with SIMKL, etc.
	...
	PlaybackManager struct {
		...
		refreshAnimeCollectionFunc func() // This function is called to refresh the SIMKL collection
		mu                         sync.Mutex
		...
		currentMediaListEntry mo.Option[*mediaapi.AnimeListEntry] // List Entry for the current video playback
		...
		currentStreamMedia        mo.Option[*mediaapi.BaseAnime]
...
func New(opts *NewPlaybackManagerOptions) *PlaybackManager {
	pm := &PlaybackManager{
		...
		currentStreamMedia:           mo.None[*mediaapi.BaseAnime](),
		...
		currentMediaListEntry:        mo.None[*mediaapi.AnimeListEntry](),
...
func (pm *PlaybackManager) SetAnimeCollection(ac *mediaapi.AnimeCollection) {
...
func (pm *PlaybackManager) StartStreamingUsingMediaPlayer(windowTitle string, opts *StartPlayingOptions, media *mediaapi.BaseAnime, aniDbEpisode string) (err error) {
...
func (pm *PlaybackManager) checkOrLoadAnimeCollection() (err error) {
	...
	if pm.animeCollection.IsAbsent() {
		...
		collection, err := pm.platformRef.Get().GetMediaCollection(context.Background(), false)
		if err != nil {
			return err
		}
