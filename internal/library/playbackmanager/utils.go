...
// GetLocalFilePlaybackDetails is called once everytime a new video is played. It returns the simkl entry, local file and local file wrapper entry.
func (pm *PlaybackManager) getLocalFilePlaybackDetails(path string) (*mediaapi.AnimeListEntry, *anime.LocalFile, *anime.LocalFileWrapperEntry, error) {
	pm.mu.Lock()
	defer pm.mu.Unlock()
	// Normalize path
	...
	reqEvent := &PlaybackLocalFileDetailsRequestedEvent{
		Path:                  path,
		LocalFiles:            lfs,
		AnimeListEntry:        &mediaapi.AnimeListEntry{},
	...
	if !ok {
		return nil, nil, nil, errors.New("simkl list entry not found")
	}
...
func (pm *PlaybackManager) getStreamPlaybackDetails(mId int) mo.Option[*mediaapi.AnimeListEntry] {
	pm.mu.Lock()
	defer pm.mu.Unlock()

	if pm.animeCollection.IsAbsent() {
		return mo.None[*mediaapi.AnimeListEntry]()
	}
