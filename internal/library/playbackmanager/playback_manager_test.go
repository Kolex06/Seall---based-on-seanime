...
func TestPlaybackManagerUnitCheckOrLoadAnimeCollectionCachesResult(t *testing.T) {
	...
	h := newPlaybackManagerTestWrapper(t)
	expectedCollection := &mediaapi.AnimeCollection{}
	...
	h.playbackManager.animeCollection = mo.None[*mediaapi.AnimeCollection]()
...
func TestPlaybackManagerUnitLocalPlaybackStatusAndProgressTracking(t *testing.T) {
	...
	h.playbackManager.currentMediaListEntry = mo.Some(&mediaapi.AnimeListEntry{
		Media:    media,
		Progress: new(1),
	})
...
func TestPlaybackManagerTrackingStartedUsesNewLocalFileState(t *testing.T) {
	...
	statusCurrent := mediaapi.MediaListStatusCurrent
	oldEntry := &mediaapi.AnimeListEntry{Media: oldMedia, Progress: new(9)}
	newEntry := &mediaapi.AnimeListEntry{Media: newMedia, Progress: new(0)}
	h.playbackManager.SetAnimeCollection(&mediaapi.AnimeCollection{
...
func TestPlaybackManagerUnitStreamPlaybackStatusAndProgressTracking(t *testing.T) {
	...
	media := testmocks.NewBaseAnimeBuilder(201, "Dungeon Meshi").
		...
		WithEpisodes(24).
		Build()
	entry := &mediaapi.AnimeListEntry{Media: media, Progress: new(1)}
	collection := newAnimeCollection(media, entry, mediaapi.MediaListStatusCurrent)
	h.playbackManager.SetAnimeCollection(collection)
...
func TestPlaybackManagerUnitManualProgressTrackingSyncsProgress(t *testing.T) {
	...
	media := testmocks.NewBaseAnimeBuilder(909, "Orb").
		...
		WithEpisodes(25).
		Build()
	entry := &mediaapi.AnimeListEntry{Media: media, Progress: new(4)}
	h.platform = testmocks.NewFakePlatformBuilder().WithAnimeCollection(newAnimeCollection(media, entry, mediaapi.MediaListStatusCurrent)).Build()
	h.playbackManager.platformRef = util.NewRef[platform.Platform](h.platform)
...
func TestPlaybackManagerLiveRepositoryStreamCompletionSyncsProgress(t *testing.T) {
	...
	media := testmocks.NewBaseAnimeBuilder(700, "Lazarus").
		...
		WithEpisodes(13).
		Build()
	h.playbackManager.SetAnimeCollection(newAnimeCollection(media, &mediaapi.AnimeListEntry{
		Media:    media,
		Progress: new(0),
	}, mediaapi.MediaListStatusCurrent))
	h.playbackManager.currentStreamMedia = mo.Some(media)
...
func newAnimeCollection(media *mediaapi.BaseAnime, entry *mediaapi.AnimeListEntry, status mediaapi.MediaListStatus) *mediaapi.AnimeCollection {
