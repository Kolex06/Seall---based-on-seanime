...
type NormalizedMedia struct {
	...
	Synonyms    []*string
	Format      *mediaapi.MediaFormat
	Status      *mediaapi.MediaStatus
	Season      *mediaapi.MediaSeason
	Year        *int
	StartDate   *NormalizedMediaDate
	Episodes    *int
	BannerImage *string
	CoverImage  *NormalizedMediaCoverImage
	//Relations         *mediaapi.CompleteAnimeById_Media_CompleteAnime_Relations
	NextAiringEpisode *NormalizedMediaNextAiringEpisode
	// Whether it was fetched from SIMKL
	fetched bool
}
...
func NewNormalizedMedia(m *mediaapi.BaseAnime) *NormalizedMedia {
...
// The media is marked as not fetched (fetched=false) since it lacks some SIMKL-specific data.
...
func FetchNormalizedMedia(simklClient mediaapi.MediaApiClient, l *limiter.Limiter, cache *mediaapi.CompleteAnimeCache, m *NormalizedMedia) error {
	if simklClient == nil || m == nil {
		return nil
	}
	...
	l.Wait()
	complete, err := simklClient.CompleteAnimeByID(context.Background(), &m.ID)
	if err != nil {
		return err
	}
...
func (m *NormalizedMedia) FetchMediaTree(
	rel mediaapi.FetchMediaTreeRelation,
	simklClient mediaapi.MediaApiClient,
	rl *limiter.Limiter,
	tree *mediaapi.CompleteAnimeRelationTree,
	cache *mediaapi.CompleteAnimeCache,
) error {
	if m == nil {
		return nil
	}
