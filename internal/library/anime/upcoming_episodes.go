...
type (
	...
	UpcomingEpisode struct {
		...
		BaseAnime       *mediaapi.BaseAnime `json:"baseAnime"`
...
func NewUpcomingEpisodes(opts *NewUpcomingEpisodesOptions) *UpcomingEpisodes {
	...
	mediaWithNextAiring := lo.Filter(allMedia, func(item *mediaapi.BaseAnime, _ int) bool {
	...
	for _, media := range mediaWithNextAiring {
		wg.Add(1)
		go func(media *mediaapi.BaseAnime) {
			defer wg.Done()
			...
			if entry.Status == nil || *entry.Status == mediaapi.MediaListStatusDropped {
				return
			}
			...
			rateLimiter.Wait()
			animeMetadata, err := opts.MetadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, media.ID)
