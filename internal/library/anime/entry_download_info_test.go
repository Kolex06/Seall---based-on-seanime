...
func TestNewEntryDownloadInfoEpisodeZeroDiscrepancy(t *testing.T) {
	// simkl counts episode 0 here, but the metadata maps it as S1.
	// the expected list should still expose that extra slot as episode 0.
	h := newAnimeTestWrapper(t)
	mediaID := 146065

	patchEntryMediaStatus(t, h.animeCollection, mediaID, mediaapi.MediaStatusReleasing)
	patchAnimeCollectionEntry(t, h.animeCollection, mediaID, mediaapi.AnimeCollectionEntryPatch{
		AiredEpisodes:     new(6),
		NextAiringEpisode: &mediaapi.BaseAnime_NextAiringEpisode{Episode: 7},
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			info := h.newEntryDownloadInfo(t, mediaID, nil, tt.progress, mediaapi.MediaListStatusCurrent)
...
func TestNewEntryDownloadInfoSpecialsDiscrepancyAndBatchFlags(t *testing.T) {
	// this covers the path where simkl's aired count includes specials.
	...
	patchEntryMediaStatus(t, h.animeCollection, mediaID, mediaapi.MediaStatusFinished)
	...
	info := h.newEntryDownloadInfo(t, mediaID, nil, 0, mediaapi.MediaListStatusCurrent)
...
func TestNewEntryDownloadInfoCompletedRewatchFiltersDownloadedEpisodes(t *testing.T) {
	...
	info := h.newEntryDownloadInfo(t, mediaID, localFiles, 4, mediaapi.MediaListStatusCompleted)
...
func TestNewEntryDownloadInfoScheduleFlags(t *testing.T) {
	...
	t.Run("next airing trims future episodes", func(t *testing.T) {
		...
		patchEntryMediaStatus(t, h.animeCollection, mediaID, mediaapi.MediaStatusReleasing)
		patchAnimeCollectionEntry(t, h.animeCollection, mediaID, mediaapi.AnimeCollectionEntryPatch{
			NextAiringEpisode: &mediaapi.BaseAnime_NextAiringEpisode{Episode: 4},
		})
...
func TestNewEntryDownloadInfoEarlyReturnsAndErrors(t *testing.T) {
	t.Run("not yet released returns empty result", func(t *testing.T) {
		...
		patchEntryMediaStatus(t, h.animeCollection, mediaID, mediaapi.MediaStatusNotYetReleased)
