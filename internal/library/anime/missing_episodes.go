...
func NewMissingEpisodes(opts *NewMissingEpisodesOptions) *MissingEpisodes {
	...
	for mId, lfs := range groupedLfs {
		...
		go func(mId int, lfs []*LocalFile) {
			...
			if entry.Status == nil || *entry.Status == mediaapi.MediaListStatusDropped || *entry.Status == mediaapi.MediaListStatusCompleted {
				return
			}
			...
			animeMetadata, err := opts.MetadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, entry.Media.ID)
			if err != nil {
				return
			}
