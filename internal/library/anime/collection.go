...
type (
	...
	//  - UnknownGroups: a list of UnknownGroup instances. Group of files whose media is not in the user's SIMKL "Resolve unknown media" feature.
	...
	UnmatchedGroup struct {
		...
		Suggestions []*mediaapi.BaseAnime `json:"suggestions"`
	}
	// UnknownGroup holds the data for a group of local files whose media is not in the user's SIMKL.
	// The client will use this data to suggest media to the user, so they can add it to their SIMKL.
	UnknownGroup struct {
		MediaId    int          `json:"mediaId"`
		LocalFiles []*LocalFile `json:"localFiles"`
	}
...
func (lc *LibraryCollection) hydrateCollectionLists(
	localFiles []*LocalFile,
	aniLists []*mediaapi.AnimeCollection_MediaListCollection_Lists,
) {
					...
					if slices.Contains(mIds, entry.Media.ID) {
						...
						return &LibraryCollectionEntry{
							...
							EntryListData: &EntryListData{
								...
								StartedAt:   mediaapi.ToEntryStartDate(entry.StartedAt),
								CompletedAt: mediaapi.ToEntryCompletionDate(entry.CompletedAt),
							},
						}
					} else {
						return nil
	...
	if ok {
		...
		if len(repeatingList.Entries) > 0 && ok {
			...
			newCurrentList.Type = mediaapi.MediaListStatusCurrent
	...
	for _, list := range lc.Lists {
		for _, entry := range list.Entries {
			...
			if entry.Media.Format != nil {
				if *entry.Media.Format == mediaapi.MediaFormatMovie {
					stats.TotalMovies++
				} else if *entry.Media.Format == mediaapi.MediaFormatSpecial || *entry.Media.Format == mediaapi.MediaFormatOva {
					stats.TotalSpecials++
				} else {
					stats.TotalShows++
				}
...
func (lc *LibraryCollection) hydrateUnmatchedGroups() {
	...
	for key, value := range groupedLfs {
		groups = append(groups, &UnmatchedGroup{
			Dir:         key,
			LocalFiles:  value,
			Suggestions: make([]*mediaapi.BaseAnime, 0),
		})
	}
...
// getLibraryCollectionEntryFromListStatus maps mediaapi.MediaListStatus to LibraryCollectionListType.
func getLibraryCollectionEntryFromListStatus(st mediaapi.MediaListStatus) mediaapi.MediaListStatus {
