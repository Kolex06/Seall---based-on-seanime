...
func TestNewLibraryCollectionMergesRepeatingAndHydratesStats(t *testing.T) {
	...
	onePieceEntry := patchAnimeCollectionEntry(t, h.animeCollection, 21, mediaapi.AnimeCollectionEntryPatch{
		Status:   new(mediaapi.MediaListStatusRepeating),
		Progress: new(1060),
	})
	mushokuEntry := patchAnimeCollectionEntry(t, h.animeCollection, 146065, mediaapi.AnimeCollectionEntryPatch{
	...
	ovaFormat := mediaapi.MediaFormatOva
	...
	currentList := findCollectionListByStatus(t, libraryCollection, mediaapi.MediaListStatusCurrent)
	...
	require.Nil(t, findOptionalCollectionListByStatus(libraryCollection, mediaapi.MediaListStatusRepeating))
	...
	require.Equal(t, mediaapi.MediaListStatusRepeating, *repeatingEntry.EntryListData.Status)
...
func findCollectionListByStatus(t *testing.T, libraryCollection *anime.LibraryCollection, status mediaapi.MediaListStatus) *anime.LibraryCollectionList {
	t.Helper()
...
func findOptionalCollectionListByStatus(libraryCollection *anime.LibraryCollection, status mediaapi.MediaListStatus) *anime.LibraryCollectionList {
