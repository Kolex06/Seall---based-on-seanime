...
type (
	...
	Episode struct {
		...
		ProgressNumber        int                 `json:"progressNumber"` // Usually the same as EpisodeNumber, unless there is a discrepancy between SIMKL and AniDB
		...
		MetadataIssue         string              `json:"metadataIssue,omitempty"` // Alerts the user that there is a discrepancy between SIMKL and AniDB
		BaseAnime             *mediaapi.BaseAnime `json:"baseAnime,omitempty"`
	...
	// Metadata is fetched from Animap (AniDB) and, optionally, SIMKL (if Animap is not available).
...
type (
	...
	NewEpisodeOptions struct {
		...
		// This is used when there is a discrepancy between SIMKL and AniDB
	...
	if opts.LocalFile != nil {
		...
		if len(entryEp.DisplayTitle) == 0 {
			...
			case LocalFileTypeMain:
				if foundAnimapEpisode {
					entryEp.AniDBEpisode = aniDBEp
					if *opts.Media.GetFormat() == mediaapi.MediaFormatMovie {
...
// NewEpisodeMetadata creates a new EpisodeMetadata from an Animap episode and SIMKL media.
