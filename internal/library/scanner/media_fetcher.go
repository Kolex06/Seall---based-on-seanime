...
import (
	"context"
	"errors"
	"seall/internal/api/animeofflinedb"
	"seall/internal/api/mal"
...
// MediaFetcher holds all mediaapi.BaseAnime that will be used for the comparison process
type MediaFetcher struct {
	...
	AnimeCollectionWithRelations *mediaapi.AnimeCollectionWithRelations
...
type MediaFetcherOptions struct {
	...
	OptionalAnimeCollection *mediaapi.AnimeCollection
}

// NewMediaFetcher
// Calling this method will kickstart the fetch process
// When enhancing is false, MediaFetcher.AllMedia will be all mediaapi.BaseAnime from the user's SIMKL collection.
// When enhancing is true, MediaFetcher.AllMedia will be mediaapi.BaseAnime for each unique, parsed anime title and their relations.
func NewMediaFetcher(ctx context.Context, opts *MediaFetcherOptions) (ret *MediaFetcher, retErr error) {
	...
	// Fetch latest user's SIMKL collection
	animeCollectionWithRelations, err := opts.PlatformRef.Get().GetMediaCollectionWithRelations(ctx)
	if err != nil {
		return nil, err
	}
	...
	allCompleteAnime := make([]*mediaapi.CompleteAnime, 0)
	...
	if mf.ScanLogger != nil {
		mf.ScanLogger.LogMediaFetcher(zerolog.DebugLevel).
			...
			Msg("Fetched media from SIMKL collection")
	}

	//--------------------------------------------

	// Get the media IDs from the collection
	mf.CollectionMediaIds = lop.Map(allCompleteAnime, func(m *mediaapi.CompleteAnime, index int) int {
		return m.ID
	})
	...
	if opts.Enhanced && !opts.EnhanceWithOfflineDatabase {
		...
		if ok {
			...
			allCompleteAnime = make([]*mediaapi.CompleteAnime, 0)
			opts.CompleteAnimeCache.Range(func(key int, value *mediaapi.CompleteAnime) bool {
	...
	mf.AllMedia = NormalizedMediaFromSimklComplete(allCompleteAnime)
...
func NormalizedMediaFromSimklComplete(c []*mediaapi.CompleteAnime) []*anime.NormalizedMedia {
	...
	for _, m := range c {
		...
		if m.Relations != nil && m.Relations.Edges != nil && len(m.Relations.Edges) > 0 {
			for _, edgeM := range m.Relations.Edges {
				...
				if *edgeM.Node.Format != mediaapi.MediaFormatMovie &&
					*edgeM.Node.Format != mediaapi.MediaFormatOva &&
					*edgeM.Node.Format != mediaapi.MediaFormatSpecial &&
					*edgeM.Node.Format != mediaapi.MediaFormatTv {
					continue
				}
				if *edgeM.RelationType != mediaapi.MediaRelationPrequel &&
					*edgeM.RelationType != mediaapi.MediaRelationSequel &&
					*edgeM.RelationType != mediaapi.MediaRelationSpinOff &&
					*edgeM.RelationType != mediaapi.MediaRelationAlternative &&
					*edgeM.RelationType != mediaapi.MediaRelationParent {
					continue
				}
...
// uses these search results to get SIMKL IDs using metadata.AnimeMetadata mappings,
// queries SIMKL to retrieve all mediaapi.BaseAnime using mediaapi.GetBaseAnimeById and their relations using mediaapi.FetchMediaTree.
...
func FetchMediaFromLocalFiles(
	...
	localFiles []*anime.LocalFile,
	completeAnime *mediaapi.CompleteAnimeCache,
	metadataProvider metadata_provider.Provider,
	simklRateLimiter *limiter.Limiter,
	scanLogger *ScanLogger,
) (ret []*mediaapi.CompleteAnime, ok bool) {
	...
	// This step is necessary because MAL doesn't provide SIMKL IDs and some MAL media don't exist on SIMKL
	...
	// Retrieve the SIMKL IDs from the Animap mappings stored in the cache
	simklIds := make([]int, 0)
	metadataProvider.GetCache().Range(func(key string, value *metadata.AnimeMetadata) bool {
		if value != nil {
			simklIds = append(simklIds, value.GetMappings().MediaId)
		}
		return true
	})

	// Fetch all media from the SIMKL IDs
	simklMedia := make([]*mediaapi.CompleteAnime, 0)
	lop.ForEach(simklIds, func(id int, index int) {
		simklRateLimiter.Wait()
		var media *mediaapi.CompleteAnime
		var err error
		...
		if err == nil {
			simklMedia = append(simklMedia, media)
			if scanLogger != nil {
				scanLogger.LogMediaFetcher(zerolog.DebugLevel).
					Str("module", "Enhanced").
					Str("title", media.GetTitleSafe()).
					Msg("Fetched Simkl media")
			}
		} else {
			if scanLogger != nil {
				scanLogger.LogMediaFetcher(zerolog.WarnLevel).
					Str("module", "Enhanced").
					Int("id", id).
					Msg("Failed to fetch Simkl media")
			}
		}
	})
	...
	if scanLogger != nil {
		scanLogger.LogMediaFetcher(zerolog.DebugLevel).
			Str("module", "Enhanced").
			Str("context", spew.Sprint(lo.Map(simklMedia, func(n *mediaapi.CompleteAnime, _ int) string {
				return n.GetTitleSafe()
			}))).
			Msg("Fetched Simkl media from MAL ids")
	}
	...
	lop.ForEach(simklMedia, func(m *mediaapi.CompleteAnime, index int) {
		// We ignore errors because we want to continue even if one of the media fails
		_ = m.FetchMediaTree(mediaapi.FetchMediaTreeAll, platform.GetMediaApiClient(), simklRateLimiter, tree, completeAnime)
	})
	...
	scanned := make([]*mediaapi.CompleteAnime, 0)
	completeAnime.Range(func(key int, value *mediaapi.CompleteAnime) bool {
	...
	if scanLogger != nil {
		scanLogger.LogMediaFetcher(zerolog.InfoLevel).
			...
			Str("context", spew.Sprint(lo.Map(scanned, func(n *mediaapi.CompleteAnime, _ int) string {
				return n.GetTitleSafe()
			}))).
