...
func TestMatcher1(t *testing.T) {
	...
	animeCollection, err := simklClient.AnimeCollectionWithRelations(context.Background(), nil)
	if err != nil {
		t.Fatal(err.Error())
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mediaapi.EnsureAnimeCollectionWithRelationsEntry(animeCollection, tt.expectedMediaId, mediaapi.AnimeCollectionEntryPatch{Status: &currentStatus}, simklClient)
			for _, otherMediaId := range tt.otherMediaIds {
				mediaapi.EnsureAnimeCollectionWithRelationsEntry(animeCollection, otherMediaId, mediaapi.AnimeCollectionEntryPatch{Status: &currentStatus}, simklClient)
			}
			...
			mc := NewMediaContainer(&MediaContainerOptions{
				AllMedia:   NormalizedMediaFromSimklComplete(allMedia),
...
func TestMatcher2(t *testing.T) {
	...
	animeCollection, err := wrapper.Platform.GetMediaCollectionWithRelations(t.Context())
	if err != nil {
		t.Fatal(err.Error())
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, id := range tt.otherMediaIds {
				mediaapi.EnsureAnimeCollectionWithRelationsEntry(animeCollection, id, mediaapi.AnimeCollectionEntryPatch{Status: &currentStatus}, simklClient)
			}
			allMedia := animeCollection.GetAllAnime()
...
func TestMatcherWithOfflineDB(t *testing.T) {
	...
	mf, err := NewMediaFetcher(t.Context(), &MediaFetcherOptions{
		...
		Logger:                     logger,
		MediaApiRateLimiter:        wrapper.MediaApiRateLimiter,
		ScanLogger:                 scanLogger,
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, expectedID := range tt.expectedMediaIds {
				mediaapi.EnsureAnimeCollectionWithRelationsEntry(
					animeCollection,
					expectedID,
					mediaapi.AnimeCollectionEntryPatch{Status: &currentStatus},
					simklClient,
				)
			}
