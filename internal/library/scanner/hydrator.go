...
// HydrateMetadata will hydrate the metadata of each LocalFile with the metadata of the matched mediaapi.BaseAnime.
...
func (fh *FileHydrator) hydrateGroupMetadata(
	...
	_ = anime.FetchNormalizedMedia(fh.PlatformRef.Get().GetMediaApiClient(), fh.MediaApiRateLimiter, fh.CompleteAnimeCache, media)
	...
	lop.ForEach(lfs, func(lf *anime.LocalFile, index int) {
		...
		if lf.IsProbablySpecial() {
			...
			if *media.Format == mediaapi.MediaFormatMovie {
		...
		// - Or, the torrent files were divided into multiple episodes from a media that is listed as a movie on SIMKL
		...
		if episode > media.GetCurrentEpisodeCount() && fh.ForceMediaId == 0 {
			...
			if !treeFetched {
				...
				if err := media.FetchMediaTree(mediaapi.FetchMediaTreeAll, fh.PlatformRef.Get().GetMediaApiClient(), fh.MediaApiRateLimiter, tree, fh.CompleteAnimeCache); err == nil {
		...
		if fh.ForceMediaId != 0 && episode > media.GetCurrentEpisodeCount() {
			...
			// we have a forced media ID, we will fetch the media from SIMKL and get the offset
			animeMetadata, err := fh.MetadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, fh.ForceMediaId)
			if err != nil {
				/*Log */
				if fh.ScanLogger != nil {
					fh.logFileHydration(zerolog.ErrorLevel, lf, mId, episode).
...
func (fh *FileHydrator) tryPartRelativeNormalization(
	...
	animeMetadata, err := fh.MetadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, mediaId)
	if err != nil {
		return 0, false
	}
