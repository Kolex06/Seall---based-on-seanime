...
func NewMediaTreeAnalysis(opts *MediaTreeAnalysisOptions) (*MediaTreeAnalysis, error) {
	...
	relations := make([]*mediaapi.CompleteAnime, 0)
	opts.tree.Range(func(key int, value *mediaapi.CompleteAnime) bool {
	...
	for _, rel := range relations {
		p.Go(func() (*MediaTreeAnalysisBranch, error) {
			...
			animeMetadata, err := opts.metadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, rel.ID)
			if err != nil {
				return nil, err
			}
			...
			// discrepancy check: if anidb has 1 main episode but simkl has more than 1 (e.g. movie split into parts)
			// use the simkl episode count
