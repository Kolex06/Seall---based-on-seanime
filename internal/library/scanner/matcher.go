...
// MatchLocalFilesWithMedia will match each anime.LocalFile with a specific mediaapi.BaseAnime and modify the LocalFile's `mediaId`
...
func calculateFormatScore(fileFormat fileFormatType, media *anime.NormalizedMedia) float64 {
	...
	switch fileFormat {
	case fileFormatOVA:
		if mediaFormat == mediaapi.MediaFormatOva || mediaFormat == mediaapi.MediaFormatSpecial {
			return scoreFormatMatch
		}
		if mediaFormat == mediaapi.MediaFormatTv || mediaFormat == mediaapi.MediaFormatTvShort {
			return scoreFormatMismatch
		}

	case fileFormatSpecial:
		if mediaFormat == mediaapi.MediaFormatSpecial || mediaFormat == mediaapi.MediaFormatOva {
			return scoreFormatMatch
		}
		if mediaFormat == mediaapi.MediaFormatTv || mediaFormat == mediaapi.MediaFormatTvShort {
			return scoreFormatMismatch
	...
	case fileFormatMovie:
		if mediaFormat == mediaapi.MediaFormatMovie {
			return scoreFormatMatch
		}
		if mediaFormat == mediaapi.MediaFormatTv || mediaFormat == mediaapi.MediaFormatTvShort {
			return scoreFormatMismatch
