...
func mergeNormalizedTorrents(groups ...[]*NormalizedTorrent) []*NormalizedTorrent {
	...
	for _, group := range groups {
		for _, torrent := range group {
			if torrent == nil || torrent.MediaTorrent == nil {
				continue
			}
...
func (ad *AutoDownloader) fetchTorrentsFromProviders(
	ctx context.Context,
	providers []extension.MediaTorrentProviderExtension,
	...
	for _, providerExt := range providers {
		wg.Add(1)
		go func(pExt extension.MediaTorrentProviderExtension) {
			defer wg.Done()
...
func (t *NormalizedTorrent) GetMagnet(providerExtension hibiketorrent.AnimeProvider) (string, error) {
	if t.magnet == "" {
		magnet, err := providerExtension.GetTorrentMagnetLink(t.MediaTorrent)
		if err != nil {
			return "", err
		}
		t.magnet = magnet
