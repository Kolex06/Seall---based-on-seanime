...
func newTestAnimeCollection(t *testing.T, mediaId int) *mediaapi.AnimeCollection {
	t.Helper()

	simklClient := mediaapi.NewTestMediaApiClient()
	animeCollection, err := simklClient.AnimeCollection(context.Background(), nil)
...
func TestIsConstraintsMatch(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			torrent:  &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Seeders: 10}},
			rule:     &anime.AutoDownloaderRule{MinSeeders: 5},
			expected: true,
		},
		{
			name:     "Min seeders pass (no data)",
			torrent:  &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Seeders: -1}},
			rule:     &anime.AutoDownloaderRule{MinSeeders: 5},
			expected: true,
		},
		{
			name:     "Min seeders fail",
			torrent:  &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Seeders: 2}},
			rule:     &anime.AutoDownloaderRule{MinSeeders: 5},
			expected: false,
		},
		{
			name:     "Min size pass",
			torrent:  &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Size: 2048}}, // 2KB
			rule:     &anime.AutoDownloaderRule{MinSize: "1KB"},
			expected: true,
		},
		{
			name:     "Min size pass (no data)",
			torrent:  &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Size: 0}},
			rule:     &anime.AutoDownloaderRule{MinSize: "1KB"},
			expected: true,
		},
		{
			name:     "Min size fail",
			torrent:  &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Size: 512}}, // 0.5KB
			rule:     &anime.AutoDownloaderRule{MinSize: "1KB"},
			expected: false,
		},
		{
			name:     "Max size pass",
			torrent:  &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Size: 1024}}, // 1KB
			rule:     &anime.AutoDownloaderRule{MaxSize: "2KB"},
			expected: true,
		},
		{
			name:     "Max size fail",
			torrent:  &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Size: 3072}}, // 3KB
			rule:     &anime.AutoDownloaderRule{MaxSize: "2KB"},
			expected: false,
		},
	}
...
func TestIsTorrentAlreadyDownloaded(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "torrent exists",
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{InfoHash: " HASH2 "},
			},
			expected: true,
		},
		{
			...
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{InfoHash: "hash3"},
			},
			expected: true,
		},
		{
			...
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{InfoHash: "hash999"},
			},
			expected: false,
		},
	}
...
func TestCalculateCandidateScore(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[SubsPlease] One Piece - 1000 (1080p) [ABCD].mkv"},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
					MinimumScore: 10,
...
func TestSelectBestCandidate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			candidates: []*Candidate{
				{Score: 10, Torrent: &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "a1", Seeders: 50}}},
				{Score: 25, Torrent: &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "a2", Seeders: 30}}},
				{Score: 15, Torrent: &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "a3", Seeders: 100}}},
			},
			expectedTorrent: "a2",
		},
		{
			...
			candidates: []*Candidate{
				{Score: 20, Torrent: &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "a1", Seeders: 50}}},
				{Score: 20, Torrent: &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "a2", Seeders: 150}}},
				{Score: 20, Torrent: &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "a3", Seeders: 100}}},
			},
			expectedTorrent: "a2",
		},
		{
			...
			candidates: []*Candidate{
				{Score: 10, Torrent: &NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "a1", Seeders: 10}}},
			},
			expectedTorrent: "a1",
		},
	}
...
func TestTorrentScoring(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[SubsPlease] Shangri-La Frontier S02 - 14 (1080p).mkv"},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
					Conditions: []anime.AutoDownloaderCondition{
		...
		},
		{
			...
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[ASW] Solo Leveling - S02E01 [1080p HEVC x265 10bit] (Batch)"},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
					Conditions: []anime.AutoDownloaderCondition{
		...
		},
		{
			...
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[EMBER] Frieren - 01 [BDRip] [1080p] [HEVC]"},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
					Conditions: []anime.AutoDownloaderCondition{
		...
		},
		{
			...
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[Nippon-Yasan] Monogatari Series - Off & Monster Season - 01 (1080p).mkv"},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
					Name: "Global Format",
		...
		},
		{
			...
			torrent: &NormalizedTorrent{
				MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[SubsPlease] Dandadan - 01v2 (1080p).mkv"},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
					Conditions: []anime.AutoDownloaderCondition{
...
func TestTorrentRanking(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			candidates: []*NormalizedTorrent{
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[SubsPlease] Sousou no Frieren - 01 (1080p) [Web-DL].mkv", Seeders: 3500, Size: 1400000000}},
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[Erai-raws] Sousou no Frieren - 01 [1080p].mkv", Seeders: 1200, Size: 1600000000}},
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[CoalGirls] Sousou no Frieren - 01 [BDRip 1920x1080 HEVC FLAC].mkv", Seeders: 85, Size: 5200000000}},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
		...
		},
		{
			...
			candidates: []*NormalizedTorrent{
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[Judas] Kaiju No. 8 - 01 [1080p][HEVC x265 10bit].mkv", Seeders: 2200}},
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[SubsPlease] Kaiju No. 8 - 01 (1080p).mkv", Seeders: 1800}},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
					Name: "H264 Only",
		...
		},
		{
			...
			candidates: []*NormalizedTorrent{
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[SubsPlease] Dungeon Meshi - 01 (1080p).mkv", Seeders: 2800}},
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[EMBER] Dungeon Meshi - 01 (1080p) [Dual Audio].mkv", Seeders: 210}},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
					Name: "English Dub",
		...
		},
		{
			...
			candidates: []*NormalizedTorrent{
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[Group] Metallic Rouge - 01 [1080p].mkv", Seeders: 900}},
				{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[Group] Metallic Rouge - 01v2 [1080p].mkv", Seeders: 340}},
			},
			profiles: []*anime.AutoDownloaderProfile{
				{
...
func TestIntegration(t *testing.T) {
	...
	tests := []struct {
		...
		mediaId          int
		userProgress     int
		torrents         []*hibiketorrent.MediaTorrent
		rule             *anime.AutoDownloaderRule
		profiles         []*anime.AutoDownloaderProfile
		expectedResults  int
			...
			existingItems: []*models.AutoDownloaderItem{
				{
					...
					TorrentData: mustMarshalTorrent(&NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "[SubsPlease] Sousou no Frieren - 01 (1080p).mkv", InfoHash: "hash1"}, ExtensionID: "fake"}),
				},
			},
			profile: &anime.AutoDownloaderProfile{
				Conditions:   []anime.AutoDownloaderCondition{{Term: "1080p", Action: anime.AutoDownloaderProfileRuleFormatActionScore, Score: 10}},
			...
			existingItems: []*models.AutoDownloaderItem{
				{
					...
					TorrentData: mustMarshalTorrent(&NormalizedTorrent{MediaTorrent: &hibiketorrent.MediaTorrent{Name: "Name", InfoHash: "hash_bad"}, ExtensionID: "fake"}),
				},
			},
			profile: &anime.AutoDownloaderProfile{
				Conditions: []anime.AutoDownloaderCondition{
