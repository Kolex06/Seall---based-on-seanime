...
func (ad *AutoDownloader) SetAnimeCollection(ac *mediaapi.AnimeCollection) {
...
func (ad *AutoDownloader) fetchRunData(ctx context.Context, ruleIDs ...uint) (*runData, error) {
	...
	beforeFetchEvent := &AutoDownloaderBeforeFetchTorrentsEvent{
		Rules:           rules,
		Profiles:        profiles,
		ProviderIDs:     lo.Map(providerExtensions, func(ext extension.MediaTorrentProviderExtension, _ int) string { return ext.GetID() }),
...
func (ad *AutoDownloader) torrentFollowsRule(
	t *NormalizedTorrent,
	rule *anime.AutoDownloaderRule,
	listEntry *mediaapi.AnimeListEntry,
	profiles []*anime.AutoDownloaderProfile,
) (int, bool) {
...
func (ad *AutoDownloader) isTitleMatch(torrentParsedData *habari.Metadata, torrentName string, rule *anime.AutoDownloaderRule, listEntry *mediaapi.AnimeListEntry) (ok bool) {
	...
	case anime.AutoDownloaderRuleTitleComparisonLikely:
		...
		if parsedComparisonTitle.Title != "" && len(parsedComparisonTitle.SeasonNumber) > 0 {
			...
			// e.g. Torrent: "[Seall] Jujutsu Kaisen 2nd Season - 20 [...].mkv" -> "Jujutsu Kaisen"
...
func (ad *AutoDownloader) isSeasonAndEpisodeMatch(
	...
	if !ok {
		...
		if listEntry.GetMedia().GetCurrentEpisodeCount() == 1 || *listEntry.GetMedia().GetFormat() == mediaapi.MediaFormatMovie {
	...
	if rule.CustomEpisodeNumberAbsoluteOffset > 0 {
		...
		if listEntry.GetMedia().GetCurrentEpisodeCount() != -1 && episode > listEntry.GetMedia().GetCurrentEpisodeCount() {
			...
			ad.mu.Lock()
			animeMetadata, err := ad.metadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, listEntry.GetMedia().GetID())
...
func (ad *AutoDownloader) getRuleListEntry(rule *anime.AutoDownloaderRule) (*mediaapi.AnimeListEntry, bool) {
...
func (ad *AutoDownloader) getProvidersForRules(rules []*anime.AutoDownloaderRule, profiles []*anime.AutoDownloaderProfile) []extension.MediaTorrentProviderExtension {
	...
	ret := make([]extension.MediaTorrentProviderExtension, 0)
