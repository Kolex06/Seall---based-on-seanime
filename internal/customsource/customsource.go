...
const (
	...
	// ExtensionIdOffset uses the sign bit to separate custom source IDs from SIMKL IDs
	//	SIMKL IDs: 0 to 2^31-1 (31 bits)
...
func (m *Manager) GetProviderFromBaseAnime(baseAnime *mediaapi.BaseAnime) (ext extension.CustomSourceExtension, localId int, isCustom bool, extensionExists bool) {
	if baseAnime == nil {
...
func (m *Manager) GetProviderFromBaseManga(baseManga *mediaapi.BaseManga) (ext extension.CustomSourceExtension, localId int, isCustom bool, extensionExists bool) {
	if baseManga == nil {
		return nil, 0, false, false
	}
...
func formatSiteUrl(extId string, siteUrl *string) *string {
	...
	if strings.HasPrefix(*siteUrl, "https://mediaapi.co") {
		return siteUrl
	}
...
func NormalizeMedia(extensionIdentifier int, extId string, obj interface{}) {
	...
	case *mediaapi.BaseAnime:
		...
		if v.Title == nil {
			v.Title = &mediaapi.BaseAnime_Title{
				UserPreferred: new("???"),
				English:       new("???"),
				Romaji:        nil,
				Native:        nil,
	...
	case *mediaapi.CompleteAnime:
		...
		if v.Title == nil {
			v.Title = &mediaapi.CompleteAnime_Title{
				UserPreferred: new("???"),
				English:       new("???"),
				Romaji:        nil,
				Native:        nil,
		...
		if v.Relations != nil {
			for _, edge := range v.Relations.Edges {
				if edge.Node != nil {
					// don't normalize if media comes from simkl
					if edge.Node.SiteURL != nil && strings.HasPrefix(*edge.Node.SiteURL, "https://mediaapi.co") {
						continue
					}
	...
	case *mediaapi.BaseManga:
		...
		if v.Title == nil {
			v.Title = &mediaapi.BaseManga_Title{
				UserPreferred: new("???"),
				English:       new("???"),
				Romaji:        nil,
				Native:        nil,
	...
	case *mediaapi.AnimeDetailsById_Media:
		v.ID = GenerateMediaId(extensionIdentifier, v.ID)
		v.SiteURL = formatSiteUrl(extId, v.SiteURL)
		if v.Relations != nil {
			for _, edge := range v.Relations.Edges {
		...
		if v.Recommendations != nil {
			for _, edge := range v.Recommendations.Edges {
				if edge.Node != nil && edge.Node.MediaRecommendation != nil {
					// don't normalize if media comes from simkl
					if edge.Node.MediaRecommendation.SiteURL != nil && strings.HasPrefix(*edge.Node.MediaRecommendation.SiteURL, "https://mediaapi.co") {
						continue
					}
	...
	case *mediaapi.MangaDetailsById_Media:
		v.ID = GenerateMediaId(extensionIdentifier, v.ID)
		v.SiteURL = formatSiteUrl(extId, v.SiteURL)
		if v.Relations != nil {
			for _, edge := range v.Relations.Edges {
...
func (m *Manager) SaveCustomSourceAnimeEntries(extId string, input map[int]*mediaapi.AnimeListEntry) error {
	return m._saveCustomSourceEntries(extId, AnimeType, input)
}

func (m *Manager) SaveCustomSourceMangaEntries(extId string, input map[int]*mediaapi.MangaListEntry) error {
	return m._saveCustomSourceEntries(extId, MangaType, input)
}

func (m *Manager) GetCustomSourceAnimeEntries() (map[string]map[int]*mediaapi.AnimeListEntry, bool) {
	lc, ok := m._getAllCustomSourceEntries(AnimeType)
	if !ok {
		return nil, false
	}

	extEntries := make(map[string]map[int]*mediaapi.AnimeListEntry)

	for _, source := range lc {
		var entries map[int]*mediaapi.AnimeListEntry
		...
		refreshedEntries := make(map[int]*mediaapi.AnimeListEntry)
		...
		if len(localIds) > 0 {
			...
			if err == nil {
				mediaMap := make(map[int]*mediaapi.BaseAnime)
				for _, m := range media {
					mediaMap[m.ID] = m
				}
...
func (m *Manager) GetCustomSourceMangaCollection() (map[string]map[int]*mediaapi.MangaListEntry, bool) {
	lc, ok := m._getAllCustomSourceEntries(MangaType)
	if !ok {
		return nil, false
	}

	extEntries := make(map[string]map[int]*mediaapi.MangaListEntry)

	for _, source := range lc {
		var entries map[int]*mediaapi.MangaListEntry
		err := json.Unmarshal(source.Value, &entries)
		if err != nil {
			continue
		}
		...
		// Refresh media data from the extension
		refreshedEntries := make(map[int]*mediaapi.MangaListEntry)
		localIds := make([]int, 0, len(entries))
		for localId := range entries {
			localIds = append(localIds, localId)
		}
		...
		if len(localIds) > 0 {
			...
			if err == nil {
				mediaMap := make(map[int]*mediaapi.BaseManga)
				for _, m := range media {
					mediaMap[m.ID] = m
				}
...
func (m *Manager) UpdateEntry(ctx context.Context, mediaID int, status *mediaapi.MediaListStatus, scoreRaw *int, progress *int, startedAt *mediaapi.FuzzyDateInput, completedAt *mediaapi.FuzzyDateInput) error {
	...
	if err == nil && len(media) > 0 {
		...
		entries := make(map[int]*mediaapi.AnimeListEntry)
		if hasAnime {
		...
		if startedAt != nil {
			newEntry.StartedAt = &mediaapi.AnimeCollection_MediaListCollection_Lists_Entries_StartedAt{
				Year:  startedAt.Year,
				Month: startedAt.Month,
				Day:   startedAt.Day,
			}
		...
		if completedAt != nil {
			newEntry.CompletedAt = &mediaapi.AnimeCollection_MediaListCollection_Lists_Entries_CompletedAt{
				Year:  completedAt.Year,
				Month: completedAt.Month,
				Day:   completedAt.Day,
			}
	...
	if err == nil && len(mangaMedia) > 0 {
		...
		entries := make(map[int]*mediaapi.MangaListEntry)
		if hasManga {
		...
		newEntry := &mediaapi.MangaListEntry{
			Status:   status,
			Progress: progress,
			Media:    mangaMedia[0],
		}
		...
		if startedAt != nil {
			newEntry.StartedAt = &mediaapi.MangaCollection_MediaListCollection_Lists_Entries_StartedAt{
				Year:  startedAt.Year,
				Month: startedAt.Month,
				Day:   startedAt.Day,
			}
		...
		if completedAt != nil {
			newEntry.CompletedAt = &mediaapi.MangaCollection_MediaListCollection_Lists_Entries_CompletedAt{
				Year:  completedAt.Year,
				Month: completedAt.Month,
				Day:   completedAt.Day,
			}
...
func (m *Manager) MergeAnimeEntries(collection *mediaapi.AnimeCollection) {
	...
	for extId, entries := range customEntries {
		...
		entriesByStatus := make(map[mediaapi.MediaListStatus][]*mediaapi.AnimeCollection_MediaListCollection_Lists_Entries)
		...
		for localId, entry := range entries {
			...
			collectionEntry := &mediaapi.AnimeCollection_MediaListCollection_Lists_Entries{
				ID:          mediaId,
				Status:      entry.Status,
				Score:       entry.Score,
		...
		for status, statusEntries := range entriesByStatus {
			...
			var targetList *mediaapi.AnimeCollection_MediaListCollection_Lists
			...
			if targetList == nil {
				...
				targetList = &mediaapi.AnimeCollection_MediaListCollection_Lists{
					Status:  &status,
...
func (m *Manager) MergeMangaEntries(collection *mediaapi.MangaCollection) {
	...
	for extId, entries := range customEntries {
		...
		// Group entries by status for collection lists
		entriesByStatus := make(map[mediaapi.MediaListStatus][]*mediaapi.MangaCollection_MediaListCollection_Lists_Entries)
		...
		for localId, entry := range entries {
			...
			// Create collection entry
			collectionEntry := &mediaapi.MangaCollection_MediaListCollection_Lists_Entries{
				ID:          mediaId,
				Status:      entry.Status,
				Score:       entry.Score,
				Progress:    entry.Progress,
		...
		for status, statusEntries := range entriesByStatus {
			// Find or create the list for this status
			var targetList *mediaapi.MangaCollection_MediaListCollection_Lists
			for _, list := range collection.MediaListCollection.Lists {
				if list.Status != nil && *list.Status == status {
					targetList = list
					break
			...
			if targetList == nil {
				// Create new list for this status
				targetList = &mediaapi.MangaCollection_MediaListCollection_Lists{
					Status:  &status,
