...
import (
	...
	hibikecustomsource "seall/internal/extension/hibike/customsource"
...
// and that plain SIMKL IDs are not mistaken for custom source IDs.
...
// This protects the split between extension-owned URLs and normal SIMKL URLs.
func TestSiteURLHelpers(t *testing.T) {
	...
	t.Run("keeps SIMKL urls untouched", func(t *testing.T) {
		// SIMKL URLs are intentionally left alone so downstream code can still treat them as native SIMKL media.
		aniListURL := "https://mediaapi.co/anime/1"
...
func TestNormalizeMedia(t *testing.T) {
	t.Run("normalizes anime ids urls and title fallback", func(t *testing.T) {
		// Normalization rewrites both the ID and the site URL so the rest of the app can tell this apart from SIMKL media.
	...
	t.Run("fills missing manga title", func(t *testing.T) {
		...
		manga := &mediaapi.BaseManga{ID: 30}
...
func TestManagerProviderResolution(t *testing.T) {
	...
	baseAnimeExt, animeLocalID, animeIsCustom, animeExists := manager.GetProviderFromBaseAnime(&mediaapi.BaseAnime{ID: customID})
	require.True(t, animeIsCustom)
	require.True(t, animeExists)
	require.NotNil(t, baseAnimeExt)
	require.Equal(t, 55, animeLocalID)

	baseMangaExt, mangaLocalID, mangaIsCustom, mangaExists := manager.GetProviderFromBaseManga(&mediaapi.BaseManga{ID: 123})
...
func TestGetCustomSourceAnimeEntriesRefreshesMedia(t *testing.T) {
	...
	require.NoError(t, manager.SaveCustomSourceAnimeEntries("anime-ext", map[int]*mediaapi.AnimeListEntry{
		101: {
			ID:     101,
	...
	require.NoError(t, manager.SaveCustomSourceAnimeEntries("missing-ext", map[int]*mediaapi.AnimeListEntry{
...
func TestUpdateEntryAnimeLifecycle(t *testing.T) {
	...
	score := 84
	progress := 6
	startedAt := &mediaapi.FuzzyDateInput{Year: new(2024), Month: new(2), Day: new(10)}
	completedAt := &mediaapi.FuzzyDateInput{Year: new(2024), Month: new(3), Day: new(1)}
	...
	require.Equal(t, mediaapi.MediaListStatusCompleted, *entry.Status)
...
// Verifies that custom source anime entries are merged into an existing SIMKL collection
...
func TestMergeAnimeEntries(t *testing.T) {
	...
	require.NoError(t, manager.SaveCustomSourceAnimeEntries("merge-ext", map[int]*mediaapi.AnimeListEntry{
		41: {
			ID:       41,
			Status:   new(mediaapi.MediaListStatusCurrent),
			Progress: new(4),
	...
	collection := &mediaapi.AnimeCollection{
		MediaListCollection: &mediaapi.AnimeCollection_MediaListCollection{
			Lists: []*mediaapi.AnimeCollection_MediaListCollection_Lists{{
				Status:  new(mediaapi.MediaListStatusPlanning),
				Entries: []*mediaapi.AnimeCollection_MediaListCollection_Lists_Entries{},
			}},
		},
	}

	// Merge keeps the existing SIMKL lists and appends a generated list for the custom source status bucket.
	manager.MergeAnimeEntries(collection)

	require.Len(t, collection.MediaListCollection.Lists, 2)
	currentList := findAnimeListByStatus(t, collection, mediaapi.MediaListStatusCurrent)
...
func findAnimeListByStatus(t *testing.T, collection *mediaapi.AnimeCollection, status mediaapi.MediaListStatus) *mediaapi.AnimeCollection_MediaListCollection_Lists {
	t.Helper()
...
func newBaseAnime(id int, title string, siteURL string) *mediaapi.BaseAnime {
	return &mediaapi.BaseAnime{
		ID:      id,
		SiteURL: new(siteURL),
		Title: &mediaapi.BaseAnime_Title{
			English: new(title),
...
func newBaseManga(id int, title string, siteURL string) *mediaapi.BaseManga {
	return &mediaapi.BaseManga{
		ID:      id,
		SiteURL: new(siteURL),
...
func (f *fakeCustomSourceProvider) GetAnime(_ context.Context, ids []int) ([]*mediaapi.BaseAnime, error) {
	if f.animeErr != nil {
		return nil, f.animeErr
	}

	ret := make([]*mediaapi.BaseAnime, 0, len(ids))
	for _, id := range ids {
...
func (f *fakeCustomSourceProvider) GetAnimeWithRelations(_ context.Context, _ int) (*mediaapi.CompleteAnime, error) {
	return nil, nil
}
...
func (f *fakeCustomSourceProvider) GetAnimeDetails(_ context.Context, _ int) (*mediaapi.AnimeDetailsById_Media, error) {
	return nil, nil
}

func (f *fakeCustomSourceProvider) GetManga(_ context.Context, ids []int) ([]*mediaapi.BaseManga, error) {
	if f.mangaErr != nil {
		return nil, f.mangaErr
	}

	ret := make([]*mediaapi.BaseManga, 0, len(ids))
	for _, id := range ids {
...
func (f *fakeCustomSourceProvider) GetMangaDetails(_ context.Context, _ int) (*mediaapi.MangaDetailsById_Media, error) {
	return nil, nil
}
