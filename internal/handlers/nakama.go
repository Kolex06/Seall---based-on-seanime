...
//	@route /api/v1/nakama/host/media/library [GET]
...
//	@route /api/v1/nakama/host/media/library/shared [GET]
//	@returns bool
...
//
//	@summary return the local files for the given SIMKL anime media id.
//	@desc This is used by the anime media entry pages to get all the data about the anime.
//	@route /api/v1/nakama/host/media/library/files/{id} [POST]
...
//	@desc This is used to share the local anime collection with Nakama clients.
//	@route /api/v1/nakama/host/media/library/files [POST]
//	@returns nakama.NakamaLocalFiles
...
func (h *Handler) buildNakamaLocalFiles(lfs []*anime.LocalFile) (*nakama.NakamaLocalFiles, error) {
	...
	customSourceMediaMap := make(map[int]*mediaapi.BaseAnime)
...
// route /api/v1/nakama/host/media/library/stream?path={base64_encoded_path}
...
func (h *Handler) HandleNakamaProxyStream(c echo.Context) error {
	...
	if streamType == "debrid" {
		...
		req.Header.Set("X-Seall-Nakama-Token", h.App.Settings.GetNakama().RemoteServerPassword)
	...
	case "file":
		...
		requestUrl = hostServerUrl + "/api/v1/nakama/host/media/library/stream?path=" + filepath
	case "torrent":
...
//
//	@summary creates a Seall Room and connects to it as host.
//	@desc This creates a room on the Seall Rooms relay server and connects as the host. Peers can then join using the returned URL.
