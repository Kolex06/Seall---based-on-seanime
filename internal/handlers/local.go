...
//	@param type - string - true - "Type of media (media/reading)"
//	@returns bool
...
// HandleLocalSyncData
//
//	@summary syncs local data with SIMKL.
...
// HandleLocalSyncMediaData
//
//	@summary syncs SIMKL data with local.
//	@route /api/v1/local/simkl [POST]
//	@returns bool
func (h *Handler) HandleLocalSyncMediaData(c echo.Context) error {
	err := h.App.LocalManager.SynchronizeMediaApi()
	if err != nil {
		return h.RespondWithError(c, err)
	}
	return h.RespondWithData(c, true)
...
//
//	@summary sets the flag to determine if there are local changes that need to be synced with SIMKL.
...
//
//	@summary gets the flag to determine if there are local changes that need to be synced with SIMKL.
...
// HandleLocalSyncSimulatedDataToMedia
//
//	@summary syncs the simulated data to SIMKL.
//	@route /api/v1/local/sync-simulated-to-simkl [POST]
//	@returns bool
func (h *Handler) HandleLocalSyncSimulatedDataToMedia(c echo.Context) error {
	err := h.App.LocalManager.SynchronizeSimulatedCollectionToMediaApi()
	if err != nil {
		return h.RespondWithError(c, err)
	}
	return h.RespondWithData(c, true)
