...
func (h *Handler) HandleGetOnlineStreamEpisodeList(c echo.Context) error {
	...
	if media.Status == nil || *media.Status == mediaapi.MediaStatusNotYetReleased {
	...
	if onlinestream.MediaTypeForBaseMedia(media) == "anime" {
...
func (h *Handler) HandleGetOnlineStreamEpisodeSource(c echo.Context) error {
	...
	sources, err := h.App.OnlinestreamRepository.GetEpisodeSources(c.Request().Context(), b.Provider, b.MediaId, b.EpisodeNumber, b.SeasonNumber, b.Dubbed, media.GetStartYearSafe())
	if err != nil {
		return h.RespondWithError(c, err)
	}
...
func (h *Handler) HandleOnlinestreamManualSearch(c echo.Context) error {
	...
	var media *mediaapi.BaseAnime
	if b.MediaId > 0 {
		media, _ = h.App.OnlinestreamRepository.GetMedia(c.Request().Context(), b.MediaId)
	}

	ret, err := h.App.OnlinestreamRepository.ManualSearch(b.Provider, b.Query, b.Dubbed, media)
	if err != nil {
		return h.RespondWithError(c, err)
	}
...
//
//	@summary manually maps a media entry to a provider media ID.
//	@desc This is used to manually map a media entry to a provider media ID.
...
func (h *Handler) HandleOnlinestreamManualMapping(c echo.Context) error {
	...
	if b.ProviderMediaId == "" {
		return h.RespondWithError(c, errors.New("provider media ID is required"))
	}

	err := h.App.OnlinestreamRepository.ManualMapping(b.Provider, b.MediaId, b.ProviderMediaId)
	if err != nil {
		return h.RespondWithError(c, err)
	}
...
//
//	@summary returns the mapping for a media entry.
//	@desc This is used to get the mapping for a media entry.
//	@desc An empty string is returned if there's no manual mapping. If there is, the provider media ID will be returned.
...
//
//	@summary removes the mapping for a media entry.
//	@desc This is used to remove the mapping for a media entry.
//	@desc The client should re-fetch the chapter container after this.
