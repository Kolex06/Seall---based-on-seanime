...
var (
	baseMangaCache    = result.NewCache[int, *mediaapi.BaseManga]()
	mangaDetailsCache = result.NewCache[int, *mediaapi.MangaDetailsById_Media]()
)

// HandleGetSimklMangaCollection
//
//	@summary returns the user's SIMKL manga collection.
//	@route /api/v1/reading/simkl/collection [GET]
//	@returns mediaapi.MangaCollection
func (h *Handler) HandleGetSimklMangaCollection(c echo.Context) error {
...
// HandleGetRawMangaCollection
//
//	@summary returns the user's SIMKL manga collection.
//	@route /api/v1/reading/simkl/collection/raw [GET,POST]
//	@returns mediaapi.MangaCollection
func (h *Handler) HandleGetRawMangaCollection(c echo.Context) error {
...
var mangaTagsCache *mediaapi.MediaTagMap
...
func (h *Handler) HandleGetRawMangaCollectionTags(c echo.Context) error {
	h.App.OnRefreshMediaCollectionFuncs.Set("HandleGetRawMangaCollectionTags", func() {
		mangaTagsCache = nil
	})
...
//	@route /api/v1/reading/collection [GET]
...
// HandleGetMangaEntry
//
//	@summary returns a manga entry for the given SIMKL manga id.
//	@desc This is used by the manga media entry pages to get all the data about the anime. It includes metadata and SIMKL list data.
//	@route /api/v1/reading/entry/{id} [GET]
//	@param id - int - true - "SIMKL manga media ID"
//	@returns manga.Entry
...
//
//	@summary returns more details about an SIMKL manga entry.
//	@desc This fetches more fields omitted from the base queries.
//	@route /api/v1/reading/entry/{id}/details [GET]
//	@param id - int - true - "SIMKL manga media ID"
//	@returns mediaapi.MangaDetailsById_Media
func (h *Handler) HandleGetMangaEntryDetails(c echo.Context) error {
	...
	details, err := h.App.MediaPlatformRef.Get().GetMangaDetails(c.Request().Context(), id)
	if err != nil {
		return h.RespondWithError(c, err)
	}
...
//	@route /api/v1/reading/latest-chapter-numbers [GET]
...
//	@route /api/v1/reading/refetch-chapter-containers [POST]
//	@returns bool
...
//	@route /api/v1/reading/entry/cache [DELETE]
//	@returns bool
...
//	@route /api/v1/reading/chapters [POST]
...
func (h *Handler) HandleGetMangaEntryChapters(c echo.Context) error {
	...
	if !found {
		var err error
		baseManga, err = h.App.MediaPlatformRef.Get().GetManga(c.Request().Context(), b.MediaId)
		if err != nil {
			return h.RespondWithError(c, err)
		}
...
//	@route /api/v1/reading/pages [POST]
...
//	@route /api/v1/reading/downloaded-chapters/{id} [GET]
//	@param id - int - true - "SIMKL manga media ID"
//	@returns []manga.ChapterContainer
...
var (
	simklListMangaCache = result.NewCache[string, *mediaapi.ListManga]()
)
...
//	@route /api/v1/reading/simkl/list [POST]
//	@returns mediaapi.ListManga
func (h *Handler) HandleListManga(c echo.Context) error {
	...
	if p.IsAdult != nil {
		isAdult = new(*p.IsAdult && h.App.Settings.GetMediaApi().EnableAdultContent)
	}

	cacheKey := mediaapi.ListMangaCacheKey(
		p.Page,
		p.Search,
		p.PerPage,
		p.Sort,
	...
	cached, ok := simklListMangaCache.Get(cacheKey)
	if ok {
		return h.RespondWithData(c, cached)
	}

	ret, err := mediaapi.ListMangaM(
	...
	if ret != nil {
		simklListMangaCache.SetT(cacheKey, ret, time.Minute*10)
	}
...
//	@route /api/v1/reading/update-progress [POST]
//	@returns bool
func (h *Handler) HandleUpdateMangaProgress(c echo.Context) error {
	...
	_, _ = h.App.RefreshMangaCollection() // Refresh the SIMKL collection
...
//	@route /api/v1/reading/search [POST]
...
//	@route /api/v1/reading/manual-mapping [POST]
//	@returns bool
...
//	@route /api/v1/reading/get-mapping [POST]
...
//	@desc The client should re-fetch the chapter container after this.
//	@route /api/v1/reading/remove-mapping [POST]
//	@returns bool
...
//	@route /api/v1/reading/local-page/{path} [GET]
//	@returns manga.PageContainer
