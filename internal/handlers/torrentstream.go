...
func (h *Handler) HandleGetTorrentstreamTorrentFilePreviews(c echo.Context) error {
	...
	animeMetadata, _ := h.App.MetadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, b.Media.ID)
	absoluteOffset := 0
...
func (h *Handler) HandleTorrentstreamStartStream(c echo.Context) error {
	...
	type body struct {
		...
		Torrent           *hibiketorrent.MediaTorrent      `json:"torrent,omitempty"` // Nil if autoSelect is true
