...
func (h *Handler) HandleTorrentClientDownload(c echo.Context) error {
	...
	var completeAnime *mediaapi.CompleteAnime
	var err error
	completeAnime, err = h.App.MediaPlatformRef.Get().GetAnimeWithRelations(c.Request().Context(), b.Media.ID)
	if err != nil {
	...
	go func() {
		...
		if b.Media != nil {
			...
			err = h.App.MediaPlatformRef.Get().AddMediaToCollection(c.Request().Context(), []int{b.Media.ID})
			if err != nil {
				h.App.Logger.Error().Err(err).Msg("simkl: Failed to add media to collection")
			}
...
func resolveAutoDownloaderItemMagnet(item *models.AutoDownloaderItem, torrentRepository *torrentrepo.Repository) (string, error) {
	...
	if len(item.TorrentData) > 0 {
		...
		if err := json.Unmarshal(item.TorrentData, &storedTorrent); err != nil {
			resolveErr = err
		} else if storedTorrent.MediaTorrent != nil {
			if fallbackHash == "" {
				fallbackHash = storedTorrent.MediaTorrent.InfoHash
			}

			if storedTorrent.MediaTorrent.Provider == "" && storedTorrent.ExtensionID != "" {
				storedTorrent.MediaTorrent.Provider = storedTorrent.ExtensionID
			}

			if torrentRepository != nil {
				magnet, err := torrentRepository.ResolveMagnetLink(storedTorrent.MediaTorrent)
				if err == nil && magnet != "" {
					return magnet, nil
				}
				resolveErr = err
