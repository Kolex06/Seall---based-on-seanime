...
//	@route /api/v1/auto-downloader/rule/media/{id} [GET]
//	@param id - int - true - "The SIMKL anime id of the rules"
//	@returns []anime.AutoDownloaderRule
...
func (h *Handler) HandleDeleteAutoDownloaderRule(c echo.Context) error {
	...
	if id == -1 {
		...
		for _, rule := range rules {
			...
			if media.Status != nil && *media.Status == mediaapi.MediaStatusFinished {
