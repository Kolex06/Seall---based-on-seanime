...
import (
	...
	"seall/internal/library/summary"
	"seall/internal/util"
...
// HandleGetMediaEntry
//
//	@summary return a media entry for the given SIMKL anime media id.
//	@desc This is used by the anime media entry pages to get all the data about the anime.
//	@desc This includes episodes and metadata (if any), SIMKL list data, download info...
//	@route /api/v1/library/media-entry/{id} [GET]
//	@param id - int - true - "SIMKL anime media ID"
//	@returns anime.Entry
func (h *Handler) HandleGetMediaEntry(c echo.Context) error {
...
func (h *Handler) HandleMediaEntryBulkAction(c echo.Context) error {
...
//	@returns bool
func (h *Handler) HandleOpenMediaEntryInExplorer(c echo.Context) error {
...
var (
	entriesSuggestionsCache = result.NewCache[string, []*mediaapi.BaseAnime]()
)

// HandleFetchMediaEntrySuggestions
//
...
func (h *Handler) HandleFetchMediaEntrySuggestions(c echo.Context) error {
	...
	res, err := mediaapi.ListAnimeM(
		h.App.MediaPlatformRef.Get().GetMediaApiClient(),
		new(1),
		&title,
		new(8),
		nil,
		[]*mediaapi.MediaStatus{new(mediaapi.MediaStatusFinished), new(mediaapi.MediaStatusReleasing), new(mediaapi.MediaStatusCancelled), new(mediaapi.MediaStatusHiatus)},
		nil,
		nil,
		nil,
		nil,
...
//	@returns []anime.LocalFile
func (h *Handler) HandleMediaEntryManualMatch(c echo.Context) error {
	...
	animeCollectionWithRelations, err := h.App.MediaPlatformRef.Get().GetMediaCollectionWithRelations(c.Request().Context())
	if err != nil {
		return h.RespondWithError(c, err)
	}
	...
	// Get the media
	media, err := h.App.MediaPlatformRef.Get().GetAnime(c.Request().Context(), b.MediaId)
	if err != nil {
		return h.RespondWithError(c, err)
	}
	...
	// Event
	event := new(anime.MediaEntryManualMatchBeforeSaveEvent)
	event.MediaId = b.MediaId
	event.Paths = b.Paths
	event.MatchedLocalFiles = selectedLfs
	err = hook.GlobalHookManager.OnMediaEntryManualMatchBeforeSave().Trigger(event)
	if err != nil {
		return h.RespondWithError(c, fmt.Errorf("OnMediaEntryManualMatchBeforeSave: %w", err))
	}
...
//	@desc It detects missing episodes by comparing the user's SIMKL collection 'next airing' data with the local files.
...
func (h *Handler) HandleGetMissingEpisodes(c echo.Context) error {
	h.App.AddOnRefreshMediaCollectionFunc("HandleGetMissingEpisodes", func() {
		anime.ClearMissingEpisodesCache()
	})
...
//	@desc It uses the SIMKL 'next airing episode' data to determine upcoming episodes.
//	@desc This route can be called multiple times, as it does not bypass the cache.
...
func (h *Handler) HandleGetUpcomingEpisodes(c echo.Context) error {
	h.App.AddOnRefreshMediaCollectionFunc("HandleGetUpcomingEpisodes", func() {
...
func (h *Handler) HandleGetMediaEntrySilenceStatus(c echo.Context) error {
	mId, err := strconv.Atoi(c.Param("id"))
	if err != nil {
...
//	@returns bool
func (h *Handler) HandleToggleMediaEntrySilenceStatus(c echo.Context) error {
...
// HandleUpdateMediaEntryProgress
//
//	@summary update the progress of the given anime media entry.
//	@desc This is used to update the progress of the given anime media entry on SIMKL.
//	@desc The response is not used in the frontend, the client should just refetch the entire media entry data.
...
//	@route /api/v1/library/media-entry/update-progress [POST]
//	@returns bool
func (h *Handler) HandleUpdateMediaEntryProgress(c echo.Context) error {
	...
	// Update the progress on SIMKL
	err := h.App.MediaPlatformRef.Get().UpdateEntryProgress(
		c.Request().Context(),
		b.MediaId,
		b.EpisodeNumber,
		&b.TotalEpisodes,
	...
	_, _ = h.App.RefreshAnimeCollection() // Refresh the SIMKL collection
...
// HandleUpdateMediaEntryRepeat
//
//	@summary update the repeat value of the given anime media entry.
//	@desc This is used to update the repeat value of the given anime media entry on SIMKL.
//	@desc The response is not used in the frontend, the client should just refetch the entire media entry data.
//	@route /api/v1/library/media-entry/update-repeat [POST]
//	@returns bool
func (h *Handler) HandleUpdateMediaEntryRepeat(c echo.Context) error {
	...
	err := h.App.MediaPlatformRef.Get().UpdateEntryRepeat(
		c.Request().Context(),
		b.MediaId,
		b.Repeat,
	)
