...
func (h *Handler) NewStatus(c echo.Context) *Status {
	...
	status := &Status{
		...
		SimklClientID:         h.App.Config.Simkl.ClientID,
		Updating:              false,
...
func (h *Handler) HandleGetLogFilenames(c echo.Context) error {
	...
	if len(filenames) > 0 {
		...
		for _, filename := range filenames {
			if strings.HasPrefix(strings.ToLower(filename), "seall-") {
...
//	@desc This returns the content of the most recent seall- log file after flushing logs.
...
func (h *Handler) HandleGetLatestLogContent(c echo.Context) error {
	...
	for _, entry := range dirEntries {
		...
		name := entry.Name()
		if filepath.Ext(name) != ".log" || !strings.HasPrefix(strings.ToLower(name), "seall-") {
			continue
		}
...
func (h *Handler) HandleGetMemoryProfile(c echo.Context) error {
	...
	if heap {
		filename = fmt.Sprintf("seall-heap-profile-%s.pprof", timestamp)
		profile = pprof.Lookup("heap")
	} else if allocs {
		filename = fmt.Sprintf("seall-allocs-profile-%s.pprof", timestamp)
...
func (h *Handler) HandleGetGoRoutineProfile(c echo.Context) error {
	...
	timestamp := time.Now().Format("2006-01-02_15-04-05")
	filename := fmt.Sprintf("seall-goroutine-profile-%s.pprof", timestamp)
...
func (h *Handler) HandleGetCPUProfile(c echo.Context) error {
	...
	timestamp := time.Now().Format("2006-01-02_15-04-05")
	filename := fmt.Sprintf("seall-cpu-profile-%s.pprof", timestamp)
