...
func PatchAnimeCollectionEntry(collection *AnimeCollection, mediaID int, patch AnimeCollectionEntryPatch) *AnimeCollection {
	if collection == nil {
		panic("simkl: anime collection is nil")
	}

	entry, currentList := findAnimeCollectionEntry(collection, mediaID)
	if entry == nil {
		panic(fmt.Sprintf("simkl: anime %d not found in collection; use EnsureAnimeCollectionEntry for missing media", mediaID))
	}
...
func EnsureAnimeCollectionEntry(collection *AnimeCollection, mediaID int, patch AnimeCollectionEntryPatch, client MediaApiClient) *AnimeCollection {
	if collection == nil {
		panic("simkl: anime collection is nil")
	}
	...
	if client == nil {
		panic(fmt.Sprintf("simkl: cannot add anime %d without a client", mediaID))
	}
	if patch.Status == nil {
		panic(fmt.Sprintf("simkl: cannot add anime %d without a status", mediaID))
	}
...
func PatchAnimeCollectionWithRelationsEntry(collection *AnimeCollectionWithRelations, mediaID int, patch AnimeCollectionEntryPatch) *AnimeCollectionWithRelations {
	if collection == nil {
		panic("simkl: anime collection with relations is nil")
	}

	entry, currentList := findAnimeCollectionWithRelationsEntry(collection, mediaID)
	if entry == nil {
		panic(fmt.Sprintf("simkl: anime %d not found in relation collection; use EnsureAnimeCollectionWithRelationsEntry for missing media", mediaID))
	}
...
func EnsureAnimeCollectionWithRelationsEntry(collection *AnimeCollectionWithRelations, mediaID int, patch AnimeCollectionEntryPatch, client MediaApiClient) *AnimeCollectionWithRelations {
	if collection == nil {
		panic("simkl: anime collection with relations is nil")
	}
