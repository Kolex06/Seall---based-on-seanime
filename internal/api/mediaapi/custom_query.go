...
func customQuery(body []byte, logger *zerolog.Logger, token ...string) (data interface{}, err error) {
	...
	defer func() {
		...
		if err != nil {
			logger.Error().Str("duration", formattedDur).Str("rlr", rlRemainingStr).Err(err).Msg("simkl: Failed Request (custom query)")
		} else {
			if timeSince > 600*time.Millisecond {
				logger.Warn().Str("rtt", formattedDur).Str("rlr", rlRemainingStr).Msg("simkl: Long Request")
			} else {
				logger.Trace().Str("rtt", formattedDur).Str("rlr", rlRemainingStr).Msg("simkl: Successful Request")
			}
		}
	}()

	defer util.HandlePanicInModuleThen("api/simkl/custom_query", func() {
		err = errors.New("panic in customQuery")
	})

	var req *http.Request
	req, err = http.NewRequest("POST", constants.MediaApiUrl, bytes.NewBuffer(body))
	if err != nil {
		return nil, err
	}
	...
	resp, rlRemainingStr, err = doSIMKLRequestWithRetries(
		...
		func(waitSeconds int) {
			notifySIMKLRateLimit(logger, waitSeconds)
		},
	)
	if err != nil {
		return nil, err
