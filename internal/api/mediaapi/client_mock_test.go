...
const recordCompleteAnimeIDsEnvName = "SEALL_TEST_RECORD_COMPLETE_ANIME_IDS"

func TestMaybeWriteJSONFixtureCreatesDirectories(t *testing.T) {
	t.Setenv(testutil.RecordMediaApiFixturesEnvName, "true")
...
func TestCustomQueryFixturePathIsStable(t *testing.T) {
	...
	assert.Contains(t, path1, filepath.Join("test", "testdata", "simkl-custom-query"))
}

func TestFixtureMangaCollectionUsesCommittedFixture(t *testing.T) {
	client := NewFixtureMediaApiClient()
...
func TestRecordCompleteAnimeByIDFixtures(t *testing.T) {
	if !testutil.ShouldRecordMediaApiFixtures() {
		t.Skip("SIMKL fixture recording disabled")
	}
	...
	if cfg.Provider.MediaApiJwt == "" {
		t.Skip("SIMKL fixture recording requires provider.simkl_jwt")
	}

	client := NewFixtureMediaApiClientWithToken(cfg.Provider.MediaApiJwt)
