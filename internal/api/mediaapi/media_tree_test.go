...
func TestBaseAnime_FetchMediaTree_BaseAnime(t *testing.T) {
	...
	lim := limiter.NewMediaApiLimiter()
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			mediaF, err := simklClient.CompleteAnimeByID(context.Background(), &tt.mediaId)
...
func TestBaseAnime_FetchMediaTree_BaseAnimeLive(t *testing.T) {
	...
	mediaF, err := simklClient.CompleteAnimeByID(context.Background(), &mediaID)
	if !assert.NoError(t, err) {
		return
	}
