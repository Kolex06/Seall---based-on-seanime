...
func (m *BaseAnime) FetchMediaTree(rel FetchMediaTreeRelation, simklClient MediaApiClient, rl *limiter.Limiter, tree *CompleteAnimeRelationTree, cache *CompleteAnimeCache) (err error) {
	if m == nil {
		return nil
	}

	defer util.HandlePanicInModuleWithError("simkl/BaseAnime.FetchMediaTree", &err)

	rl.Wait()
	res, err := simklClient.CompleteAnimeByID(context.Background(), &m.ID)
	if err != nil {
		return err
	}
	return res.GetMedia().FetchMediaTree(rel, simklClient, rl, tree, cache)
}
...
// It also takes a limiter.Limiter to limit the number of requests made to the SIMKL API.
func (m *CompleteAnime) FetchMediaTree(rel FetchMediaTreeRelation, simklClient MediaApiClient, rl *limiter.Limiter, tree *CompleteAnimeRelationTree, cache *CompleteAnimeCache) (err error) {
	if m == nil {
		return nil
	}

	defer util.HandlePanicInModuleWithError("simkl/CompleteAnime.FetchMediaTree", &err)
	...
	processEdges(edges, rel, simklClient, rl, tree, cache, doneCh)
...
func processEdges(edges []*CompleteAnime_Relations_Edges, rel FetchMediaTreeRelation, simklClient MediaApiClient, rl *limiter.Limiter, tree *CompleteAnimeRelationTree, cache *CompleteAnimeCache, doneCh chan struct{}) {
	...
	for i, item := range edges {
		go func(edge *CompleteAnime_Relations_Edges, _ int) {
			...
			processEdge(edge, rel, simklClient, rl, tree, cache)
		}(item, i)
	}
...
func processEdge(edge *CompleteAnime_Relations_Edges, rel FetchMediaTreeRelation, simklClient MediaApiClient, rl *limiter.Limiter, tree *CompleteAnimeRelationTree, cache *CompleteAnimeCache) {
	defer util.HandlePanicInModuleThen("simkl/processEdge", func() {})
	cacheV, ok := cache.Get(edge.GetNode().ID)
	edgeCompleteAnime := cacheV
	if !ok {
		rl.Wait()
		// Fetch the next node
		res, err := simklClient.CompleteAnimeByID(context.Background(), &edge.GetNode().ID)
		if err == nil {
	...
	err := edgeCompleteAnime.FetchMediaTree(edgeRel, simklClient, rl, tree, cache)
	if err != nil {
		return
	}
}
