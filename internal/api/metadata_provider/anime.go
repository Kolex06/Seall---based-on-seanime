...
func (aw *AnimeWrapperImpl) GetEpisodeMetadata(ep string) (ret metadata.EpisodeMetadata) {
	...
	// If TVDB image is not set, use Animap image, if that is not set, use the SIMKL banner image
	if ret.Image == "" {
...
func getDefaultOverview(baseAnime *mediaapi.BaseAnime, ep string, epNumber int) string {
	if ep == "" {
		return ""
	}
