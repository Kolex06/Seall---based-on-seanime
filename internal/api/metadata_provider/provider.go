...
type (
	...
	Provider interface {
		...
		GetAnimeMetadataWrapper(anime *mediaapi.BaseAnime, metadata *metadata.AnimeMetadata) AnimeMetadataWrapper
...
func NewProvider(options *NewProviderImplOptions) Provider {
	ret := &ProviderImpl{
		...
		simklClientRef:      options.SimklClientRef,
	}
...
func (p *ProviderImpl) fetchAnimeMetadata(platform metadata.Platform, mId int) (*metadata.AnimeMetadata, error) {
	...
	if customSource, localId, isCustom, hasExtension := p.customSourceManager.GetProviderFromId(mId); isCustom {
		...
		ret = m
	} else if platform == metadata.MediaPlatform {
		m, err := p.fetchSimklEpisodeMetadata(context.Background(), mId)
		if err == nil && m != nil {
			ret = m
		} else if p.useFallbackProvider.Load() {
			return p.AnizipFallback(platform, mId)
		} else {
			p.logger.Debug().Err(err).Int("mediaId", mId).Msg("simkl: Falling back to legacy episode metadata")
			m, err := p.fetchAnimapMetadata(platform, mId, ret)
			if err != nil {
				return nil, err
			}
			ret = m
...
func (p *ProviderImpl) GetAnimeMetadataWrapper(media *mediaapi.BaseAnime, m *metadata.AnimeMetadata) AnimeMetadataWrapper {
	aw := &AnimeWrapperImpl{
	...
	if aw.db != nil {
		...
		if err == nil {
			aw.parentEntry, err = p.GetAnimeMetadata(metadata.MediaPlatform, parent.ParentId)
			if err == nil {
...
func (p *ProviderImpl) AnizipFallback(platform metadata.Platform, mId int) (ret *metadata.AnimeMetadata, err error) {
	...
	ret.Mappings.MediaId = anizipMedia.Mappings.MediaID
