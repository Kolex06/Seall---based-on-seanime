...
var (
	ErrNoMediaFound         = errors.New("media not found, try manual matching")
...
func (r *Repository) getEpisodeContainer(provider string, media *mediaapi.BaseAnime, from int, to int, dubbed bool, year int) (*episodeContainer, error) {
	...
	if found, _ := r.fileCacher.Get(fcEpisodeListBucket, providerEpisodeListKey, &providerEpisodeList); !found {
		...
		if err != nil {
			...
			return nil, err // ErrNoMediaFound or ErrNoEpisodes
		}
	...
	if len(ec.ProviderEpisodeList) == 0 {
		r.logger.Error().Msg("onlinestream: No stream items found for this media")
		return nil, fmt.Errorf("no stream items found for this media")
	}
...
func (r *Repository) getProviderEpisodeServers(provider string, episodeDetails *hibikeonlinestream.EpisodeDetails) ([]*hibikeonlinestream.EpisodeServer, error) {
	...
	for _, episodeServer := range providerExtension.GetProvider().GetSettings().EpisodeServers {
		res, err := providerExtension.GetProvider().FindMediaItemServer(episodeDetails, episodeServer)
		if err == nil {
...
// getProviderEpisodeList gets all the hibikeonlinestream.EpisodeDetails from the provider based on the media titles.
// It returns ErrNoMediaFound if the media is not found or ErrNoEpisodes if no playable items are found.
func (r *Repository) getProviderEpisodeList(provider string, media *mediaapi.BaseAnime, dubbed bool, year int) ([]*hibikeonlinestream.EpisodeDetails, error) {
	...
	if found {
		r.logger.Debug().Str("providerMediaId", mapping.ProviderMediaID).Msg("onlinestream: Using manual mapping")
		matchId = mapping.ProviderMediaID
	}
	...
	if matchId == "" {
		...
		queryMedia := queryMediaFromBaseMedia(media, mediaType)
	...
	// Fetch episodes.
	ret, err := providerExtension.GetProvider().FindMediaItems(matchId)
	if err != nil {
