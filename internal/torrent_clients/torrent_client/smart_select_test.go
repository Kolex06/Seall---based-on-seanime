...
func newClientRepositoryWithProvider(providerID string) *torrent_client.Repository {
	...
	bank.Set(providerID, extension.NewMediaTorrentProviderExtension(&extension.Extension{
		ID:          providerID,
		Name:        "Test Provider",
		Version:     "1.0.0",
		ManifestURI: "builtin",
...
func testTorrent(providerID string) *hibiketorrent.MediaTorrent {
	return &hibiketorrent.MediaTorrent{
		Provider: providerID,
		InfoHash: "hash",
	}
}

func mediaWithEpisodes(count int) *mediaapi.CompleteAnime {
	return &mediaapi.CompleteAnime{Episodes: &count}
}

type stubAnimeProvider struct{}

func (stubAnimeProvider) Search(hibiketorrent.AnimeSearchOptions) ([]*hibiketorrent.MediaTorrent, error) {
	return nil, nil
}

func (stubAnimeProvider) SmartSearch(hibiketorrent.AnimeSmartSearchOptions) ([]*hibiketorrent.MediaTorrent, error) {
	return nil, nil
}

func (stubAnimeProvider) GetTorrentInfoHash(*hibiketorrent.MediaTorrent) (string, error) {
	return "hash", nil
}

func (stubAnimeProvider) GetTorrentMagnetLink(*hibiketorrent.MediaTorrent) (string, error) {
	return "magnet:?xt=urn:btih:hash", nil
}

func (stubAnimeProvider) GetLatest() ([]*hibiketorrent.MediaTorrent, error) {
	return nil, nil
}
...
func TestSmartSelectValidation(t *testing.T) {
	...
	t.Run("nil repository", func(t *testing.T) {
		...
		require.EqualError(t, err, "media or simkl client wrapper is nil")
	})
