...
func migrateTables(db *gorm.DB) error {
	...
	if db.Migrator().HasColumn(&models.OnlinestreamMapping{}, "anime_id") &&
		db.Migrator().HasColumn(&models.OnlinestreamMapping{}, "provider_media_id") {
		_ = db.Exec("UPDATE onlinestream_mappings SET provider_media_id = anime_id WHERE (provider_media_id IS NULL OR provider_media_id = '') AND anime_id IS NOT NULL AND anime_id != ''").Error
	}
