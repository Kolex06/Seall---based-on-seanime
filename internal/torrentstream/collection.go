...
func (r *Repository) HydrateStreamCollection(opts *HydrateStreamCollectionOptions) {
	...
	var currentlyWatching *mediaapi.AnimeCollection_MediaListCollection_Lists
	//var pausedList *mediaapi.AnimeCollection_MediaListCollection_Lists
	//var planningList *mediaapi.AnimeCollection_MediaListCollection_Lists
	for _, list := range lists {
		if list.Status == nil {
			continue
		}
		if *list.Status == mediaapi.MediaListStatusCurrent || *list.Status == mediaapi.MediaListStatusRepeating {
			if currentlyWatching == nil {
				currentlyWatching = &mediaapi.AnimeCollection_MediaListCollection_Lists{
		...
		//if *list.Status == mediaapi.MediaListStatusPaused {
		//	if pausedList == nil {
		//		pausedList = &mediaapi.AnimeCollection_MediaListCollection_Lists{
		//			Status:       new(mediaapi.MediaListStatusPaused),
		//			Name:         new("PAUSED"),
		//			IsCustomList: new(false),
		//			Entries:      make([]*mediaapi.AnimeCollection_MediaListCollection_Lists_Entries, 0),
		//		}
		...
		//	continue
		//}
		//if *list.Status == mediaapi.MediaListStatusPlanning {
		//	if planningList == nil {
		//		planningList = &mediaapi.AnimeCollection_MediaListCollection_Lists{
	...
	ret := &StreamCollection{
		...
		Anime:                make([]*mediaapi.BaseAnime, 0),
		ListData:             make(map[int]*anime.EntryListData),
	}

	visitedMediaIds := make(map[int]struct{})

	animeAdded := make(map[int]*mediaapi.AnimeListEntry)
	...
	for _, entry := range currentlyWatching.Entries {
		go func(entry *mediaapi.AnimeListEntry) {
			defer wg.Done()
			...
			if entry.GetMedia().GetStatus() == nil || *entry.GetMedia().GetStatus() == mediaapi.MediaStatusNotYetReleased {
				return
			}

			// Get the media info
			animeMetadata, err := opts.MetadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, mediaId)
			if err != nil {
			...
			if anime.FindDiscrepancy(entry.GetMedia(), animeMetadata) == anime.DiscrepancySIMKLCountsEpisodeZero {
				progressOffset = 1
	...
	for _, list := range opts.LibraryCollection.Lists {
		if list.Status == mediaapi.MediaListStatusCurrent {
			for _, entry := range list.Entries {
	...
	for _, entry := range currentlyWatching.Entries {
		...
		if *entry.GetMedia().GetStatus() == mediaapi.MediaStatusNotYetReleased {
			continue
		}
	...
	for _, a := range animeAdded {
		...
		ret.ListData[a.GetMedia().GetID()] = &anime.EntryListData{
			...
			StartedAt:   mediaapi.FuzzyDateToString(a.StartedAt),
			CompletedAt: mediaapi.FuzzyDateToString(a.CompletedAt),
		}
	}
