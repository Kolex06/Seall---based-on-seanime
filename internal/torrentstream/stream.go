...
type StartStreamOptions struct {
	...
	Torrent           *hibiketorrent.MediaTorrent      `json:"torrent"`       // Selected torrent (Manual selection)
...
func (r *Repository) sendStreamToExternalPlayer(opts *StartStreamOptions, completeAnime *mediaapi.CompleteAnime, aniDbEpisode string) {
...
func (r *Repository) GetMediaInfo(ctx context.Context, mediaId int) (media *mediaapi.CompleteAnime, animeMetadata *metadata.AnimeMetadata, err error) {
	// Get the media
	var found bool
	...
	// Get the media
	animeMetadata, err = r.metadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, mediaId)
	if err != nil {
