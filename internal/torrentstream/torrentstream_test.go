...
func newTorrentstreamTestRepositoryWithMetadataProvider(t *testing.T, metadataProvider metadata_provider.Provider) (*Repository, *testutil.TestEnv, *recordingWSEventManager) {
	...
	repo := NewRepository(&NewRepositoryOptions{
		Logger:              env.Logger(),
		BaseAnimeCache:      mediaapi.NewBaseAnimeCache(),
...
func TestHydrateStreamCollectionMergesSIMKLAndLibraryState(t *testing.T) {
	...
	unreleasedMedia := testmocks.NewBaseAnimeBuilder(3, "Unreleased Show").WithEpisodes(12).WithStatus(mediaapi.MediaStatusNotYetReleased).Build()
			...
			MediaListCollection: &mediaapi.AnimeCollection_MediaListCollection{
				Lists: []*mediaapi.AnimeCollection_MediaListCollection_Lists{
					newAnimeCollectionList(mediaapi.MediaListStatusCurrent,
						newAnimeCollectionEntry(mediaInLibrary, 1, mediaapi.MediaListStatusCurrent),
						newAnimeCollectionEntry(mediaAlreadyQueued, 0, mediaapi.MediaListStatusCurrent),
						newAnimeCollectionEntry(unreleasedMedia, 0, mediaapi.MediaListStatusCurrent),
					),
					newAnimeCollectionList(mediaapi.MediaListStatusRepeating,
						newAnimeCollectionEntry(mediaInLibrary, 1, mediaapi.MediaListStatusRepeating),
					),
				},
			},
		},
	...
	repo.HydrateStreamCollection(&HydrateStreamCollectionOptions{
		AnimeCollection: &mediaapi.AnimeCollection{
			MediaListCollection: &mediaapi.AnimeCollection_MediaListCollection{
				Lists: []*mediaapi.AnimeCollection_MediaListCollection_Lists{
					newAnimeCollectionList(mediaapi.MediaListStatusCurrent,
						newAnimeCollectionEntry(media, 1, mediaapi.MediaListStatusCurrent),
					),
				},
			},
		},
...
func TestAddBatchHistoryPersistsAndInvalidatesQueries(t *testing.T) {
	...
	torrent := &hibiketorrent.MediaTorrent{
		Provider: "provider",
		Name:     "[Seall] Example Show - 01-12 (1080p).mkv",
		InfoHash: "hash-1",
		IsBatch:  true,
	}
	files := &hibiketorrent.BatchEpisodeFiles{
		...
		Files: []*hibiketorrent.MediaTorrentFile{{
			Index: 0,
			Path:  "/downloads/[Seall] Example Show - 01.mkv",
			Name:  "[Seall] Example Show - 01.mkv",
		}},
	}
	...
	require.Equal(t, "Seall", history.Metadata.ReleaseGroup)
...
func TestAddBatchHistoryUpdatesExistingRecord(t *testing.T) {
	...
	repo.AddBatchHistory(101, &hibiketorrent.MediaTorrent{
		Name:     "[Seall] Example Show - 01-12 (1080p).mkv",
		InfoHash: "old-hash",
	}, nil)
	require.Eventually(t, func() bool {
	...
	repo.AddBatchHistory(101, &hibiketorrent.MediaTorrent{
		Name:     "[Seall] Example Show - 05 (1080p).mkv",
		InfoHash: "new-hash",
	}, updatedFiles)
...
func TestDeleteBatchHistoryRemovesRecordAndInvalidatesQueries(t *testing.T) {
	...
	repo.AddBatchHistory(102, &hibiketorrent.MediaTorrent{
		Name:     "[Seall] Example Show - 01-12 (1080p).mkv",
...
func newAnimeCollectionEntry(media *mediaapi.BaseAnime, progress int, status mediaapi.MediaListStatus) *mediaapi.AnimeCollection_MediaListCollection_Lists_Entries {
