...
func (a *AppContextImpl) BindScannerToContextObj(vm *goja.Runtime, obj *goja.Object, logger *zerolog.Logger, _ *extension.Extension, scheduler *gojautil.Scheduler) {
	...
	_ = scannerObj.Set("scan", func(opts scanOptions) goja.Value {
		...
		platformRef, ok := a.mediaPlatformRef.Get()
		if !ok {
			goja_bindings.PanicThrowErrorString(vm, "simkl platform not set")
		}
		...
		refreshAnimeCollection, _ := a.onRefreshMediaCollection.Get()
		...
		go func() {
			...
			animeCollection, err := platformRef.Get().GetMediaCollection(context.Background(), false)
			if err != nil {
				animeCollection = nil
			}
