...
func (a *AppContextImpl) BindDatabase(vm *goja.Runtime, logger *zerolog.Logger, ext *extension.Extension) {
	...
	_ = simklObj.Set("getToken", db.getSimklToken)
	_ = simklObj.Set("getUsername", db.getMediaApiUsername)
	_ = dbObj.Set("simkl", simklObj)
...
func (d *Database) saveLocalFiles(filesToSave []*anime.LocalFile) error {
	...
	if ok {
		ws.SendEvent(events.InvalidateQueries, []string{events.GetLocalFilesEndpoint, events.GetMediaEntryEndpoint, events.GetLibraryCollectionEndpoint, events.GetMissingEpisodesEndpoint})
	}
...
func (d *Database) getSimklToken() (string, error) {
	if d.ext.Plugin == nil || len(d.ext.Plugin.Permissions.Scopes) == 0 {
		return "", errors.New("permission denied")
	}
	if !util.Contains(d.ext.Plugin.Permissions.Scopes, extension.PluginPermissionSimklToken) {
		return "", errors.New("permission denied")
	}
	db, ok := d.ctx.database.Get()
	if !ok {
	...
	return db.GetSimklToken(), nil
}

func (d *Database) getMediaApiUsername() (string, error) {
	db, ok := d.ctx.database.Get()
	if !ok {
		return "", errors.New("database not initialized")
	}
...
func (d *Database) updateAutoDownloaderRule(id uint, rule *anime.AutoDownloaderRule) error {
	...
	ws, ok := d.ctx.wsEventManager.Get()
	if ok {
		ws.SendEvent(events.InvalidateQueries, []string{events.GetAutoDownloaderRulesEndpoint, events.GetAutoDownloaderItemsEndpoint, events.GetMediaEntryEndpoint})
	}
...
func (d *Database) insertAutoDownloaderRule(rule *anime.AutoDownloaderRule) error {
	...
	ws, ok := d.ctx.wsEventManager.Get()
	if ok {
		ws.SendEvent(events.InvalidateQueries, []string{events.GetAutoDownloaderRulesEndpoint, events.GetAutoDownloaderRulesByAnimeEndpoint, events.GetAutoDownloaderRuleEndpoint, events.GetAutoDownloaderItemsEndpoint, events.GetMediaEntryEndpoint})
	}
...
func (d *Database) setSilenced(mediaId int, silenced bool) error {
	...
	ws, ok := d.ctx.wsEventManager.Get()
	if ok {
		ws.SendEvent(events.InvalidateQueries, []string{events.GetMediaEntrySilenceStatusEndpoint})
	}
...
func (d *Database) insertMediaFiller(provider string, mediaId int, slug string, fillerEpisodes []string) error {
	...
	ws, ok := d.ctx.wsEventManager.Get()
	if ok {
		ws.SendEvent(events.InvalidateQueries, []string{events.GetMediaEntryEndpoint})
	}
