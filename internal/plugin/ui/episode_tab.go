...
func (m *EpisodeTabManager) listenRender() {
	listener := m.ctx.RegisterEventListener(ClientMediaEntryEpisodeTabsRenderEvent)
	listener.SetCallback(func(event *ClientPluginEvent) {
		payload := ClientMediaEntryEpisodeTabsRenderEventPayload{}
		if !event.ParsePayloadAs(ClientMediaEntryEpisodeTabsRenderEvent, &payload) {
			return
		}
...
func (m *EpisodeTabManager) listenOpen() {
	listener := m.ctx.RegisterEventListener(ClientMediaEntryEpisodeTabOpenEvent)
	listener.SetCallback(func(event *ClientPluginEvent) {
		payload := ClientMediaEntryEpisodeTabOpenEventPayload{}
		if !event.ParsePayloadAs(ClientMediaEntryEpisodeTabOpenEvent, &payload) {
			return
		}
		...
		m.ctx.SendEventToClient(ServerMediaEntryEpisodeTabEpisodeCollectionEvent, ServerMediaEntryEpisodeTabEpisodeCollectionEventPayload{
			EpisodeCollection: episodeCollection,
		})
	})
}
...
func (m *EpisodeTabManager) listenSelection() {
	listener := m.ctx.RegisterEventListener(ClientMediaEntryEpisodeTabSelectEpisodeEvent)
	listener.SetCallback(func(event *ClientPluginEvent) {
		payload := ClientMediaEntryEpisodeTabSelectEpisodeEventPayload{}
		if !event.ParsePayloadAs(ClientMediaEntryEpisodeTabSelectEpisodeEvent, &payload) {
			return
		}
...
func (m *EpisodeTabManager) listenStateChanges() {
	listener := m.ctx.RegisterEventListener(ClientMediaEntryEpisodeTabStateChangedEvent)
	listener.SetCallback(func(event *ClientPluginEvent) {
		payload := ClientMediaEntryEpisodeTabStateChangedEventPayload{}
		if !event.ParsePayloadAs(ClientMediaEntryEpisodeTabStateChangedEvent, &payload) {
			return
		}
...
func (m *EpisodeTabManager) renderSelectTabs(t []*EpisodeTab) {
	...
	m.ctx.SendEventToClient(ServerMediaEntryEpisodeTabsUpdatedEvent, ServerMediaEntryEpisodeTabsUpdatedEventPayload{
		Tabs: tabs,
	})
}
...
func (m *EpisodeTabManager) getEpisodeCollection(mediaID int) (*anime.EpisodeCollection, error) {
	mediaPlatformRef, ok := m.ctx.ui.appContext.MediaPlatformRef().Get()
	if !ok {
		return nil, fmt.Errorf("simkl platform not found")
	}
	...
	media, err := mediaPlatformRef.Get().GetAnime(context.Background(), mediaID)
	if err != nil {
		return nil, err
	}
