...
func (c *WebviewChannel) sendStateToWebview(key string, value interface{}) {
	...
	// Security: Iframe won't receive simkl token
	if str, ok := value.(string); ok {
		strings.ReplaceAll(str, c.webview.webviewManager.ctx.simklToken, "[TOKEN]")
	} else {
		encoded, err := json.Marshal(value)
		if err == nil {
			if strings.Contains(string(encoded), c.webview.webviewManager.ctx.simklToken) {
				return
			}
		}
	}
