...
const asyncMediaTorrentProviderPayload = `
class Provider {
  async search() {
...
func newTestGojaMediaTorrentProvider(t testing.TB) *GojaMediaTorrentProvider {
	t.Helper()
	...
	ext := &extension.Extension{
		...
		Payload:  asyncMediaTorrentProviderPayload,
	}

	_, provider, err := NewGojaMediaTorrentProvider(ext, ext.Language, logger, runtimeManager, wsEventManager)
	require.NoError(t, err)

	return provider
}

func TestGojaMediaTorrentProviderCallClassMethod(t *testing.T) {
	provider := newTestGojaMediaTorrentProvider(t)
...
func TestGojaMediaTorrentProviderAsyncMethods(t *testing.T) {
	provider := newTestGojaMediaTorrentProvider(t)
	...
	infoHash, err := provider.GetTorrentInfoHash(&hibiketorrent.MediaTorrent{InfoHash: "async-search-hash"})
	require.NoError(t, err)
