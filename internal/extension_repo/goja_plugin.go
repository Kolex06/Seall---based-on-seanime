...
func (p *GojaPlugin) ListMediaEntryEpisodeTabs() []*plugin_ui.EpisodeTabItem {
	if p.ui == nil {
		return nil
	}
	return p.ui.ListMediaEntryEpisodeTabs()
}
...
func (p *GojaPlugin) BindPluginAPIs(vm *goja.Runtime, logger *zerolog.Logger) {
	...
	if p.ext.Plugin != nil {
		for _, permission := range p.ext.Plugin.Permissions.Scopes {
			...
			case extension.PluginPermissionSimkl: // Simkl
				plugin.GlobalAppContext.BindSimkl(vm, logger, p.ext)
