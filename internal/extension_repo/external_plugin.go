...
type (
	...
	pluginEpisodeTabProvider interface {
		ListMediaEntryEpisodeTabs() []*plugin_ui.EpisodeTabItem
	}
)
...
func (r *Repository) ListMediaEntryEpisodeTabExtensions() (ret []*PluginEpisodeTabExtensionItem) {
	r.gojaExtensions.Range(func(_ string, gojaExt GojaExtension) bool {
		...
		for _, tab := range provider.ListMediaEntryEpisodeTabs() {
			if tab == nil {
				continue
			}
