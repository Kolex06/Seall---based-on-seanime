...
type (
	...
	MediaTorrentProviderExtensionItem struct {
		ID       string                              `json:"id"`
		Name     string                              `json:"name"`
		Lang     string                              `json:"lang"` // ISO 639-1 language code
...
func (r *Repository) ListMediaTorrentProviderExtensions() []*MediaTorrentProviderExtensionItem {
	ret := make([]*MediaTorrentProviderExtensionItem, 0)

	extension.RangeExtensions(r.extensionBankRef.Get(), func(key string, ext extension.MediaTorrentProviderExtension) bool {
		settings := ext.GetProvider().GetSettings()
		ret = append(ret, &MediaTorrentProviderExtensionItem{
			ID:       ext.GetID(),
			Name:     ext.GetName(),
			Lang:     extension.GetExtensionLang(ext.GetLang()),
			Settings: settings,
...
func (r *Repository) GetMediaTorrentProviderExtensionByID(id string) (extension.MediaTorrentProviderExtension, bool) {
