...
import (
	...
	plugin_ui "seall/internal/plugin/ui"
...
func InitTestPlugin(t testing.TB, opts TestPluginOptions) (*GojaPlugin, *zerolog.Logger, *goja_runtime.Manager, *media_platform.MediaPlatform, events.WSEventManagerInterface, error) {
	env := testutil.NewTestEnv(t)
	if opts.SetupHooks {
	...
	mediaClientRef := util.NewRef[mediaapi.MediaApiClient](mediaapi.NewFixtureMediaApiClient())
	extensionBankRef := util.NewRef(extension.NewUnifiedBank())
	mediaPlatform := media_platform.NewMediaPlatform(mediaClientRef, extensionBankRef, logger, database).(*media_platform.MediaPlatform)
	...
	return p, logger, manager, mediaPlatform, wsEventManager, err
}
...
func TestGojaPluginAnime(t *testing.T) {
	...
	function init() {
		...
		$ui.register(async (ctx) => {
			try {
				...
				ctx.anime.getAnimeMetadata("simkl", 21).then((metadata) => {
	...
	opts.Permissions = extension.PluginPermissions{
		Scopes: []extension.PluginPermissionScope{
			extension.PluginPermissionSimkl,
...
func TestGojaPluginUIAndHooks(t *testing.T) {
	...
	_, _, manager, mediaPlatform, _, err := InitTestPlugin(t, opts)
	require.NoError(t, err)

	go func() {
		time.Sleep(time.Second)
		_, err := mediaPlatform.GetAnime(t.Context(), 178022)
		if err != nil {
			t.Errorf("GetAnime returned error: %v", err)
		}

		// _, err = mediaPlatform.GetAnime(177709)
		// if err != nil {
		// 	t.Errorf("GetAnime returned error: %v", err)
		// }
	}()
...
func TestGojaPluginStore(t *testing.T) {
	...
	plugin, _, manager, mediaPlatform, _, err := InitTestPlugin(t, opts)
	require.NoError(t, err)

	m, err := mediaPlatform.GetAnime(t.Context(), 178022)
	if err != nil {
	...
	m, err = mediaPlatform.GetAnime(t.Context(), 177709)
	if err != nil {
		t.Fatalf("GetAnime returned error: %v", err)
	}
...
func TestGojaPluginSharedModulesRejectMissingReturnValue(t *testing.T) {
	...
	_, _, _, mediaPlatform, _, err := InitTestPlugin(t, opts)
	require.NoError(t, err)
...
func TestGojaPluginContextDebugDevelopmentOnly(t *testing.T) {
	...
	opts := DefaultTestPluginOptions()
	opts.Payload = payload
	plugin, _, _, mediaPlatform, wsEventManager, err := InitTestPlugin(t, opts)
	require.NoError(t, err)
	_, err = mediaPlatform.GetAnime(t.Context(), 178022)
	require.NoError(t, err)
	...
	devPlugin, _, _, mediaPlatform, wsEventManager, err := InitTestPlugin(t, opts)
	require.NoError(t, err)
	_, err = mediaPlatform.GetAnime(t.Context(), 178022)
	require.NoError(t, err)
...
func TestGojaPluginJsonFieldNames(t *testing.T) {
	...
	err = mediaPlatform.UpdateEntryProgress(t.Context(), 178022, 1, new(1))
	if err != nil {
		t.Fatalf("GetAnime returned error: %v", err)
	}
...
func TestGojaPluginSimklCustomQuery(t *testing.T) {
	payload := `
	function init() {
		$ui.register((ctx) => {
		const token = $database.mediaapi.getToken()
			try {
				const res = $mediaapi.customQuery({ query:` + "`" + `
					query GetOnePiece {
						Media(id: 21) {
							title {
								romaji
	...
	opts.Permissions = extension.PluginPermissions{
		Scopes: []extension.PluginPermissionScope{
			extension.PluginPermissionSimkl,
			extension.PluginPermissionSimklToken,
			extension.PluginPermissionDatabase,
		},
	}
...
func TestGojaPluginListMedia(t *testing.T) {
	payload := `
	function init() {
		...
		try {
			const res = $mediaapi.listRecentAnime(1, 15, undefined, undefined, undefined)
			console.log("res", res)
		} catch (e) {
			console.error("Error fetching anime list", e)
		}
...
func TestGojaPluginStorage2(t *testing.T) {
	...
	p, _, _, mediaPlatform, _, err := InitTestPlugin(t, opts)
	require.NoError(t, err)
	...
	mediaPlatform.GetAnime(t.Context(), 178022)
...
func getPlaybackManager(t *testing.T) (*playbackmanager.PlaybackManager, *mediaapi.AnimeCollection, error) {
	env := testutil.NewTestEnv(t)
	...
	mediaClientRef := util.NewRef(simklClient)
	mediaPlatform := media_platform.NewMediaPlatform(mediaClientRef, util.NewRef(extension.NewUnifiedBank()), logger, database)
