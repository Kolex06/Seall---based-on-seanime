	...
	case extension.TypeMediaTorrentProvider:
		...
		case extension.LanguageGo:
			...
			if animeProvider, ok := provider.(hibiketorrent.AnimeProvider); ok {
				r.loadBuiltInMediaTorrentProviderExtension(ext, animeProvider)
			}
		}
...
func (r *Repository) loadBuiltInMediaTorrentProviderExtension(ext extension.Extension, provider hibiketorrent.AnimeProvider) {
	r.extensionBankRef.Get().Set(ext.ID, extension.NewMediaTorrentProviderExtension(&ext, provider))
