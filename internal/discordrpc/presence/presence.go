...
import (
	...
	discordrpc_client "seall/internal/discordrpc/client"
	"seall/internal/hook"
	"seall/internal/util"
	"sync"
	"time"
...
func (p *Presence) SetSettings(settings *models.DiscordSettings) {
	...
	settings.RichPresenceShowSIMKLMediaButton = false // Devnote: Not used anymore, disable
	p.settings = settings
...
var (
	defaultActivity = discordrpc_client.Activity{
		...
		Assets: &discordrpc_client.Assets{
			LargeImage: "",
			LargeText:  "",
			SmallImage: constants.ProjectRawMainUrl + "/seall-denshi/assets/icons/256x256.png",
			SmallText:  "Seall v" + constants.Version,
			SmallURL:   constants.ProjectRepositoryUrl,
		},
		...
		Buttons: []*discordrpc_client.Button{
			{
				Label: "Seall",
				Url:   constants.ProjectRepositoryUrl,
			},
		},
		Instance:          true,
		Type:              3,
...
func isSeallButtonPresent(activity *discordrpc_client.Activity) bool {
	if activity == nil || activity.Buttons == nil {
		return false
	}
	for _, button := range activity.Buttons {
		if button.Label == "Seall" && button.Url == constants.ProjectRepositoryUrl {
			return true
		}
	}
	return false
...
func simklSearchURL(title string) string {
	if title == "" {
		return "https://simkl.com"
	}
	return "https://simkl.com/search/?q=" + url.QueryEscape(title)
}
...
func (p *Presence) SetAnimeActivity(a *AnimeActivity) {
	...
	activity.DetailsURL = simklSearchURL(a.Title)
	activity.State = state
	activity.Assets.LargeImage = a.Image
	activity.Assets.LargeText = a.Title
	activity.Assets.LargeURL = simklSearchURL(a.Title)
	...
	if p.settings.RichPresenceShowSIMKLProfileButton {
		activity.Buttons = append(activity.Buttons, &discordrpc_client.Button{
			Label: "View Profile",
			Url:   fmt.Sprintf("https://simkl.com/user/%s", p.username),
		})
	}

	if !(p.settings.RichPresenceHideSeallRepositoryButton || len(activity.Buttons) > 1) {
		activity.Buttons = append(activity.Buttons, &discordrpc_client.Button{
			Label: "Seall",
			Url:   constants.ProjectRepositoryUrl,
		})
	...
	// Only allow changing small image and text if Seall button is present
	if isSeallButtonPresent(&activity) {
...
// SetMangaActivity sets the presence to reading media.
func (p *Presence) SetMangaActivity(a *MangaActivity) {
	...
	if p.settings.RichPresenceShowSIMKLProfileButton && p.username != "" {
		activity.Buttons = append(activity.Buttons, &discordrpc_client.Button{
			Label: "View Profile",
			Url:   fmt.Sprintf("https://simkl.com/user/%s", p.username),
		})
