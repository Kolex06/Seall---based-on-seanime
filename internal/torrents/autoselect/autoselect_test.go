...
func TestAutoSelect_Filter(t *testing.T) {
	...
	t1 := &hibiketorrent.MediaTorrent{Name: "[SubsPlease] One Piece - 1000 (1080p).mkv", Seeders: 100, Size: 1024 * 1024 * 1000}                             // 1GB
	t2 := &hibiketorrent.MediaTorrent{Name: "[Erai-raws] One Piece - 1000 [720p][Multiple Subtitle].mkv", Seeders: 50, Size: 1024 * 1024 * 500}              // 500MB
	t3 := &hibiketorrent.MediaTorrent{Name: "[EMBER] One Piece - 1000 [1080p] [Dual Audio] [HEVC].mkv", Seeders: 200, Size: 1024 * 1024 * 800}               // 800MB
	t4 := &hibiketorrent.MediaTorrent{Name: "[French-Fansub] One Piece - 1000 [480p] [French].mkv", Seeders: 10, Size: 1024 * 1024 * 200}                    // 200MB
	t5 := &hibiketorrent.MediaTorrent{Name: "[Cleo] One Piece - Batch [1080p] [Dual Audio].mkv", Seeders: 500, Size: 1024 * 1024 * 1024 * 50, IsBatch: true} // 50GB Batch
	t6 := &hibiketorrent.MediaTorrent{Name: "[Judas] One Piece - 1000 [1080p][HEVC x265 10bit].mkv", Seeders: 150, Size: 1024 * 1024 * 600}                  // 600MB
	t7 := &hibiketorrent.MediaTorrent{Name: "[HorribleSubs] One Piece - 1000 [720p].mkv", Seeders: 20, Size: 1024 * 1024 * 400}                              // 400MB
	t8 := &hibiketorrent.MediaTorrent{Name: "[SourceCheck] One Piece - 1000 [Web-DL].mkv", Seeders: 30, Size: 1024 * 1024 * 900}                             // 900MB

	torrents := []*hibiketorrent.MediaTorrent{t1, t2, t3, t4, t5, t6, t7, t8}
...
func TestAutoSelect_Sort(t *testing.T) {
	...
	t1 := &hibiketorrent.MediaTorrent{Name: "[A] Show - 01 [1080p].mkv", Seeders: 100, Provider: "catsound"}
	t2 := &hibiketorrent.MediaTorrent{Name: "[B] Show - 01 [720p].mkv", Seeders: 200, Provider: "catsound"}
	t3 := &hibiketorrent.MediaTorrent{Name: "[C] Show - 01 [1080p] [Dual-Audio].mkv", Seeders: 50, Provider: "tosho"}
	t4 := &hibiketorrent.MediaTorrent{Name: "[D] Show - 01 [1080p] [HEVC].mkv", Seeders: 80, Provider: "catsound"}

	torrents := []*hibiketorrent.MediaTorrent{t1, t2, t3, t4}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			testTorrents := make([]*hibiketorrent.MediaTorrent, len(torrents))
...
func TestAutoSelect_Sort_OrderedPreferencesBeatSoftBonuses(t *testing.T) {
	...
	preferred := &hibiketorrent.MediaTorrent{
		Name:     "[ToonsHub] Show - 01 1080p CR WEB-DL AAC2.0 H.264 (Multi-Subs).mkv",
		Seeders:  50,
		Provider: "catnoise",
	}
	lowerPriorityButBonusHeavy := &hibiketorrent.MediaTorrent{
	...
	torrents := []*hibiketorrent.MediaTorrent{lowerPriorityButBonusHeavy, preferred}
	profile := &anime.AutoSelectProfile{
...
func TestAutoSelect_Sort_StrongerPrimaryMatchCanBeatHigherReleaseGroup(t *testing.T) {
	...
	higherReleaseGroupButLowerQuality := &hibiketorrent.MediaTorrent{
		Name:     "[ToonsHub] Show - 01 720p CR WEB-DL AAC2.0 H.264.mkv",
		Seeders:  80,
		Provider: "catnoise",
	}
	lowerReleaseGroupButHigherResolution := &hibiketorrent.MediaTorrent{
		Name:     "Show - 01 1080p DSNP WEB-DL AAC2.0 H.264-VARYG.mkv",
		Seeders:  40,
		Provider: "catnoise",
	}

	torrents := []*hibiketorrent.MediaTorrent{higherReleaseGroupButLowerQuality, lowerReleaseGroupButHigherResolution}
	profile := &anime.AutoSelectProfile{
		ReleaseGroups:    []string{"ToonsHub", "VARYG"},
		Resolutions:      []string{"1080p"},
		PreferredCodecs:  []string{"AVC, x264, H.264, H264, H 264"},
...
func TestAutoSelect_SmartCachedPrioritization(t *testing.T) {
	...
	highQuality1080p := &hibiketorrent.MediaTorrent{
		Name:     "[SubsPlease] Show - 01 [1080p][HEVC].mkv",
		InfoHash: "hash1",
		Seeders:  200,
		Provider: "catsound",
	}
	mediumQuality1080p := &hibiketorrent.MediaTorrent{
		Name:     "[RandomGroup] Show - 01 [1080p].mkv",
		InfoHash: "hash2",
		Seeders:  100,
		Provider: "catsound",
	}
	lowQuality720p := &hibiketorrent.MediaTorrent{
		Name:     "[LowQuality] Show - 01 [720p].mkv",
		InfoHash: "hash3",
		Seeders:  50,
		Provider: "catsound",
	}
	veryLowQuality480p := &hibiketorrent.MediaTorrent{
		Name:     "[BadGroup] Show - 01 [480p].mkv",
		InfoHash: "hash4",
		Seeders:  10,
		Provider: "catsound",
	}
	highQuality1080pAlt := &hibiketorrent.MediaTorrent{
	...
	tests := []struct {
	...
	}{
		{
			...
			torrents:     []*hibiketorrent.MediaTorrent{highQuality1080p, mediumQuality1080p, lowQuality720p, veryLowQuality480p},
		...
		},
		{
			...
			torrents:     []*hibiketorrent.MediaTorrent{highQuality1080p, mediumQuality1080p, lowQuality720p, veryLowQuality480p, highQuality1080pAlt},
		...
		},
		{
			...
			torrents:     []*hibiketorrent.MediaTorrent{highQuality1080p, lowQuality720p},
		...
		},
		{
			...
			torrents:     []*hibiketorrent.MediaTorrent{highQuality1080p, veryLowQuality480p},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			postSearchSort := func(torrents []*hibiketorrent.MediaTorrent) []*TorrentWithCacheStatus {
			...
			testTorrents := make([]*hibiketorrent.MediaTorrent, len(tt.torrents))
...
func TestAutoSelect_SmartCachedPrioritization_EdgeCases(t *testing.T) {
	...
	t.Run("Empty torrents list", func(t *testing.T) {
		...
		result := s.filterAndSort([]*hibiketorrent.MediaTorrent{}, nil, postSearchSort)
		assert.Empty(t, result)
	})

	t.Run("Single cached torrent", func(t *testing.T) {
		torrent := &hibiketorrent.MediaTorrent{
		...
		result := s.filterAndSort([]*hibiketorrent.MediaTorrent{torrent}, nil, postSearchSort)
	...
	t.Run("Nil postSearchSort function", func(t *testing.T) {
		...
		result := s.filterAndSort([]*hibiketorrent.MediaTorrent{torrent}, nil, nil)
		assert.Len(t, result, 1)
		assert.Equal(t, torrent.Name, result[0].Name)
	})
	...
	t.Run("Cached torrent exactly at 70% threshold", func(t *testing.T) {
		...
		highQuality := &hibiketorrent.MediaTorrent{
			Name:     "[SubsPlease] Show - 01 [1080p][HEVC].mkv",
			InfoHash: "hash1",
			Seeders:  200,
			Provider: "tosho",
		}
		thresholdQuality := &hibiketorrent.MediaTorrent{
		...
		result := s.filterAndSort([]*hibiketorrent.MediaTorrent{highQuality, thresholdQuality}, profile, postSearchSort)
