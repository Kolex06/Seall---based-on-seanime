...
import (
	...
	itorrent "seall/internal/torrents/torrent"
	"seall/internal/util"
...
type TestSearchProvider struct {
	SearchResults    map[string][]*hibiketorrent.MediaTorrent // keyed by resolution
	CanSmartSearch   bool
	SearchCallCount  int
...
func (f *TestSearchProvider) Search(opts hibiketorrent.AnimeSearchOptions) ([]*hibiketorrent.MediaTorrent, error) {
	f.SearchCallCount++
	...
	return []*hibiketorrent.MediaTorrent{}, nil
}

func (f *TestSearchProvider) SmartSearch(opts hibiketorrent.AnimeSmartSearchOptions) ([]*hibiketorrent.MediaTorrent, error) {
	f.SearchCallCount++
...
func (f *TestSearchProvider) GetTorrentInfoHash(torrent *hibiketorrent.MediaTorrent) (string, error) {
	return torrent.InfoHash, nil
}

func (f *TestSearchProvider) GetTorrentMagnetLink(torrent *hibiketorrent.MediaTorrent) (string, error) {
	return torrent.MagnetLink, nil
}

func (f *TestSearchProvider) GetLatest() ([]*hibiketorrent.MediaTorrent, error) {
	return []*hibiketorrent.MediaTorrent{}, nil
}
...
func createTestMedia(t *testing.T) *mediaapi.CompleteAnime {
	return &mediaapi.CompleteAnime{
		...
		Status: new(mediaapi.MediaStatusReleasing),
...
func TestSearchFromProvider_SingleResolution(t *testing.T) {
	...
	t1080p := []*hibiketorrent.MediaTorrent{
	...
	provider := &TestSearchProvider{
		SearchResults: map[string][]*hibiketorrent.MediaTorrent{
			"1080p": t1080p,
		},
		CanSmartSearch: true,
	}
...
func TestSearchFromProvider_MultipleResolutions_Fallback(t *testing.T) {
	...
	t720p := []*hibiketorrent.MediaTorrent{
...
func TestSearchFromProvider_NoResolutionsInProfile(t *testing.T) {
	...
	tAny := []*hibiketorrent.MediaTorrent{
...
func TestSearchFromProvider_BatchFallback(t *testing.T) {
	...
	tSingle := []*hibiketorrent.MediaTorrent{
	...
	tests := []struct {
	...
	}{
		{
			...
			media: &mediaapi.CompleteAnime{
				...
				Format: new(mediaapi.MediaFormatTv),
				EndDate: &mediaapi.CompleteAnime_EndDate{
		...
		},
		{
			...
			media: &mediaapi.CompleteAnime{
				Status: new(mediaapi.MediaStatusFinished),
				Format: new(mediaapi.MediaFormatMovie),
				EndDate: &mediaapi.CompleteAnime_EndDate{
					Year:  new(oldDate.Year()),
					Month: new(int(oldDate.Month())),
					Day:   new(oldDate.Day()),
...
func TestGetProvidersToSearch(t *testing.T) {
	...
	ext1 := extension.NewMediaTorrentProviderExtension(&extension.Extension{
		ID:   "provider1",
		Type: extension.TypeMediaTorrentProvider,
		Name: "Provider 1",
	}, provider1)
	...
	ext2 := extension.NewMediaTorrentProviderExtension(&extension.Extension{
		ID:   "provider2",
		Type: extension.TypeMediaTorrentProvider,
		Name: "Provider 2",
	}, provider2)
