...
func (s *AutoSelect) filterAndSort(torrents []*hibiketorrent.MediaTorrent, profile *anime.AutoSelectProfile, postSearchSort func([]*hibiketorrent.MediaTorrent) []*TorrentWithCacheStatus) []*hibiketorrent.MediaTorrent {
	...
	if postSearchSort != nil {
		filteredTorrents := make([]*hibiketorrent.MediaTorrent, len(candidates))
...
func (s *AutoSelect) filter(torrents []*hibiketorrent.MediaTorrent, profile *anime.AutoSelectProfile) []*hibiketorrent.MediaTorrent {
	candidates := make([]*candidate, len(torrents))
	for i, t := range torrents {
		candidates[i] = &candidate{
			torrent:   t,
	...
	candidates = s.filterCandidates(candidates, profile)
	ret := make([]*hibiketorrent.MediaTorrent, len(candidates))
	for i, c := range candidates {
		ret[i] = c.torrent
	}
	return ret
...
func (s *AutoSelect) sort(torrents []*hibiketorrent.MediaTorrent, profile *anime.AutoSelectProfile) {
	candidates := make([]*candidate, len(torrents))
	for i, t := range torrents {
		candidates[i] = &candidate{
			torrent:   t,
...
func (s *AutoSelect) smartCachedPrioritization(
	...
	postSearchSort func([]*hibiketorrent.MediaTorrent) []*TorrentWithCacheStatus,
) []*hibiketorrent.MediaTorrent {
	...
	highQualityCached := make([]*hibiketorrent.MediaTorrent, 0)
	lowQualityCached := make([]*hibiketorrent.MediaTorrent, 0)
	uncached := make([]*hibiketorrent.MediaTorrent, 0)
	...
	result := make([]*hibiketorrent.MediaTorrent, 0, len(torrents))
