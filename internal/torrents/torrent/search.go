...
type (
	...
	AnimeSearchOptions struct {
		...
		Media    *mediaapi.BaseAnime `json:"media,omitempty"`
		// Search options
	...
	SearchData struct {
		Torrents                  []*hibiketorrent.MediaTorrent                    `json:"torrents"`                  // Torrents found
...
func (r *Repository) SearchAnime(ctx context.Context, opts AnimeSearchOptions) (ret *SearchData, retErr error) {
	...
	var torrents []*hibiketorrent.MediaTorrent
	...
	animeMetadataF, errM := r.metadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, opts.Media.GetID())
	if errM == nil {
	...
	providerResults := make([][]*hibiketorrent.MediaTorrent, len(providers))
	providerErrors := make([]error, len(providers))
	for i, provider := range providers {
		go func(i int, provider extension.MediaTorrentProviderExtension) {
	...
	bestReleases := make([]*hibiketorrent.MediaTorrent, 0)
	other := make([]*hibiketorrent.MediaTorrent, 0)
	...
	torrents = lo.UniqBy(torrents, func(t *hibiketorrent.MediaTorrent) string {
		return t.InfoHash
	})
	...
	mu := sync.Mutex{}
	for _, t := range torrents {
		go func(t *hibiketorrent.MediaTorrent) {
			defer wg.Done()
	...
	if opts.Type == AnimeSearchTypeSmart && !opts.SkipPreviews {
		...
		for _, t := range torrents {
			go func(t *hibiketorrent.MediaTorrent) {
				...
				preview := r.createMediaTorrentPreview(createMediaTorrentPreviewOptions{
					torrent:       t,
					media:         opts.Media,
	...
	slices.SortFunc(torrents, func(i, j *hibiketorrent.MediaTorrent) int {
...
func (r *Repository) getAnimeSearchProviders(provider string) ([]extension.MediaTorrentProviderExtension, string, error) {
	...
	if len(ids) == 0 {
		...
		return []extension.MediaTorrentProviderExtension{ext}, ext.GetID(), nil
	}

	providers := make([]extension.MediaTorrentProviderExtension, 0, len(ids))
	resolvedIDs := make([]string, 0, len(ids))
	for _, id := range ids {
		ext, ok := extension.GetExtension[extension.MediaTorrentProviderExtension](r.extensionBankRef.Get(), id)
		if !ok {
			continue
		}
...
func sortSearchData(data *SearchData) {
	...
	slices.SortFunc(data.Torrents, func(i, j *hibiketorrent.MediaTorrent) int {
		if i.IsBestRelease != j.IsBestRelease {
			if i.IsBestRelease {
				return -1
			}
...
type createMediaTorrentPreviewOptions struct {
...
func (r *Repository) createMediaTorrentPreview(opts createMediaTorrentPreviewOptions) *Preview {
	defer util.HandlePanicInModuleThen("torrents/torrent/createMediaTorrentPreview", func() {})
