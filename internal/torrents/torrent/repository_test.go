...
func TestRepositoryProviderSelection(t *testing.T) {
	...
	filtered := repo.GetAnimeProviderExtensionsBy(func(ext extension.MediaTorrentProviderExtension) bool {
...
func TestSearchAnimeSimpleFallbackDedupAndSorting(t *testing.T) {
	...
	provider.searchResults = []*hibiketorrent.MediaTorrent{
...
func TestSearchAnimeAppliesSearchHookBeforeCaching(t *testing.T) {
	...
	hm.OnTorrentSearch().BindFunc(func(e hook_resolver.Resolver) error {
		...
		hookCalls++
		event.SearchData.Torrents = append(event.SearchData.Torrents, &hibiketorrent.MediaTorrent{
...
func TestSearchAnimeSmartSearchesRequestedProviders(t *testing.T) {
	...
	mainProvider.smartResults = []*hibiketorrent.MediaTorrent{{
	...
	specialSimpleProvider.searchResults = []*hibiketorrent.MediaTorrent{{
	...
	specialSmartProvider.smartResults = []*hibiketorrent.MediaTorrent{{
	...
	unusedSpecialProvider.smartResults = []*hibiketorrent.MediaTorrent{{
...
func TestSearchAnimeIgnoresIncludeSpecialProviders(t *testing.T) {
	...
	specialProvider.smartResults = []*hibiketorrent.MediaTorrent{{
...
type stubAnimeProvider struct {
	...
	smartResults  []*hibiketorrent.MediaTorrent
	searchErr     error
	smartErr      error
...
func (s *stubAnimeProvider) Search(opts hibiketorrent.AnimeSearchOptions) ([]*hibiketorrent.MediaTorrent, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.searchCalls++
...
func (s *stubAnimeProvider) SmartSearch(opts hibiketorrent.AnimeSmartSearchOptions) ([]*hibiketorrent.MediaTorrent, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.smartCalls++
	s.lastSmartOpt = opts
...
func (s *stubAnimeProvider) GetTorrentInfoHash(torrent *hibiketorrent.MediaTorrent) (string, error) {
	if torrent == nil {
		return "", nil
	}
...
func (s *stubAnimeProvider) GetTorrentMagnetLink(*hibiketorrent.MediaTorrent) (string, error) {
	return "magnet:?xt=urn:btih:test", nil
}

func (s *stubAnimeProvider) GetLatest() ([]*hibiketorrent.MediaTorrent, error) {
	return nil, nil
}
...
func cloneTorrents(in []*hibiketorrent.MediaTorrent) []*hibiketorrent.MediaTorrent {
	if in == nil {
		return nil
	}
	out := make([]*hibiketorrent.MediaTorrent, 0, len(in))
