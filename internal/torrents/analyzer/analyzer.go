...
type (
	NewAnalyzerOptions struct {
		...
		Media               *mediaapi.CompleteAnime // The media to compare the files with
		PlatformRef         *util.Ref[platform.Platform]
		MetadataProviderRef *util.Ref[metadata_provider.Provider]
...
func (a *Analyzer) AnalyzeTorrentFiles() (*Analysis, error) {
	if a.platformRef.IsAbsent() {
		return nil, errors.New("simkl client wrapper is nil")
	}
...
func (a *Analyzer) scanFiles() error {
	...
	simklRateLimiter := limiter.NewMediaApiLimiter()
	...
	if err := a.media.FetchMediaTree(mediaapi.FetchMediaTreeAll, a.platformRef.Get().GetMediaApiClient(), simklRateLimiter, tree, completeAnimeCache); err != nil {
		return err
	}
	...
	mc := scanner.NewMediaContainer(&scanner.MediaContainerOptions{
		AllMedia: scanner.NormalizedMediaFromSimklComplete(allMedia),
	})
	...
	fh := &scanner.FileHydrator{
		...
		MediaApiRateLimiter: simklRateLimiter,
		Logger:              a.logger,
		ScanLogger:          nil,
		ScanSummaryLogger:   nil,
