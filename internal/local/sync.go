...
type (
	...
	Syncer struct {
		...
		failedAnimeQueue *result.Cache[int, *mediaapi.AnimeListEntry]
		failedMangaQueue *result.Cache[int, *mediaapi.MangaListEntry]
...
func NewQueue(manager *ManagerImpl) *Syncer {
	ret := &Syncer{
		...
		failedAnimeQueue:             result.NewCache[int, *mediaapi.AnimeListEntry](),
		failedMangaQueue:             result.NewCache[int, *mediaapi.MangaListEntry](),
...
func (q *Syncer) synchronizeCollections() (err error) {
	...
	for _, _animeList := range _animeCollection.MediaListCollection.GetLists() {
		...
		list := &mediaapi.AnimeCollection_MediaListCollection_Lists{
			...
			Entries:      []*mediaapi.AnimeListEntry{},
		}
	...
	for _, _mangaList := range _mangaCollection.MediaListCollection.GetLists() {
		...
		list := &mediaapi.MangaCollection_MediaListCollection_Lists{
			...
			Entries:      []*mediaapi.MangaListEntry{},
		}
		...
		for _, _animeList := range _animeCollection.MediaListCollection.GetLists() {
			...
			for _, _animeEntry := range _animeList.GetEntries() {
				...
				for _, list := range localAnimeCollection.MediaListCollection.GetLists() {
					...
					editedAnime.CoverImage = &mediaapi.BaseAnime_CoverImage{
					...
					var startedAt *mediaapi.AnimeCollection_MediaListCollection_Lists_Entries_StartedAt
					...
					var completedAt *mediaapi.AnimeCollection_MediaListCollection_Lists_Entries_CompletedAt
		...
		for _, _mangaList := range _mangaCollection.MediaListCollection.GetLists() {
			...
			for _, _mangaEntry := range _mangaList.GetEntries() {
				...
				for _, list := range localMangaCollection.MediaListCollection.GetLists() {
					...
					editedManga.CoverImage = &mediaapi.BaseManga_CoverImage{
						ExtraLarge: FormatAssetUrl(snapshot.MediaId, snapshot.CoverImagePath),
						Large:      FormatAssetUrl(snapshot.MediaId, snapshot.CoverImagePath),
						Medium:     FormatAssetUrl(snapshot.MediaId, snapshot.CoverImagePath),
						Color:      FormatAssetUrl(snapshot.MediaId, snapshot.CoverImagePath),
					...
					var startedAt *mediaapi.MangaCollection_MediaListCollection_Lists_Entries_StartedAt
					...
					var completedAt *mediaapi.MangaCollection_MediaListCollection_Lists_Entries_CompletedAt
...
func (q *Syncer) sendAnimeToFailedQueue(entry *mediaapi.AnimeListEntry) {
	q.failedAnimeQueue.Set(entry.Media.ID, entry)
}

func (q *Syncer) sendMangaToFailedQueue(entry *mediaapi.MangaListEntry) {
...
func (q *Syncer) synchronizeAnime(diff *AnimeDiffResult) {
	...
	if diff.DiffType == DiffTypeMissing || diff.DiffType == DiffTypeMetadata {
		...
		var err error
		animeMetadata, err = q.manager.metadataProviderRef.Get().GetAnimeMetadata(metadata.MediaPlatform, entry.Media.ID)
		if err != nil {
			...
			simpleEntry, err := anime.NewSimpleEntry(context.Background(), &anime.NewSimpleAnimeEntryOptions{
				...
				PlatformRef:         q.manager.mediaPlatformRef,
