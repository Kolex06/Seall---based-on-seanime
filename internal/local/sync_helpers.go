...
import (
	...
	"seall/internal/util"
	"seall/internal/util/image_downloader"
...
func BaseAnimeDeepCopy(animeCollection *mediaapi.BaseAnime) *mediaapi.BaseAnime {
	bytes, err := json.Marshal(animeCollection)
	if err != nil {
		return nil
	}

	deepCopy := &mediaapi.BaseAnime{}
...
func BaseMangaDeepCopy(animeCollection *mediaapi.BaseManga) *mediaapi.BaseManga {
	bytes, err := json.Marshal(animeCollection)
	if err != nil {
		return nil
	}

	deepCopy := &mediaapi.BaseManga{}
	err = json.Unmarshal(bytes, deepCopy)
	if err != nil {
		return nil
	}
...
func MediaListStatusPointerValue(a *mediaapi.MediaListStatus) mediaapi.MediaListStatus {
	if a == nil {
...
func DownloadMangaImages(logger *zerolog.Logger, assetsDir string, entry *mediaapi.MangaListEntry) (string, string, bool) {
