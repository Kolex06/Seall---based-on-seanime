...
func (ldb *Database) SaveAnimeCollection(ac *mediaapi.AnimeCollection) error {
	return ldb._saveLocalCollection(AnimeType, ac)
}

func (ldb *Database) SaveMangaCollection(mc *mediaapi.MangaCollection) error {
	return ldb._saveLocalCollection(MangaType, mc)
}

func (ldb *Database) GetLocalAnimeCollection() (*mediaapi.AnimeCollection, bool) {
	lc, ok := ldb._getLocalCollection(AnimeType)
	if !ok {
		return nil, false
	}

	var ac mediaapi.AnimeCollection
	err := json.Unmarshal(lc.Value, &ac)

	return &ac, err == nil
}

func (ldb *Database) GetLocalMangaCollection() (*mediaapi.MangaCollection, bool) {
	lc, ok := ldb._getLocalCollection(MangaType)
	if !ok {
		return nil, false
	}

	var mc mediaapi.MangaCollection
...
func (ldb *Database) filterOutCustomSourceAnime(ac *mediaapi.AnimeCollection) *mediaapi.AnimeCollection {
	if ac == nil || ac.MediaListCollection == nil {
		return ac
	}

	// Create a deep copy
	filtered := &mediaapi.AnimeCollection{
	...
	for _, list := range ac.MediaListCollection.Lists {
		...
		filteredList := &mediaapi.AnimeCollection_MediaListCollection_Lists{
			Status:  list.Status,
...
func (ldb *Database) filterOutCustomSourceManga(mc *mediaapi.MangaCollection) *mediaapi.MangaCollection {
	if mc == nil || mc.MediaListCollection == nil {
		return mc
	}

	// Create a deep copy
	filtered := &mediaapi.MangaCollection{
		MediaListCollection: &mediaapi.MangaCollection_MediaListCollection{
			Lists: make([]*mediaapi.MangaCollection_MediaListCollection_Lists, 0),
		},
	}
	...
	for _, list := range mc.MediaListCollection.Lists {
		...
		filteredList := &mediaapi.MangaCollection_MediaListCollection_Lists{
			Status:  list.Status,
			Entries: make([]*mediaapi.MangaCollection_MediaListCollection_Lists_Entries, 0),
		}
...
func (ldb *Database) SaveSimulatedAnimeCollection(ac *mediaapi.AnimeCollection) error {
...
func (ldb *Database) SaveSimulatedMangaCollection(mc *mediaapi.MangaCollection) error {
	// Filter out custom sources
	filtered := ldb.filterOutCustomSourceManga(mc)
	return ldb._saveSimulatedCollection(MangaType, filtered)
}

func (ldb *Database) GetSimulatedAnimeCollection() (*mediaapi.AnimeCollection, bool) {
...
func (ldb *Database) GetSimulatedMangaCollection() (*mediaapi.MangaCollection, bool) {
