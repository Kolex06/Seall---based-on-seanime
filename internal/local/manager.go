...
type Manager interface {
	...
	// SynchronizeSimkl syncs the user's SIMKL data with data stored in the local database.
	SynchronizeMediaApi() error
	// SetRefreshMediaCollectionsFunc sets the function to call to refresh the online SIMKL collections.
	SetRefreshMediaCollectionsFunc(func())
	...
	GetSimulatedAnimeCollection() mo.Option[*mediaapi.AnimeCollection]
	// GetSimulatedMangaCollection returns the simulated manga collection for unauthenticated users.
	GetSimulatedMangaCollection() mo.Option[*mediaapi.MangaCollection]
	// SaveSimulatedAnimeCollection sets the simulated anime collection for unauthenticated users.
	SaveSimulatedAnimeCollection(ac *mediaapi.AnimeCollection)
	// SaveSimulatedMangaCollection sets the simulated manga collection for unauthenticated users.
	SaveSimulatedMangaCollection(mc *mediaapi.MangaCollection)
	// SynchronizeSimulatedCollectionToSimkl synchronizes the simulated anime and manga collections to the user's SIMKL account.
	SynchronizeSimulatedCollectionToMediaApi() error
	// SynchronizeMediaToSimulatedCollection synchronizes the user's SIMKL account to the simulated anime and manga collections.
	SynchronizeMediaToSimulatedCollection() error
...
type (
	ManagerImpl struct {
		...
		localAnimeCollection mo.Option[*mediaapi.AnimeCollection]
		// Manga collection stored in the local database, without modifications
		localMangaCollection mo.Option[*mediaapi.MangaCollection]

		// Anime collection from the user's SIMKL account, changed by ManagerImpl.SetAnimeCollection
		animeCollection mo.Option[*mediaapi.AnimeCollection]
		// Manga collection from the user's SIMKL account, changed by ManagerImpl.SetMangaCollection
		...
		RefreshMediaCollectionsFunc func()
	}
	TrackedMediaItem struct {
		MediaId    int                      `json:"mediaId"`
		Type       string                   `json:"type"`
		AnimeEntry *mediaapi.AnimeListEntry `json:"animeEntry,omitempty"`
		MangaEntry *mediaapi.MangaListEntry `json:"mangaEntry,omitempty"`
	}
...
func NewManager(opts *NewManagerOptions) (Manager, error) {
	...
	ret := &ManagerImpl{
		...
		localAnimeCollection:        mo.None[*mediaapi.AnimeCollection](),
		localMangaCollection:        mo.None[*mediaapi.MangaCollection](),
		...
		mediaPlatformRef:            opts.MediaPlatformRef,
		RefreshMediaCollectionsFunc: func() {},
	}
...
func (m *ManagerImpl) SetRefreshMediaCollectionsFunc(f func()) {
	m.RefreshMediaCollectionsFunc = f
}
...
func (m *ManagerImpl) loadLocalAnimeCollection() {
	...
	if !ok {
		m.localAnimeCollection = mo.None[*mediaapi.AnimeCollection]()
		return
	}
...
func (m *ManagerImpl) loadLocalMangaCollection() {
	...
	if !ok {
		m.localMangaCollection = mo.None[*mediaapi.MangaCollection]()
		return
	}
	m.localMangaCollection = mo.Some(collection)
}

func (m *ManagerImpl) SetAnimeCollection(ac *mediaapi.AnimeCollection) {
	if ac == nil {
		m.animeCollection = mo.None[*mediaapi.AnimeCollection]()
	} else {
		m.animeCollection = mo.Some[*mediaapi.AnimeCollection](ac)
	}
}

func (m *ManagerImpl) SetMangaCollection(mc *mediaapi.MangaCollection) {
	if mc == nil {
		m.mangaCollection = mo.None[*mediaapi.MangaCollection]()
	} else {
		m.mangaCollection = mo.Some[*mediaapi.MangaCollection](mc)
	}
}

func (m *ManagerImpl) GetLocalAnimeCollection() mo.Option[*mediaapi.AnimeCollection] {
	return m.localAnimeCollection
}

func (m *ManagerImpl) GetLocalMangaCollection() mo.Option[*mediaapi.MangaCollection] {
	return m.localMangaCollection
}

func (m *ManagerImpl) UpdateLocalAnimeCollection(ac *mediaapi.AnimeCollection) {
	_ = m.localDb.SaveAnimeCollection(ac)
	m.loadLocalAnimeCollection()
}

func (m *ManagerImpl) UpdateLocalMangaCollection(mc *mediaapi.MangaCollection) {
	...
	if ok {
		for _, list := range animeCollection.MediaListCollection.Lists {
			for _, entry := range list.GetEntries() {
				if entry.Status == nil || *entry.GetStatus() != mediaapi.MediaListStatusCurrent {
					continue
				}
...
func (m *ManagerImpl) TrackAnime(mId int) error {
	...
	if _, found := m.animeCollection.MustGet().GetListEntryFromAnimeId(mId); !found {
		...
		return fmt.Errorf("anime is not in SIMKL collection")
	}
...
func (m *ManagerImpl) TrackManga(mId int) error {
	...
	if _, found := m.mangaCollection.MustGet().GetListEntryFromMangaId(mId); !found {
		...
		return fmt.Errorf("manga is not in SIMKL collection")
	}
...
func (m *ManagerImpl) SynchronizeMediaApi() error {
	if m.animeCollection.IsAbsent() {
		return fmt.Errorf("local manager: Anime collection not set")
	}
	...
	if localAnimeCollection, ok := m.localAnimeCollection.Get(); ok {
		for _, list := range localAnimeCollection.MediaListCollection.Lists {
			...
			for _, entry := range list.GetEntries() {
				...
				// Get the entry from SIMKL
				var originalEntry *mediaapi.AnimeListEntry
				...
				var startDate *mediaapi.FuzzyDateInput
				if entry.GetStartedAt() != nil {
					startDate = &mediaapi.FuzzyDateInput{
				...
				var endDate *mediaapi.FuzzyDateInput
				if entry.GetCompletedAt() != nil {
					endDate = &mediaapi.FuzzyDateInput{
				...
				_ = m.mediaPlatformRef.Get().UpdateEntry(
					context.Background(),
	...
	if localMangaCollection, ok := m.localMangaCollection.Get(); ok {
		for _, list := range localMangaCollection.MediaListCollection.Lists {
			...
			for _, entry := range list.GetEntries() {
				...
				// Get the entry from SIMKL
				var originalEntry *mediaapi.MangaListEntry
	...
	m.RefreshMediaCollectionsFunc()

	m.wsEventManager.SendEvent(events.RefreshedMediaCollection, nil)
	m.wsEventManager.SendEvent(events.RefreshedMangaCollection, nil)
...
func (m *ManagerImpl) GetSimulatedAnimeCollection() mo.Option[*mediaapi.AnimeCollection] {
	ac, ok := m.localDb.GetSimulatedAnimeCollection()
	if !ok {
		return mo.None[*mediaapi.AnimeCollection]()
	}
	return mo.Some(ac)
}

func (m *ManagerImpl) GetSimulatedMangaCollection() mo.Option[*mediaapi.MangaCollection] {
	mc, ok := m.localDb.GetSimulatedMangaCollection()
	if !ok {
		return mo.None[*mediaapi.MangaCollection]()
	}
	return mo.Some(mc)
}

func (m *ManagerImpl) SaveSimulatedAnimeCollection(ac *mediaapi.AnimeCollection) {
...
func (m *ManagerImpl) SaveSimulatedMangaCollection(mc *mediaapi.MangaCollection) {
...
func (m *ManagerImpl) SynchronizeMediaToSimulatedCollection() error {
	m.logger.Trace().Msg("local manager: Synchronizing Simkl to simulated (local) collection")
...
func (m *ManagerImpl) SynchronizeSimulatedCollectionToMediaApi() error {
	if m.animeCollection.IsAbsent() {
		return fmt.Errorf("local manager: Cannot sync to simkl, anime collection not set")
	}

	if m.mangaCollection.IsAbsent() {
		return fmt.Errorf("local manager: Cannot sync to simkl, manga collection not set")
	}
