...
type GetAnimeDiffOptions struct {
	...
	LocalCollection mo.Option[*mediaapi.AnimeCollection]
...
type AnimeDiffResult struct {
	AnimeEntry    *mediaapi.AnimeListEntry
...
type GetMangaDiffOptions struct {
	Collection                  *mediaapi.MangaCollection
	LocalCollection             mo.Option[*mediaapi.MangaCollection]
...
type MangaDiffResult struct {
	MangaEntry    *mediaapi.MangaListEntry
...
func GetAnimeReferenceKey(bAnime *mediaapi.BaseAnime, lfs []*anime.LocalFile) string {
...
func GetMangaReferenceKey(bManga *mediaapi.BaseManga, dcc []*manga.ChapterContainer) string {
	// Reference key is used to compare the snapshot with the current data.
	// If the reference key is different, the snapshot is outdated.
...
func GetAnimeListDataKey(entry *mediaapi.AnimeListEntry) string {
...
func GetMangaListDataKey(entry *mediaapi.MangaListEntry) string {
	return fmt.Sprintf("%s-%d-%f-%d-%v-%v-%v-%v-%v-%v",
		MediaListStatusPointerValue(entry.GetStatus()),
		IntPointerValue(entry.GetProgress()),
		Float64PointerValue(entry.GetScore()),
