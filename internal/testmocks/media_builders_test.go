...
func TestBaseAnimeBuilder(t *testing.T) {
	anime := NewBaseAnimeBuilder(44, "seed title").
		...
		WithStatus(mediaapi.MediaStatusReleasing).
		WithFormat(mediaapi.MediaFormatMovie).
		WithEpisodes(24).
		WithIsAdult(true).
	...
	require.Equal(t, mediaapi.MediaTypeAnime, *anime.Type)
	...
	require.Equal(t, mediaapi.MediaStatusReleasing, *anime.Status)
	require.Equal(t, mediaapi.MediaFormatMovie, *anime.Format)
...
func TestBaseMangaBuilder(t *testing.T) {
	manga := NewBaseMangaBuilder(88, "seed manga").
		...
		WithStatus(mediaapi.MediaStatusHiatus).
		WithFormat(mediaapi.MediaFormatOneShot).
		WithChapters(10).
		WithVolumes(2).
		WithIsAdult(true).
	...
	require.Equal(t, mediaapi.MediaTypeManga, *manga.Type)
	...
	require.Equal(t, mediaapi.MediaStatusHiatus, *manga.Status)
	require.Equal(t, mediaapi.MediaFormatOneShot, *manga.Format)
