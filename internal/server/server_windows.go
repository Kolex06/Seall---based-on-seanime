...
func addQuitItem() {
	...
	mQuit := systray.AddMenuItem("Quit Seall", "Quit the whole app")
	mQuit.Enable()
	go func() {
		<-mQuit.ClickedCh
...
func onReady(webFS *embed.FS, app *core.App, flags core.SeallFlags, selfupdater *updater.SelfUpdater) func() {
	return func() {
		systray.SetTemplateIcon(icon.Data, icon.Data)
		systray.SetTitle(fmt.Sprintf("Seall v%s", constants.Version))
		systray.SetTooltip(fmt.Sprintf("Seall v%s", constants.Version))
		log.Trace().Msg("systray: App is ready")

		// Menu items
		systray.AddMenuItem("Seall v"+constants.Version, "Seall version")
		mWeb := systray.AddMenuItem(app.Config.GetServerURI("127.0.0.1"), "Open web interface")
		mOpenLibrary := systray.AddMenuItem("Open Media Library", "Open media library")
