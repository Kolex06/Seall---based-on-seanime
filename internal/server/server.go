...
import (
	...
	"seall/internal/util"
	"seall/internal/util/crashlog"
	"time"

	"github.com/rs/zerolog/log"
)

func startApp(embeddedLogo []byte) (*core.App, core.SeallFlags, *updater.SelfUpdater) {
	// Print the header
	core.PrintHeader()

	// Get the flags
	flags := core.GetSeallFlags()
	...
	// Create log file
	logFilePath := filepath.Join(app.Config.Logs.Dir, fmt.Sprintf("seall-%s.log", time.Now().Format("2006-01-02_15-04-05")))
	// Open the log file
...
func startAppLoop(webFS *embed.FS, app *core.App, flags core.SeallFlags, selfupdater *updater.SelfUpdater) {
