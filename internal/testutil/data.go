...
type FlagsConfig struct {
	EnableMediaApiTests         bool `mapstructure:"enable_simkl_tests"`
	EnableMediaApiMutationTests bool `mapstructure:"enable_simkl_mutation_tests"`
...
type ProviderConfig struct {
	MediaApiJwt          string `mapstructure:"simkl_jwt"`
	MediaApiUsername     string `mapstructure:"simkl_username"`
...
const (
	...
	RecordMediaApiFixturesEnvName = "SEALL_TEST_RECORD_MEDIA_FIXTURES"
)
...
// ShouldRecordMediaApiFixtures reports whether SIMKL mock helpers may write
// back into repository fixtures during a test run.
func ShouldRecordMediaApiFixtures() bool {
	value := os.Getenv(RecordMediaApiFixturesEnvName)
	if value == "" {
		return false
	}
...
func MediaApi() SkipFunc {
	return func(t testing.TB, cfg *Config) {
		t.Helper()
		if !cfg.Flags.EnableMediaApiTests {
			t.Skip("simkl tests disabled")
		}
	}
}

func SimklMutation() SkipFunc {
	return func(t testing.TB, cfg *Config) {
		t.Helper()
		if !cfg.Flags.EnableMediaApiMutationTests {
			t.Skip("simkl mutation tests disabled")
		}
		if cfg.Provider.MediaApiJwt == "" {
			t.Skip("simkl mutation tests require simkl_jwt")
		}
	}
}
