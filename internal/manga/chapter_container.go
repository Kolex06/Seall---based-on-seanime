...
func (r *Repository) RefreshChapterContainers(mangaCollection *mediaapi.MangaCollection, selectedProviderMap map[int]string) (err error) {
	...
	for mediaId, selectedProviderId := range selectedProviderMap {
		go func() {
			...
			if *mangaEntry.GetStatus() != mediaapi.MediaListStatusCurrent && *mangaEntry.GetStatus() != mediaapi.MediaListStatusRepeating {
				return
			}
