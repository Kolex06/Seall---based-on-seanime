...
func NewEntry(ctx context.Context, opts *NewEntryOptions) (entry *Entry, err error) {
	...
	simklEntry, found := opts.MangaCollection.GetListEntryFromMangaId(opts.MediaId)

	// If the entry is not found, we fetch the manga from the Simkl API.
	if !found {
		...
		mangaEvent.Manga = simklEntry.GetMedia()
