...
func RefreshMediaDataJob(c *JobCtx) {
	defer func() {
		if r := recover(); r != nil {
		}
	}()
	...
	// Refresh the Simkl Collection
	animeCollection, _ := c.App.RefreshAnimeCollection()
	c.App.WSEventManager.SendEvent(events.RefreshedMediaCollection, animeCollection)

	mangaCollection, _ := c.App.RefreshMangaCollection()
	c.App.WSEventManager.SendEvent(events.RefreshedMangaCollection, mangaCollection)
}
