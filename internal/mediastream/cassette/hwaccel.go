...
import (
	...
	"os/exec"
	"runtime"
	"seall/internal/mediastream/videofile"
	"strings"
	"time"
...
func vaApiProfile(device string) HwAccelProfile {
	return HwAccelProfile{
		...
		DecodeFlags: []string{
			"-hwaccel", "vaapi",
			"-hwaccel_device", GetEnvOr("SEALL_TRANSCODER_VAAPI_RENDERER", device),
...
func qsvProfile(device, preset string) HwAccelProfile {
	return HwAccelProfile{
		...
		DecodeFlags: []string{
			"-hwaccel", "qsv",
			"-qsv_device", GetEnvOr("SEALL_TRANSCODER_QSV_RENDERER", device),
