...
func (m *Manager) RoomsAvailable() bool {
	if constants.SeallRoomsApiUrl == "" {
		return false
	}

	resp, err := m.reqClient.R().
		Get(constants.SeallRoomsApiUrl + "/health")
	if err != nil {
		return false
	}
	...
	if createResp.Status != "healthy" && createResp.Version != constants.SeallRoomsVersion {
		return false
	}
...
// CreateAndJoinRoom creates a room using Seall Rooms API and connects as host
...
func (m *Manager) HandlePeerConnection(w http.ResponseWriter, r *http.Request) {
	...
	username := r.Header.Get("X-Seall-Nakama-Username")
	...
	peerID := r.Header.Get("X-Seall-Nakama-Peer-Id")
	if peerID == "" {
	...
	serverVersion := r.Header.Get("X-Seall-Nakama-Server-Version")
