...
func (m *Manager) GetHostAnimeLibraryFiles(ctx context.Context, mId ...int) (lfs []*anime.LocalFile, customSourceMap NakamaCustomSourceMap, hydrated bool) {
	...
	if len(mId) > 0 {
		response, err = m.reqClient.R().
			SetContext(ctx).
			SetHeader("X-Seall-Nakama-Token", m.settings.RemoteServerPassword).
			Get(m.GetHostBaseServerURL() + "/api/v1/nakama/host/media/library/files/" + strconv.Itoa(mId[0]))
		if err != nil {
			return nil, nil, false
		}
	} else {
		response, err = m.reqClient.R().
			SetContext(ctx).
			SetHeader("X-Seall-Nakama-Token", m.settings.RemoteServerPassword).
			Get(m.GetHostBaseServerURL() + "/api/v1/nakama/host/media/library/files")
		if err != nil {
			return nil, nil, false
		}
	}
...
func (m *Manager) GetHostAnimeLibrary(ctx context.Context) (ac *NakamaAnimeLibrary, hydrated bool) {
	...
	response, err = m.reqClient.R().
		SetContext(ctx).
		SetHeader("X-Seall-Nakama-Token", m.settings.RemoteServerPassword).
		Get(m.GetHostBaseServerURL() + "/api/v1/nakama/host/media/library")
	if err != nil {
		return nil, false
	}
...
func (m *Manager) PlayHostAnimeLibraryFile(path string, userAgent string, clientId string, media *mediaapi.BaseAnime, aniDBEpisode string, forcePlaybackMethod string) error {
	...
	response, err := m.reqClient.R().
		SetHeader("X-Seall-Nakama-Token", m.settings.RemoteServerPassword).
		Get(m.GetHostBaseServerURL() + "/api/v1/nakama/host/media/library/shared")
	if err != nil {
...
func (m *Manager) PlayHostAnimeStream(streamType WatchPartyStreamType, userAgent string, clientId string, media *mediaapi.BaseAnime, aniDBEpisode string) error {
