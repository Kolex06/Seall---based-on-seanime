...
const (
	tempReleaseDir = "seall_new_release"
...
func NewSelfUpdater() *SelfUpdater {
	...
	ret.tmpExecutableName = "seall.exe.old"
	switch runtime.GOOS {
	case "windows":
		ret.tmpExecutableName = "seall.exe.old"
	default:
		ret.tmpExecutableName = "seall.old"
	}
...
func (su *SelfUpdater) recover(assetUrl string) {
	...
	su.logger.Error().Msg("selfupdate: Failed to install update. Update downloaded to 'seall_new_release'")
}

func getExePath() string {
	exe, err := os.Executable() // /path/to/seall.exe
	if err != nil {
		return ""
	}
	exePath, err := filepath.EvalSymlinks(exe) // /path/to/seall.exe
	if err != nil {
		return ""
	}
...
func (su *SelfUpdater) Run() error {
	...
	// Download the asset to exeDir/seall_tmp
	...
	// seall.exe + /backup_restore_if_failed/seall.exe
	...
	for _, file := range files {
		...
		if err != nil && (file == "seall" || file == "seall.exe") {
			renamingFailed = true
	...
	// Remove seall.exe.old and LICENSE.old
	for _, file := range files {
