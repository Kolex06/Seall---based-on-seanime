...
func (u *Updater) decompressZip(archivePath string, folderName string) (dest string, err error) {
	topFolderName := "seall-" + u.LatestRelease.Version
	if folderName != "" {
		topFolderName = folderName
	}
	// "/seall-repo/seall-v1.0.0.zip" -> "/seall-repo/seall-1.0.0/"
	dest = filepath.Join(filepath.Dir(archivePath), topFolderName) // "/seall-repo/seall-v1.0.0"
...
//   - "seall-v1.zip" -> "/seall-repo/seall-v1.zip"
