...
func newUpdaterTestFixture(t *testing.T) *updaterTestFixture {
	...
	release := &Release{
		...
		Assets: []ReleaseAsset{
			{
				Url:                server.URL + "/assets/seall-3.5.2_Windows_x86_64.zip",
				Id:                 1,
				NodeId:             "asset-zip",
				Name:               "seall-3.5.2_Windows_x86_64.zip",
				ContentType:        "application/zip",
				Uploaded:           true,
				Size:               int64(len(zipArchive)),
				BrowserDownloadUrl: server.URL + "/assets/seall-3.5.2_Windows_x86_64.zip",
			},
			{
				Url:                server.URL + "/assets/seall-3.5.2_MacOS_arm64.tar.gz",
				Id:                 2,
				NodeId:             "asset-tar",
				Name:               "seall-3.5.2_MacOS_arm64.tar.gz",
				ContentType:        "application/gzip",
				Uploaded:           true,
				Size:               int64(len(tarGzArchive)),
				BrowserDownloadUrl: server.URL + "/assets/seall-3.5.2_MacOS_arm64.tar.gz",
			},
		},
	}
	...
	mux.HandleFunc("/assets/seall-3.5.2_Windows_x86_64.zip", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/zip")
		_, _ = w.Write(zipArchive)
	})
	mux.HandleFunc("/assets/seall-3.5.2_MacOS_arm64.tar.gz", func(w http.ResponseWriter, r *http.Request) {
...
func (f *updaterTestFixture) apply(t *testing.T) {
	...
	oldSeallStableURL := seallStableUrl
	oldSeallNightlyURL := seallNightlyUrl
	...
	seallStableUrl = f.server.URL + "/api/updates/stable/stable_server.json"
	seallNightlyUrl = f.server.URL + "/api/updates/nightly/nightly_server.json"

	t.Cleanup(func() {
		websiteUrl = oldWebsiteURL
		fallbackGithubUrl = oldFallbackGithubURL
		githubCheckUrl = oldGithubCheckURL
		seallStableUrl = oldSeallStableURL
		seallNightlyUrl = oldSeallNightlyURL
	})
}
