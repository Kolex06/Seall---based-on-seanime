...
var (
	websiteUrl           = "https://api.github.com/repos/Kolex06/Seall---based-on-seanime/releases/latest"
	fallbackGithubUrl    = "https://api.github.com/repos/Kolex06/Seall---based-on-seanime/releases/latest"
	githubCheckUrl       = ""
	seallStableUrl       = ""
	seallNightlyUrl      = ""
...
func (u *Updater) GetReleaseName(version string) string {
	...
	return fmt.Sprintf("seall-%s_%s_%s.%s", version, oos, arch, ext)
}
...
func (u *Updater) fetchLatestRelease(channel string) (*Release, error) {
	...
	switch channel {
	case "seall_nightly":
		apiRelease, err := u.fetchLatestReleaseFromGitHub()
		if err != nil {
			return nil, err
		}
		release = apiRelease
...
func (u *Updater) fetchGithubStatus() (string, bool) {
	...
	if res.Status == "down" {
		...
		return cmp.Or(res.Fallback, "seall"), false
	}
