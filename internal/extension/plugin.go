...
var (
	...
	PluginPermissionSimkl         PluginPermissionScope = "simkl"          // Allows the plugin to use the Simkl client
	PluginPermissionSimklToken    PluginPermissionScope = "simkl-token"    // Allows the plugin to see and use the Simkl token
...
func (p *PluginPermissions) GetDescription() string {
	...
	if len(p.Scopes) > 0 {
		...
		for _, scope := range p.Scopes {
			...
			case PluginPermissionSimkl:
				desc.WriteString("SIMKL: View and edit your SIMKL lists\n")
			case PluginPermissionSimklToken:
				desc.WriteString("SIMKL Token: View and use your SIMKL token\n")
...
func explainPath(path string) string {
	environmentVars := map[string]string{
		"$SEALL_ANIME_LIBRARY": "Your media library directories",
