...
type (
	...
	AnimeProvider interface {
		// Search for torrents.
		Search(opts AnimeSearchOptions) ([]*MediaTorrent, error)
		// SmartSearch for torrents.
		SmartSearch(opts AnimeSmartSearchOptions) ([]*MediaTorrent, error)
		...
		GetTorrentInfoHash(torrent *MediaTorrent) (string, error)
		...
		GetTorrentMagnetLink(torrent *MediaTorrent) (string, error)
		// GetLatest returns the latest torrents.
		GetLatest() ([]*MediaTorrent, error)
	...
	MediaTorrent struct {
		...
		// Leave this empty if you want Seall to format the size.
		...
		// Else, Seall will parse the torrent name to determine if it's a batch.
		IsBatch bool `json:"isBatch,omitempty"`
		// Episode number of the torrent.
		// Return -1 if unknown / unable to determine and Seall will parse the torrent name.
		EpisodeNumber int `json:"episodeNumber,omitempty"`
		// Release group of the torrent.
		// Leave this empty if you want Seall to parse the release group from the name.
	...
	BatchEpisodeFiles struct {
		...
		Files                []*MediaTorrentFile `json:"files"`
	}
)
