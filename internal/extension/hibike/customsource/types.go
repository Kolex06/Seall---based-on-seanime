...
// Custom sources allow users to add custom media, often things not available on SIMKL, to the app.
// At runtime the extension will be assigned a unique extension identifier (int). Extension IDs use bit-based separation:
// SIMKL IDs: 0 to 2^31-1, Extension IDs: 2^31+ with embedded extension identifier and local ID.
...
type (
	...
	Provider interface {
		...
		GetAnime(ctx context.Context, id []int) ([]*mediaapi.BaseAnime, error)
		ListAnime(ctx context.Context, search string, page int, perPage int) (*ListAnimeResponse, error)
		GetAnimeWithRelations(ctx context.Context, id int) (*mediaapi.CompleteAnime, error)
		GetAnimeMetadata(ctx context.Context, id int) (*metadata.AnimeMetadata, error)
		GetAnimeDetails(ctx context.Context, id int) (*mediaapi.AnimeDetailsById_Media, error)
		GetManga(ctx context.Context, id []int) ([]*mediaapi.BaseManga, error)
		ListManga(ctx context.Context, search string, page int, perPage int) (*ListMangaResponse, error)
		GetMangaDetails(ctx context.Context, id int) (*mediaapi.MangaDetailsById_Media, error)
	}
)
