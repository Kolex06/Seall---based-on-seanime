...
type (
	Settings struct {
		...
		// SupportsBidirectionalSync indicates if Seall can pull data from the tracker
	...
	MediaEntry struct {
		// Source is "simkl" for SIMKL entries, or the custom source ID for custom sources.
		//	e.g. For "One Piece" on SIMKL, this would be "simkl".
		//		For "Plur1bus" on the SIMKL custom source, this would be "simkl".
		// When Pulling: The extension MUST populate this.
		Source string `json:"source"`
		// MediaId is the SIMKL or custom source media ID (Seall ID).
		...
		// ExternalId
		//	When Pulling: The extension MUST populate this.
		//	When Pushing: Seall will populate this (using ResolveExternalId).
	...
	Provider interface {
		...
		// ResolveExternalId finds the tracker-specific ID for a given Seall media entry.
		// Seall calls this BEFORE calling PushEntry to ensure MediaEntry.ExternalId is populated.
...
const (
	...
	DiffTypeLocalOnly    DiffType = "local-only"    // Exists in Seall, missing in Tracker
	DiffTypeRemoteOnly   DiffType = "remote-only"   // Exists in Tracker, missing in Seall
