...
type PlatformHelper struct {
	...
	baseAnimeCache      *result.BoundedCache[int, *mediaapi.BaseAnime]
	baseMangaCache      *result.BoundedCache[int, *mediaapi.BaseManga]
	completeAnimeCache  *result.BoundedCache[int, *mediaapi.CompleteAnime]
...
func NewPlatformHelper(extensionBankRef *util.Ref[*extension.UnifiedBank], db *db.Database, logger *zerolog.Logger) *PlatformHelper {
	helper := &PlatformHelper{
		logger:              logger,
		baseAnimeCache:      result.NewBoundedCache[int, *mediaapi.BaseAnime](50),
		baseMangaCache:      result.NewBoundedCache[int, *mediaapi.BaseManga](50),
		completeAnimeCache:  result.NewBoundedCache[int, *mediaapi.CompleteAnime](10),
...
func (h *PlatformHelper) HandleCustomSourceAnime(ctx context.Context, mediaID int) (*mediaapi.BaseAnime, bool, error) {
	if h.customSourceManager == nil {
...
func (h *PlatformHelper) HandleCustomSourceAnimeDetails(ctx context.Context, mediaID int) (*mediaapi.AnimeDetailsById_Media, bool, error) {
	if h.customSourceManager == nil {
		return nil, false, nil
	}
...
func (h *PlatformHelper) HandleCustomSourceAnimeWithRelations(ctx context.Context, mediaID int) (*mediaapi.CompleteAnime, bool, error) {
	if h.customSourceManager == nil {
		return nil, false, nil
	}
...
func (h *PlatformHelper) HandleCustomSourceManga(ctx context.Context, mediaID int) (*mediaapi.BaseManga, bool, error) {
	if h.customSourceManager == nil {
		return nil, false, nil
	}
...
func (h *PlatformHelper) HandleCustomSourceMangaDetails(ctx context.Context, mediaID int) (*mediaapi.MangaDetailsById_Media, bool, error) {
	if h.customSourceManager == nil {
		return nil, false, nil
	}
...
func (h *PlatformHelper) GetCachedBaseAnime(mediaID int) (*mediaapi.BaseAnime, bool) {
	return h.baseAnimeCache.Get(mediaID)
}

func (h *PlatformHelper) SetCachedBaseAnime(mediaID int, anime *mediaapi.BaseAnime) {
	h.baseAnimeCache.SetT(mediaID, anime, time.Minute*30)
}

func (h *PlatformHelper) GetCachedBaseManga(mediaID int) (*mediaapi.BaseManga, bool) {
	return h.baseMangaCache.Get(mediaID)
}

func (h *PlatformHelper) SetCachedBaseManga(mediaID int, manga *mediaapi.BaseManga) {
	h.baseMangaCache.SetT(mediaID, manga, time.Minute*30)
}

func (h *PlatformHelper) GetCachedCompleteAnime(mediaID int) (*mediaapi.CompleteAnime, bool) {
	return h.completeAnimeCache.Get(mediaID)
}

func (h *PlatformHelper) SetCachedCompleteAnime(mediaID int, anime *mediaapi.CompleteAnime) {
...
func (h *PlatformHelper) TriggerGetAnimeEvent(anime *mediaapi.BaseAnime) (*mediaapi.BaseAnime, error) {
...
func (h *PlatformHelper) TriggerGetAnimeDetailsEvent(anime *mediaapi.AnimeDetailsById_Media) (*mediaapi.AnimeDetailsById_Media, error) {
...
func (h *PlatformHelper) TriggerGetMangaEvent(manga *mediaapi.BaseManga) (*mediaapi.BaseManga, error) {
...
func (h *PlatformHelper) TriggerGetStudioDetailsEvent(studio *mediaapi.StudioDetails) (*mediaapi.StudioDetails, error) {
...
func (h *PlatformHelper) MergeCustomSourceAnimeEntries(collection *mediaapi.AnimeCollection) {
	if h.customSourceManager != nil {
		h.customSourceManager.MergeAnimeEntries(collection)
	}
}

func (h *PlatformHelper) MergeCustomSourceMangaEntries(collection *mediaapi.MangaCollection) {
	if h.customSourceManager != nil {
...
func (h *PlatformHelper) BuildAnimeAiringSchedule(ctx context.Context, collection *mediaapi.AnimeCollection, client mediaapi.MediaApiClient) (*mediaapi.AnimeAiringSchedule, error) {
	mediaIds := make([]*int, 0)
	...
	var ret *mediaapi.AnimeAiringSchedule

	now := time.Now()
	currentSeason, currentSeasonYear := mediaapi.GetSeasonInfo(now, mediaapi.GetSeasonKindCurrent)
	previousSeason, previousSeasonYear := mediaapi.GetSeasonInfo(now, mediaapi.GetSeasonKindPrevious)
	nextSeason, nextSeasonYear := mediaapi.GetSeasonInfo(now, mediaapi.GetSeasonKindNext)
		...
		if len(retB.GetPage().GetMedia()) > 0 {
			...
			for _, m := range retB.Page.GetMedia() {
				if ret.OngoingNext == nil {
					ret.OngoingNext = &mediaapi.AnimeAiringSchedule_OngoingNext{
						Media: make([]*mediaapi.AnimeSchedule, 0),
					}
				}
				if m == nil {
					continue
...
func (h *PlatformHelper) HandleCustomSourceUpdateEntry(ctx context.Context, mediaID int, status *mediaapi.MediaListStatus, scoreRaw *int, progress *int, startedAt *mediaapi.FuzzyDateInput, completedAt *mediaapi.FuzzyDateInput) (bool, error) {
...
func (h *PlatformHelper) TriggerUpdateEntryHooks(ctx context.Context, mediaID int, status *mediaapi.MediaListStatus, scoreRaw *int, progress *int, startedAt *mediaapi.FuzzyDateInput, completedAt *mediaapi.FuzzyDateInput, updateFunc func(event *platform.PreUpdateEntryEvent) error) error {
...
func (h *PlatformHelper) FilterOutCustomAnimeLists(lists []*mediaapi.AnimeCollection_MediaListCollection_Lists) []*mediaapi.AnimeCollection_MediaListCollection_Lists {
	return lo.Filter(lists, func(list *mediaapi.AnimeCollection_MediaListCollection_Lists, _ int) bool {
		return list.Status != nil
	})
}

func (h *PlatformHelper) FilterOutCustomMangaLists(lists []*mediaapi.MangaCollection_MediaListCollection_Lists) []*mediaapi.MangaCollection_MediaListCollection_Lists {
	return lo.Filter(lists, func(list *mediaapi.MangaCollection_MediaListCollection_Lists, _ int) bool {
		return list.Status != nil
	})
}

func (h *PlatformHelper) RemoveNovelsFromMangaCollection(collection *mediaapi.MangaCollection) {
	for _, list := range collection.MediaListCollection.Lists {
		// Filter out novel entries
		list.Entries = lo.Filter(list.Entries, func(e *mediaapi.MangaCollection_MediaListCollection_Lists_Entries, _ int) bool {
			if e.GetMedia().GetFormat() == nil {
				return true
			}
			return *e.GetMedia().GetFormat() != mediaapi.MediaFormatNovel
		})
	}
}
