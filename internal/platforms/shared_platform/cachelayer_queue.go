...
type queuedMediaListUpdate struct {
	...
	StartedAt     *mediaapi.FuzzyDateInput  `json:"startedAt,omitempty"`
	CompletedAt   *mediaapi.FuzzyDateInput  `json:"completedAt,omitempty"`
...
func shouldQueueMediaListUpdate(err error) bool {
	...
	if errors.Is(err, context.Canceled) || errors.Is(err, mediaapi.ErrNotAuthenticated) {
		return false
	}
...
func (c *CacheLayer) queueMediaListEntryUpdate(mediaID *int, status *mediaapi.MediaListStatus, scoreRaw *int, progress *int, startedAt *mediaapi.FuzzyDateInput, completedAt *mediaapi.FuzzyDateInput) (int, error) {
	if mediaID == nil {
		return 0, errors.New("simkl cache: media ID is required to queue list update")
	}
...
func (c *CacheLayer) queueMediaListEntryProgressUpdate(mediaID *int, progress *int, status *mediaapi.MediaListStatus) (int, error) {
	if mediaID == nil {
		return 0, errors.New("simkl cache: media ID is required to queue progress update")
	}
...
func (c *CacheLayer) queueMediaListUpdate(update queuedMediaListUpdate) (int, error) {
	if !ShouldCache.Load() {
		return 0, errors.New("simkl cache: cache layer is disabled, list update cannot be queued")
	}
	...
	if err != nil {
		c.logger.Warn().Err(err).Int("mediaId", queued.MediaID).Msg("simkl cache: Failed to apply queued list update to cache")
	}
	if !patched {
		c.logger.Debug().Int("mediaId", queued.MediaID).Msg("simkl cache: Queued list update without a cached collection entry")
	}

	c.logger.Info().Int("mediaId", queued.MediaID).Msg("simkl cache: Queued list update for retry")
	return entryID, nil
}

func (c *CacheLayer) sendMediaListEntryUpdate(ctx context.Context, mediaID *int, status *mediaapi.MediaListStatus, scoreRaw *int, progress *int, startedAt *mediaapi.FuzzyDateInput, completedAt *mediaapi.FuzzyDateInput, interceptors ...clientv2.RequestInterceptor) (*mediaapi.UpdateMediaListEntry, error) {
	if mediaID == nil {
		return c.mediaClientRef.Get().UpdateMediaListEntry(ctx, mediaID, status, scoreRaw, progress, startedAt, completedAt, interceptors...)
	}

	c.pendingUpdateSyncMutex.Lock()
	defer c.pendingUpdateSyncMutex.Unlock()

	res, err := c.mediaClientRef.Get().UpdateMediaListEntry(ctx, mediaID, status, scoreRaw, progress, startedAt, completedAt, interceptors...)
	if err == nil {
...
func (c *CacheLayer) sendMediaListEntryProgressUpdate(ctx context.Context, mediaID *int, progress *int, status *mediaapi.MediaListStatus, interceptors ...clientv2.RequestInterceptor) (*mediaapi.UpdateMediaListEntryProgress, error) {
	if mediaID == nil {
		return c.mediaClientRef.Get().UpdateMediaListEntryProgress(ctx, mediaID, progress, status, interceptors...)
	}

	c.pendingUpdateSyncMutex.Lock()
	defer c.pendingUpdateSyncMutex.Unlock()

	res, err := c.mediaClientRef.Get().UpdateMediaListEntryProgress(ctx, mediaID, progress, status, interceptors...)
	if err == nil {
		c.deleteQueuedUpdate(*mediaID)
	}
	return res, err
...
func (c *CacheLayer) deleteQueuedUpdate(mediaID int) {
	...
	if err := c.fileCacher.DeletePerm(bucket, strconv.Itoa(mediaID)); err != nil {
		c.logger.Warn().Err(err).Int("mediaId", mediaID).Msg("simkl cache: Failed to delete queued list update")
	}
}
...
func (c *CacheLayer) syncQueuedUpdates(ctx context.Context) {
	...
	if err != nil {
		c.logger.Warn().Err(err).Msg("simkl cache: Failed to load queued list updates")
		return
	}
	if len(updates) == 0 {
		return
	...
	if synced > 0 {
		c.logger.Info().Int("count", synced).Msg("simkl cache: Synced queued list updates")
	}
}
...
func (c *CacheLayer) syncQueuedUpdate(ctx context.Context, update queuedMediaListUpdate) error {
	...
	if update.FullUpdate || update.ScoreRaw != nil || update.StartedAt != nil || update.CompletedAt != nil {
		_, err := c.mediaClientRef.Get().UpdateMediaListEntry(ctx, &mediaID, update.Status, update.ScoreRaw, update.Progress, update.StartedAt, update.CompletedAt)
		return err
	}
	...
	_, err := c.mediaClientRef.Get().UpdateMediaListEntryProgress(ctx, &mediaID, update.Progress, update.Status)
	return err
}
...
func (c *CacheLayer) setQueuedUpdateSyncFailed(update queuedMediaListUpdate, syncErr error) {
	...
	if err := c.fileCacher.SetPerm(bucket, key, current); err != nil {
		c.logger.Warn().Err(err).Int("mediaId", update.MediaID).Msg("simkl cache: Failed to update queued list retry state")
	}
}
...
func (c *CacheLayer) deleteQueuedUpdateIfCurrent(update queuedMediaListUpdate) bool {
	...
	if err := c.fileCacher.DeletePerm(bucket, key); err != nil {
		c.logger.Warn().Err(err).Int("mediaId", update.MediaID).Msg("simkl cache: Failed to delete synced queued list update")
		return false
	}
...
func (c *CacheLayer) applyQueuedUpdatesToAnimeCollection(collection *mediaapi.AnimeCollection) bool {
	updates, err := c.getQueuedMediaListUpdates()
	if err != nil {
		c.logger.Warn().Err(err).Msg("simkl cache: Failed to overlay queued anime updates")
		return false
	}
...
func (c *CacheLayer) applyQueuedUpdatesToMangaCollection(collection *mediaapi.MangaCollection) bool {
	updates, err := c.getQueuedMediaListUpdates()
	if err != nil {
		c.logger.Warn().Err(err).Msg("simkl cache: Failed to overlay queued manga updates")
		return false
	}
...
func applyQueuedUpdateToAnimeCollection(collection *mediaapi.AnimeCollection, update queuedMediaListUpdate) (int, bool) {
...
func applyQueuedUpdateToMangaCollection(collection *mediaapi.MangaCollection, update queuedMediaListUpdate) (int, bool) {
	if collection == nil || collection.MediaListCollection == nil {
		return 0, false
	}
...
func applyUpdateToAnimeEntry(entry *mediaapi.AnimeCollection_MediaListCollection_Lists_Entries, update queuedMediaListUpdate) {
	if update.Status != nil {
...
func applyUpdateToMangaEntry(entry *mediaapi.MangaCollection_MediaListCollection_Lists_Entries, update queuedMediaListUpdate) {
	if update.Status != nil {
		entry.Status = newCloned(update.Status)
	}
	if update.ScoreRaw != nil {
...
func rearrangeCachedAnimeCollectionLists(collection *mediaapi.AnimeCollection) {
...
func rearrangeCachedMangaCollectionLists(collection *mediaapi.MangaCollection) {
...
func getOrCreateCachedAnimeList(collection *mediaapi.AnimeCollection, status mediaapi.MediaListStatus) *mediaapi.AnimeCollection_MediaListCollection_Lists {
	for _, list := range collection.MediaListCollection.Lists {
...
func getOrCreateCachedMangaList(collection *mediaapi.MangaCollection, status mediaapi.MediaListStatus) *mediaapi.MangaCollection_MediaListCollection_Lists {
	for _, list := range collection.MediaListCollection.Lists {
		if list != nil && list.GetStatus() != nil && *list.GetStatus() == status {
			return list
		}
	...
	list := &mediaapi.MangaCollection_MediaListCollection_Lists{
		Status:       newCloned(&status),
		Name:         new(string(status)),
		IsCustomList: new(false),
		Entries:      []*mediaapi.MangaCollection_MediaListCollection_Lists_Entries{},
	}
	collection.MediaListCollection.Lists = append(collection.MediaListCollection.Lists, list)
	return list
}
...
func cloneFuzzyDateInput(value *mediaapi.FuzzyDateInput) *mediaapi.FuzzyDateInput {
	if value == nil {
		return nil
	}
	return &mediaapi.FuzzyDateInput{
