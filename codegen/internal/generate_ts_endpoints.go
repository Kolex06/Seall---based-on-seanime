...
`)
	...
	for _, filename := range filenames {
		...
		for _, route := range groupedByFile[filename] {
			...
			writeLine(f, fmt.Sprintf("\t\t%s: {", strings.TrimPrefix(route.Name, "Handle"))) // GetMediaCollection: {
			...
			writeLine(f, fmt.Sprintf("\t\t\tendpoint: \"%s\",", route.Api.Endpoint)) // path: '/api/v1/simkl/collection',
...
`
	...
	//	hooksF, err := os.Create(filepath.Join("../seall-web/src/api/hooks", filename+".hooks.ts"))
	//	if err != nil {
	//		panic(err)
	//	}
	//	defer hooksF.Close()
