import { useGetMediaCacheLayerStatus, useToggleMediaCacheLayerStatus } from "@/api/hooks/simkl.hooks"
import { useListMediaEntryEpisodeTabExtensions } from "@/api/hooks/extensions.hooks"
...
export function ServerSettings(props: ServerSettingsProps) {
    ...
    const { data: episodeTabExtensions } = useListMediaEntryEpisodeTabExtensions()
    ...
    const { data: isApiWorking, isLoading: isFetchingApiStatus } = useGetMediaCacheLayerStatus()
    const { mutate: toggleCacheLayer, isPending: isTogglingCacheLayer } = useToggleMediaCacheLayerStatus()
        ...
        <div className="space-y-4">
            ...
            {(!isApiWorking && !isFetchingApiStatus) && (
                <Alert
                    ...
                    description={<div className="space-y-1">
                        <p>The SIMKL API is not working. All requests will be served from the cache.</p>
            ...
            <SettingsCard>
                ...
                <Field.Switch
                    ...
                    help="If enabled, Seall will remember your watch progress and resume from where you left off."
                    moreHelp="Only applies to desktop and integrated players."
                ...
                <div data-settings-default-episode-source>
                    <Field.Select
                        ...
                        help="Used when opening media pages."
                ...
                <div data-settings-enable-extension-secure-mode>
                    <Field.Switch
                        ...
                        help="If enabled, Seall will prompt you for confirmation whenever an extension tries to perform a sensitive action, even if permissions have been granted."
            ...
            <SettingsCard
                title="Local Account"
                description="Local account is used when you're not using an SIMKL account."
            >
                <div className={cn(serverStatus?.user?.isSimulated && "opacity-50 pointer-events-none")}>
                    <Field.Switch
                        side="right"
                        name="autoSyncToLocalAccount"
                        label="Automatically back up SIMKL lists"
                        help="If enabled, your local lists will be periodically updated by using your SIMKL data. This will override any local changes you've made since the last sync."
                ...
                >
                    Upload local lists to SIMKL
                </Button>
            </SettingsCard>

            <ConfirmationDialog {...confirmDialog} />

            <SettingsCard title="Offline mode" description="Only available when authenticated with SIMKL.">
                ...
                <Field.Switch
                    ...
                    moreHelp="Will be paused if you have made changes offline and have not synced them to SIMKL yet."
                ...
                <Field.Switch
                    ...
                    help="If enabled, Seall will automatically save all media you're currently watching/reading for offline use."
            ...
            <SettingsCard title="App">
                ...
                <Field.Switch
                    ...
                    label="Disable SIMKL caching"
                    help="If enabled, Seall will stop caching SIMKL requests to disk."
                    moreHelp="By default, all requests made to SIMKL are cached. This allows Seall to keep being usable when SIMKL goes down. The cache directory is modifiable in the config file."
                ...
                {!f.watch("disableCacheLayer") && (
                    <div className="space-y-1 pl-4 border-l border-[--border] ml-2">
                        <Switch
                            ...
                            moreHelp="Seall will use cached data instead of making API requests."
                        />
                    </div>
                )}
                <Separator />
                <Field.Switch
                    ...
                    help="If enabled, Seall will use an alternative source to fetch episode metadata."
                ...
                <Field.Switch
                    ...
                    help={__isElectronDesktop__ ? (<span className="flex gap-2 items-center">
                        ...
                        <span>If enabled, new releases won't be displayed. Seall Denshi may still auto-update in the background.</span>
                    </span>) : "If enabled, Seall will not check for new releases."}
                    moreHelp={__isElectronDesktop__ ? "You cannot disable auto-updates for Seall Denshi." : undefined}
                    icon={<TbDownloadOff className="" />}
                />
                <Field.Select
                    label="Update Channel"
                    name="updateChannel"
                    help={__isElectronDesktop__ ? "Also applies to Seall Denshi auto-updates." : ""}
                    options={[
                        { label: "GitHub (Default)", value: "github" },
                        { label: "Seall", value: "seall" },
                        { label: "Seall (Canary)", value: "seall_nightly" },
                    ]}
                />
                {serverStatus?.settings?.library?.updateChannel === "seall" && (
                    <Alert intent="info" description="You are currently using a release channel hosted on Seall." />
                )}
                {serverStatus?.settings?.library?.updateChannel === "seall_nightly" && (
                    <Alert
                        intent="warning"
                        description="You are currently using the canary release channel hosted on Seall. This channel may receive unstable updates without much testing."
                    />
                )}
                <Separator />
                <Field.Switch
