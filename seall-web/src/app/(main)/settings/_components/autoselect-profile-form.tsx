import { Media_AutoSelectPreference, Media_AutoSelectProfile } from "@/api/generated/types"
...
type AutoSelectProfileFormProps = {
    profile?: Media_AutoSelectProfile
    onSuccess?: () => void
}
...
function PreferenceField(props: {
    label: string
    without?: string[]
    value: Media_AutoSelectPreference
    onChange: (value: Media_AutoSelectPreference) => void
}) {
    ...
    ].filter(({ value }) => !without.includes(value as Media_AutoSelectPreference))
    ...
    return (
        <div className="space-y-2">
            ...
            <Select
                value={value}
                onValueChange={(v) => onChange(v as Media_AutoSelectPreference)}
                options={options}
            />
        </div>
    )
