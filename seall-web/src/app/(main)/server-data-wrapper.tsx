...
export function ServerDataWrapper(props: ServerDataWrapperProps) {
    ...
    useWebsocketMessageListener({
        type: WSEvents.MEDIA_DATA_LOADED,
        onMessage: () => {
            logger("Data Wrapper").info("SIMKL data loaded, refetching server status")
            refetch()
        },
    })
    ...
    if (currentServerStatus.updating) {
        return <div className="container max-w-3xl py-10">
            <div className="mb-4 flex justify-center w-full">
                <img src="/seall-logo.png" alt="logo" className="w-14 h-auto" />
            </div>
            <p className="text-center text-lg">
                Seall is currently updating. Refresh the page once the update is complete and the connection has been reestablished.
            </p>
        </div>
    }
    ...
    if (!currentServerStatus.user && host === "127.0.0.2:43221" && !__isDesktop__) {
        return <div className="container max-w-3xl py-10">
            <Card className="md:py-10">
                <AppLayoutStack>
                    <div className="text-center space-y-4">
                        <div className="mb-4 flex justify-center w-full">
                            <img src="/seall-logo.png" alt="logo" className="w-24 h-auto" />
                        </div>
                        <h3>Welcome!</h3>
                        <Button
                            onClick={() => {
                                window.open(SIMKL_PIN_URL, "_self")
                            }}
                            leftIcon={<svg
