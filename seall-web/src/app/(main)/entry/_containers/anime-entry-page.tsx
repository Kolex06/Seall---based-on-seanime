...
import { usePluginMediaEntryEpisodeTabs } from "@/app/(main)/_features/plugin/plugin-entry-episode-tabs"
import {
    getPluginEpisodeTabExtensionId,
    getPluginEpisodeTabViewId,
    PluginMediaEntryEpisodeTab,
    PluginMediaEntryEpisodeTabContent,
...
function getAutomaticAnimeEntryView(entry: Media_Entry | undefined, serverStatus: ReturnType<typeof useServerStatus>) {
...
export function AnimeEntryPage() {
    ...
    const { data: animeEntry, isLoading: animeEntryLoading } = useGetMediaEntry(mediaId)
    const { data: animeDetails, isLoading: animeDetailsLoading } = useGetMediaDetails(mediaId)
    const { data: registeredEpisodeTabExtensions, isFetched: registeredEpisodeTabExtensionsFetched } = useListMediaEntryEpisodeTabExtensions()
    ...
    const pluginEpisodeTabs = usePluginMediaEntryEpisodeTabs({
    ...
    React.useLayoutEffect(() => {
        ...
        try {
            if (animeEntry?.media?.title?.userPreferred) {
                document.title = `${animeEntry?.media?.title?.userPreferred} | Seall`
            }
                    ...
                    <AnimatePresence mode="wait" initial={false}>
                        ...
                        >
                            ...
                            <div className="space-y-4" data-anime-entry-page-online-streaming-view-content>
                                ...
                                <OnlinestreamPage
                                    entry={animeEntry}
                                    entryLoading={animeEntryLoading}
                                    hideBackButton
                                />
                                {bottomSection}
                            </div>
...
type EntrySectionTabs = {
    ...
    entry: Media_Entry
    pluginTabs?: PluginMediaEntryEpisodeTab[]
}
