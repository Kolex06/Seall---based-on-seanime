...
import { simkl_animeIsSingleEpisode } from "@/lib/helpers/media"
import React, { memo } from "react"
import { TorrentList, TorrentListItem } from "./torrent-preview-item"

type TorrentTable = {
    entry?: Media_Entry
    torrents: HibikeTorrent_MediaTorrent[]
    selectedTorrents: HibikeTorrent_MediaTorrent[]
    globalFilter: string,
    ...
    isLoading: boolean
    isFetching: boolean
    onToggleTorrent: (t: HibikeTorrent_MediaTorrent) => void
                    ...
                    <ScrollAreaBox className={searchAcrossProviders ? "h-[calc(100dvh_-_30rem)]" : "h-[calc(100dvh_-_26rem)]"}>
                        <TorrentList>
                            {sortedTorrents.map(torrent => {
                                ...
                                const isBatch = torrent.isBatch ?? (!simkl_animeIsSingleEpisode(entry?.media) && (metadata?.metadata?.episode_number?.length ?? 0) > 1 || (metadata?.metadata?.episode_number?.length ?? 0) == 0)
