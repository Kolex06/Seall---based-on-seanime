...
export const EXTENSION_TYPE = {
    "plugin": "Plugin",
    "media-torrent-provider": "Media Torrent Provider",
    "manga-provider": "Reading Provider",
...
export function ExtensionList(props: ExtensionListProps) {
    ...
    const disabledExtensions = orderExtensions(allExtensions?.disabledExtensions ?? [])
        .filter(n => n.type !== "manga-provider")
        .filter(matchesSearchTerm)

    const pluginExtensions = enabledExtensions.filter(n => n.type === "plugin")
    const mediaTorrentExtensions = enabledExtensions.filter(n => n.type === "media-torrent-provider")
    ...
    const hasVisibleResults =
        ...
        mediaTorrentExtensions.length > 0 ||
