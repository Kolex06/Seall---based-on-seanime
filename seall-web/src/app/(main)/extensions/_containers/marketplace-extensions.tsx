...
const LEGACY_MARKETPLACE_OWNER = "/5" + "rahim/"
...
export function MarketplaceExtensions(props: MarketplaceExtensionsProps) {
    ...
    React.useEffect(() => {
        if (!marketplaceUrl || marketplaceUrl.includes(LEGACY_MARKETPLACE_OWNER)) {
            setMarketplaceUrl(DEFAULT_MARKETPLACE_URL)
        }
    }, [marketplaceUrl, setMarketplaceUrl])
    ...
    const filteredExtensions = React.useMemo(() => {
        ...
        let filtered = marketplaceExtensions.filter(ext => ext.type !== "manga-provider")
    ...
    const mediaTorrentExtensions = filteredExtensions.filter(n => n.type === "media-torrent-provider")
    ...
    return (
        <AppLayoutStack className="gap-6">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <div>
                    ...
                    <p className="text-[--muted] text-xs mt-1">
                        Source: {isDefaultMarketplace ?
                        <span>Seall repository</span> :
            ...
            <div className="flex flex-wrap gap-4">
                {!!missingDefaultTypes.length && (
                    <Alert
                        ...
                        description={<div>
                            <p>To protect Seall, some extensions have been removed. Find a new repository URL online and add it.</p>
                            <Button
                                intent="primary"
                                size="sm"
                                className="mt-2"
                ...
                <StaticTabs
                    ...
                    items={[
                        ...
                        {
                            name: "Media Torrents",
                            isCurrent: filterType === "media-torrent-provider",
                            onClick: () => setFilterType("media-torrent-provider"),
            ...
            {!!mediaTorrentExtensions?.length && (
                <Card className="p-4 space-y-6">
                    <h3 className="flex gap-3 items-center"><RiFolderDownloadFill />Media torrents</h3>
                    <div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
                        {mediaTorrentExtensions.map(extension => (
                            <MarketplaceExtensionCard
                                key={extension.id}
                                extension={extension}
                                isInstalled={isExtensionInstalled(extension.id)}
            ...
            {!!customSources?.length && (
                <Card className="p-4 space-y-6">
                    <div>
                        ...
                        <p className="text-[--muted] text-sm">
                            Custom sources let you browse media beyond what SIMKL provides.
                        </p>
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-3 2xl:grid-cols-4 gap-4">
