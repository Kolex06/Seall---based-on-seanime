...
const enum Functions {
    MediaTorrentProviderSearch = "MediaTorrentProvider.search",
    MediaTorrentProviderSmartSearch = "MediaTorrentProvider.smartSearch",
    MediaTorrentProviderGetTorrentInfoHash = "MediaTorrentProvider.getTorrentInfoHash",
    MediaTorrentProviderGetTorrentMagnetLink = "MediaTorrentProvider.getTorrentMagnetLink",
    MediaTorrentProviderGetLatest = "MediaTorrentProvider.getLatest",
    ...
    OnlinestreamFindMediaItems = "Onlinestream.findMediaItems",
    OnlinestreamFindMediaItemServer = "Onlinestream.findMediaItemServer",
}
...
export function ExtensionPlayground(props: ExtensionPlaygroundProps) {
    ...
    const [selectedFunction, setSelectedFunction] = React.useState(Functions.MediaTorrentProviderSearch)
    const [inputs, setInputs] = useAtom(withImmer(paramsAtom))

    React.useLayoutEffect(() => {
        if (type === "media-torrent-provider") {
            setSelectedFunction(Functions.MediaTorrentProviderSearch)
    ...
    function handleRunCode() {
        ...
        if (selectedFunction === Functions.MediaTorrentProviderSearch) {
            func = "search"
            ret = {
                mediaId: inputs.mediaTorrentProvider.mediaId,
                query: inputs.mediaTorrentProvider.search.query,
            }
        } else if (selectedFunction === Functions.MediaTorrentProviderSmartSearch) {
            func = "smartSearch"
            ret = {
                mediaId: inputs.mediaTorrentProvider.mediaId,
                options: {
                    query: inputs.mediaTorrentProvider.smartSearch.query,
                    episodeNumber: inputs.mediaTorrentProvider.smartSearch.episodeNumber,
                    resolution: inputs.mediaTorrentProvider.smartSearch.resolution,
                    batch: inputs.mediaTorrentProvider.smartSearch.batch,
                    bestReleases: inputs.mediaTorrentProvider.smartSearch.bestReleases,
                },
            }
        } else if (selectedFunction === Functions.MediaTorrentProviderGetTorrentInfoHash) {
            func = "getTorrentInfoHash"
            ret = {
                mediaId: inputs.mediaTorrentProvider.mediaId,
                torrent: inputs.mediaTorrentProvider.getTorrentInfoHash.torrent,
            }
        } else if (selectedFunction === Functions.MediaTorrentProviderGetTorrentMagnetLink) {
            func = "getTorrentMagnetLink"
            ret = {
                mediaId: inputs.mediaTorrentProvider.mediaId,
                torrent: inputs.mediaTorrentProvider.getTorrentMagnetLink.torrent,
            }
        } else if (selectedFunction === Functions.MediaTorrentProviderGetLatest) {
            func = "getLatest"
            ret = {
                mediaId: inputs.mediaTorrentProvider.mediaId,
            }
        ...
        } else if (selectedFunction === Functions.OnlinestreamFindMediaItems) {
            func = "findMediaItems"
            ret = {
                mediaId: inputs.onlineStreamingProvider.mediaId,
                id: inputs.onlineStreamingProvider.findMediaItems.id,
            }
        } else if (selectedFunction === Functions.OnlinestreamFindMediaItemServer) {
            func = "findMediaItemServer"
            ret = {
                mediaId: inputs.onlineStreamingProvider.mediaId,
                item: inputs.onlineStreamingProvider.findMediaItemServer.item,
                server: inputs.onlineStreamingProvider.findMediaItemServer.server,
            }
        } else {
                ...
                <div className="flex items-center w-full">
                    <div className="w-full flex items-center gap-4">
                        ...
                        <Select
                            ...
                            intent="filled"
                            options={[
                                { value: "media-torrent-provider", label: "Media Torrent Provider" },
                                    ...
                                    <ResizablePanelGroup direction="vertical" autoSaveId="sea-extension-playground-3">
                                        ...
                                        <ResizablePanel defaultSize={30} className="!overflow-y-auto">
                                            ...
                                            <div className="space-y-4 p-3">
                                                ...
                                                {type === "media-torrent-provider" && (
                                                    <>
                                                        <Select
                                                            leftAddon="Method"
                                                            value={selectedFunction}
                                                            options={[
                                                                { value: Functions.MediaTorrentProviderSearch, label: "search" },
                                                                { value: Functions.MediaTorrentProviderSmartSearch, label: "smartSearch" },
                                                                {
                                                                    value: Functions.MediaTorrentProviderGetTorrentInfoHash,
                                                                    label: "getTorrentInfoHash",
                                                                },
                                                                {
                                                                    value: Functions.MediaTorrentProviderGetTorrentMagnetLink,
                                                                    label: "getTorrentMagnetLink",
                                                                },
                                                                { value: Functions.MediaTorrentProviderGetLatest, label: "getLatest" },
                                                            ]}
                                                            onValueChange={v => {
                                                        ...
                                                        <NumberInput
                                                            ...
                                                            value={inputs.mediaTorrentProvider.mediaId}
                                                            onValueChange={v => {
                                                                setInputs(d => {
                                                                    d.mediaTorrentProvider.mediaId = v
                                                                    return
                                                                })
                                                            }}
                                                            addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                        ...
                                                        {selectedFunction === Functions.MediaTorrentProviderSmartSearch && (
                                                            <>
                                                                <TextInput
                                                                    leftAddon="Query"
                                                                    type="text"
                                                                    value={inputs.mediaTorrentProvider.smartSearch.query}
                                                                    onChange={e => {
                                                                        setInputs(d => {
                                                                            d.mediaTorrentProvider.smartSearch.query = e.target.value
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                                ...
                                                                <NumberInput
                                                                    ...
                                                                    value={inputs.mediaTorrentProvider.smartSearch.episodeNumber || 0}
                                                                    min={0}
                                                                    formatOptions={{ useGrouping: false }}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.mediaTorrentProvider.smartSearch.episodeNumber = v
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                                ...
                                                                <Select
                                                                    ...
                                                                    value={inputs.mediaTorrentProvider.smartSearch.resolution || "-"}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.mediaTorrentProvider.smartSearch.resolution = v === "-" ? "" : v
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                                ...
                                                                <Switch
                                                                    side="right"
                                                                    label="Batch"
                                                                    value={inputs.mediaTorrentProvider.smartSearch.batch}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.mediaTorrentProvider.smartSearch.batch = v
                                                                            return
                                                                        })
                                                                    }}
                                                                />
                                                                ...
                                                                <Switch
                                                                    side="right"
                                                                    label="Best Releases"
                                                                    value={inputs.mediaTorrentProvider.smartSearch.bestReleases}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.mediaTorrentProvider.smartSearch.bestReleases = v
                                                                            return
                                                                        })
                                                                    }}
                                                                />
                                                        ...
                                                        {selectedFunction === Functions.MediaTorrentProviderSearch && (
                                                            <>
                                                                <TextInput
                                                                    leftAddon="Query"
                                                                    type="text"
                                                                    value={inputs.mediaTorrentProvider.search.query}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.mediaTorrentProvider.search.query = v
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                        ...
                                                        {selectedFunction === Functions.MediaTorrentProviderGetTorrentInfoHash && (
                                                            <>
                                                                <Textarea
                                                                    leftAddon="Torrent JSON"
                                                                    value={inputs.mediaTorrentProvider.getTorrentInfoHash.torrent}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.mediaTorrentProvider.getTorrentInfoHash.torrent = v
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                        ...
                                                        {selectedFunction === Functions.MediaTorrentProviderGetTorrentMagnetLink && (
                                                            <>
                                                                <Textarea
                                                                    label="Torrent JSON"
                                                                    value={inputs.mediaTorrentProvider.getTorrentMagnetLink.torrent}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.mediaTorrentProvider.getTorrentMagnetLink.torrent = v
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                ...
                                                {type === "manga-provider" && (
                                                    <>
                                                        ...
                                                        {selectedFunction === Functions.MangaProviderSearch && (
                                                            <>
                                                                <Alert intent="info">
                                                                    Seall will automatically select the best match based on the manga titles.
                                                                </Alert>
                                                            </>
                                                        )}
                                                    ...
                                                    <>
                                                        <Select
                                                            ...
                                                            options={[
                                                                ...
                                                                { value: Functions.OnlinestreamFindMediaItems, label: "findMediaItems" },
                                                                { value: Functions.OnlinestreamFindMediaItemServer, label: "findMediaItemServer" },
                                                            ]}
                                                            onValueChange={v => {
                                                                setSelectedFunction(v as Functions)
                                                            }}
                                                        ...
                                                        {selectedFunction === Functions.OnlinestreamSearch && (
                                                            <>
                                                                <Alert intent="info" className="text-sm">
                                                                    Seall will automatically select the best match based on the media titles.
                                                                </Alert>
                                                        ...
                                                        {selectedFunction === Functions.OnlinestreamFindMediaItems && (
                                                            <>
                                                                <TextInput
                                                                    leftAddon="Media ID"
                                                                    type="text"
                                                                    value={inputs.onlineStreamingProvider.findMediaItems.id}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.onlineStreamingProvider.findMediaItems.id = v
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                        ...
                                                        {selectedFunction === Functions.OnlinestreamFindMediaItemServer && (
                                                            <>
                                                                <Textarea
                                                                    leftAddon="Item JSON"
                                                                    value={inputs.onlineStreamingProvider.findMediaItemServer.item}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.onlineStreamingProvider.findMediaItemServer.item = v
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
                                                                ...
                                                                <TextInput
                                                                    leftAddon="Server"
                                                                    type="text"
                                                                    value={inputs.onlineStreamingProvider.findMediaItemServer.server}
                                                                    onValueChange={v => {
                                                                        setInputs(d => {
                                                                            d.onlineStreamingProvider.findMediaItemServer.server = v
                                                                            return
                                                                        })
                                                                    }}
                                                                    addonClass="w-[100px] border-r font-semibold text-sm justify-center text-center"
