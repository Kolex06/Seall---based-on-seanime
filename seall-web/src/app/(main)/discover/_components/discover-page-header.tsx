...
function BannerImage({ media, isTransitioning, shouldBlurBanner, showTrailer, trailerLoaded, onTrailerLoad, onTrailerError }: BannerImageProps) {
    ...
    const trailerId = (media as MediaAPI_BaseMedia)?.trailer?.id
    const trailerSite = (media as MediaAPI_BaseMedia)?.trailer?.site || "youtube"
    ...
    return (
        ...
        >
            <motion.div
                ...
                transition={{ duration: 0.5 }}
            >
                <SeaLink href={pageType === "manga" ? `/reading/entry?id=${media.id}` : `/entry?id=${media.id}`}>
            ...
            >
                ...
                <div className="flex items-center max-w-lg gap-4">
                    ...
                    {(media as MediaAPI_BaseMedia)?.nextAiringEpisode?.airingAt && (
                        <p className="text-base text-brand-200 inline-flex items-center gap-1.5">
                            <RiSignalTowerLine /> Releasing now
                        </p>
                    )}

                    {((media as MediaAPI_BaseMedia)?.nextAiringEpisode || (media as MediaAPI_BaseMedia).episodes) && (media as MediaAPI_BaseMedia)?.format !== "MOVIE" && (
                        <p className="text-base font-medium">
                            {(media as MediaAPI_BaseMedia).nextAiringEpisode?.episode ? (
                                <span>
                                    {(media as MediaAPI_BaseMedia).nextAiringEpisode?.episode! - 1} episode{(media as MediaAPI_BaseMedia).nextAiringEpisode?.episode! - 1 === 1
                                        ? ""
                                        : "s"} released
                                </span>
                            ) : (
                                <span>
                                    {(media as MediaAPI_BaseMedia).episodes} total episode{(media as MediaAPI_BaseMedia).episodes === 1 ? "" : "s"}
                                </span>
                            )}
                        </p>
                    )}
...
export function DiscoverPageHeader({ playTrailer }: { playTrailer?: boolean }) {
    ...
    React.useEffect(() => {
        ...
        if ((isHoveringHeader || playTrailer) && randomTrending && !isTransitioning && (randomTrending as MediaAPI_BaseMedia)?.trailer?.id) {
    ...
    React.useEffect(() => {
        if (trailerLoaded && !trailerPlaying && !isHoveringHeader && (randomTrending as MediaAPI_BaseMedia)?.trailer?.id) {
