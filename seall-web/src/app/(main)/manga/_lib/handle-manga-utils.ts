...
export function useMangaReaderUtils() {
    ...
    React.useLayoutEffect(() => {
        (async () => {
            ...
            setLocalPageToken(await getHMACTokenQueryParam("/api/v1/reading/local-page", "?"))
        })()
    }, [password])
    ...
    const getChapterPageUrl = React.useCallback((url: string, isDownloaded: boolean | undefined, headers?: Record<string, string>) => {
        if (url.startsWith("{{manga-local-assets}}")) {
            return `${getServerBaseUrl()}/api/v1/reading/local-page/${encodeURIComponent(url)}${localPageToken}`
        }
        ...
        return `${getServerBaseUrl()}/reading-downloads/${url}`
