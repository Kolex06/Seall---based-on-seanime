...
export function OnlinestreamManualMappingModal(props: OnlinestreamManualMappingModalProps) {
    ...
    return (
        <>
            <Modal
                title="Manual match"
                description="Match this media to a search result from the provider."
                trigger={children}
...
function Content({ entry }: { entry: Media_Entry }) {
    ...
    const [providerMediaId, setProviderMediaId] = React.useState<string | null>(null)
    const confirmMatch = useConfirmationDialog({
        title: "Manual match",
        description: "Are you sure you want to match this media to the search result?",
        actionText: "Confirm",
        actionIntent: "success",
        onConfirm: () => {
            if (providerMediaId && selectedProvider) {
                match({
                    provider: selectedProvider,
                    mediaId: entry.mediaId,
                    providerMediaId,
            ...
            {mappingLoading ? (
                ...
                <AppLayoutStack>
                    <div className="text-center">
                        {!!existingMapping?.providerMediaId ? (
                            <AppLayoutStack>
                                <p>
                                    Current mapping: <span>{existingMapping.providerMediaId}</span>
                                </p>
                                <Button
