...
function getOnlineStreamMediaType(media: Media_Entry["media"] | null | undefined) {
    const siteUrl = media?.siteUrl?.toLowerCase() ?? ""
    if (siteUrl.includes("simkl.com/movie/")) return "movies"
    if (siteUrl.includes("simkl.com/tv/")) return "shows"
    if (siteUrl.includes("simkl.com/anime/")) return "anime"
    if (media?.format === "MOVIE" && !media?.idMal) return "movies"
    return "anime"
}

function getOnlineStreamMediaTypeLabel(mediaType: string) {
    if (mediaType === "movies") return "movies"
    if (mediaType === "shows") return "shows"
    return "media"
}

export function OnlinestreamPage({ entry, entryLoading, hideBackButton }: OnlinestreamPageProps) {
    ...
    const mediaType = React.useMemo(() => getOnlineStreamMediaType(media), [media?.siteUrl, media?.format, media?.idMal])
    ...
    // get extensions
    const { providerExtensions, allProviderExtensions, providerExtensionOptions } = useHandleOnlinestreamProviderExtensions(mediaType)
    const extension = React.useMemo(() => providerExtensions.find(p => p.id === provider), [providerExtensions, provider])
    const activeProvider = extension ? provider : null
            ...
            if (videoSource?.url) {
                ...
                React.startTransition(() => {
                    (async () => {
                        ...
                        React.startTransition(() => {
                            log.info("Setting stream URL", { url: _url, quality, server, dubbed, provider: activeProvider })
                            setUrl(_url)
                        })
                    })()
                })
    ...
    }, [videoSource, server, quality, dubbed, activeProvider])
        ...
        <>
            ...
            <VideoCoreInlineLayout
                ...
                leftHeaderActions={<>
                    ...
                    {(entry && !!activeProvider) && <OnlinestreamManualMappingModal entry={entry}>
                        <Button
                            size="sm"
                            intent="gray-basic"
                            className="rounded-full"
                ...
                mediaPlayer={!provider ? (
                    <div className="flex items-center flex-col justify-center w-full h-full">
                        <LuffyError title={allProviderExtensions.length ? `No provider for ${getOnlineStreamMediaTypeLabel(mediaType)}` : "No provider selected"}>
                            {allProviderExtensions.length ? "Your installed streaming providers do not support this type yet." : undefined}
                        </LuffyError>
                        <div className="flex gap-2">
                            {parameters}
                        </div>
                    </div>
                ) : isEpisodeListError ? <LuffyError title="Provider error">
                    {(errorEpisodeList as AxiosError<{ error: string }>)?.response?.data?.error || "Could not fetch stream list from provider."}
                </LuffyError> : (
                    <>
