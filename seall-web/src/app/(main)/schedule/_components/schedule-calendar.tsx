import { MediaAPI_MediaListStatus, Media_ScheduleItem } from "@/api/generated/types"
...
import { __simkl_userAnimeListDataAtom } from "../../_atoms/media.atoms"

type CalendarParams = {
    indicateWatchedEpisodes: boolean
    source?: ScheduleSource
    listStatuses: MediaAPI_MediaListStatus[]
    mediaTypes?: string[]
}

type ScheduleSource = "list" | "all"

const MAX_EVENT_COUNT = 4
const DEFAULT_CALENDAR_SOURCE: ScheduleSource = "list"
const DEFAULT_CALENDAR_MEDIA_TYPES = ["movies", "shows", "anime"]
...
export const calendarParamsAtom = atomWithStorage<CalendarParams>("sea-release-calendar-params", {
    indicateWatchedEpisodes: true,
    source: DEFAULT_CALENDAR_SOURCE,
    listStatuses: ["PLANNING", "CURRENT", "COMPLETED", "PAUSED"] as MediaAPI_MediaListStatus[],
    mediaTypes: DEFAULT_CALENDAR_MEDIA_TYPES,
})

type ScheduleCalendarProps = {
    children?: React.ReactNode
    items?: Media_ScheduleItem[]
}

type ScheduleItem = Media_ScheduleItem & {
    mediaType?: "movies" | "shows" | "anime" | string
...
export function ScheduleCalendar(props: ScheduleCalendarProps) {
    ...
    const simklListData = useAtomValue(__simkl_userAnimeListDataAtom)

    const [calendarParams, setCalendarParams] = useImmerAtom(calendarParamsAtom)
    const scheduleSource = calendarParams.source ?? DEFAULT_CALENDAR_SOURCE

    const { data: _schedule, isLoading: scheduleLoading } = useGetMediaCollectionSchedule({ enabled: !items, source: scheduleSource })
    const schedule = (items ?? _schedule) as ScheduleItem[] | undefined
    const isUserSchedule = !items
    const isListSchedule = isUserSchedule && scheduleSource === "list"
    ...
    function isStatusIncluded(item: ScheduleItem) {
        if (!isListSchedule) return true
        const status = item.listStatus ?? simklListData[String(item.mediaId)]?.status
        if (!status) return true
        return calendarParams.listStatuses.includes(status)
    }

    function isMediaTypeIncluded(item: ScheduleItem) {
        const mediaType = item.mediaType || (item.isMovie ? "movies" : "shows")
        const selectedMediaTypes = calendarParams.mediaTypes ?? DEFAULT_CALENDAR_MEDIA_TYPES
        return selectedMediaTypes.includes(mediaType)
    }

    function isEpisodeWatched(item: ScheduleItem) {
        const progress = item.listProgress ?? simklListData[String(item.mediaId)]?.progress
        if (!progress) return false
        return progress >= item.episodeNumber
    }
    ...
    const days = React.useMemo(() => {
        ...
        while (day <= endOfCalendar) {
            let events = schedule?.filter(item => isSameLocalDay(new Date(item.dateTime!), day) && isStatusIncluded(item) && isMediaTypeIncluded(item))?.map(item => {
                return {
                    ...
                    isWatched: isEpisodeWatched(item),
                }
            }) ?? []
            events = sortBy(events, (e) => mediaTypePreviewPriority(e.mediaType), (e) => e.datetime, (e) => e.episode)
    ...
    const hasVisibleEvents = React.useMemo(() => {
        return days.some(day => day.isCurrentMonth && day.events.length > 0)
    }, [days])
            ...
            <div className="flex h-full flex-col rounded-xl border" data-schedule-calendar>
                ...
                >
                    ...
                    >
                        ...
                        <Separator />
                        <RadioGroup
                            label="Schedule source" options={[
                            { label: "My list", value: "list" },
                            { label: "All SIMKL", value: "all" },
                        ]} value={scheduleSource} onValueChange={v => setCalendarParams(draft => {
                            draft.source = v as ScheduleSource
                            return
                        })}
                            data-schedule-calendar-header-settings-popover-source
                        />
                        <Separator />
                        <CheckboxGroup
                            label="Media type" options={[
                            { label: "Movies", value: "movies" },
                            { label: "Shows", value: "shows" },
                            { label: "Anime", value: "anime" },
                        ]} value={calendarParams.mediaTypes ?? DEFAULT_CALENDAR_MEDIA_TYPES} onValueChange={v => setCalendarParams(draft => {
                            draft.mediaTypes = v.length ? v : DEFAULT_CALENDAR_MEDIA_TYPES
                            return
                        })}
                            stackClass="grid grid-cols-3 gap-0 items-center !space-y-0"
                            data-schedule-calendar-header-settings-popover-media-type
                        />
                        <Separator />
                        {isListSchedule && <>
                            <CheckboxGroup
                                label="Your list status" options={[
                                { label: "Watching", value: "CURRENT" },
                                { label: "Planning", value: "PLANNING" },
                                { label: "Completed", value: "COMPLETED" },
                                { label: "Paused", value: "PAUSED" },
                            ]} value={calendarParams.listStatuses} onValueChange={v => setCalendarParams(draft => {
                                draft.listStatuses = v as MediaAPI_MediaListStatus[]
                                return
                            })}
                ...
                >
                    ...
                    <div className="lg:hidden flex-auto bg-[--background] overflow-y-auto" data-schedule-calendar-mobile-list>
                        <MobileCalendarList days={days} isLoading={scheduleLoading} />
                    </div>

                    <div className="hidden lg:flex bg-[--background] text-xs leading-6 text-gray-200 flex-auto" data-schedule-calendar-desktop>
                        <div className="relative w-full grid grid-cols-7 grid-rows-6 gap-2 p-2" data-schedule-calendar-desktop-grid>
                            {days.map((day, index) => (
                                <CalendarDay
                                    key={index}
                                    day={day}
                            ...
                            {!scheduleLoading && !hasVisibleEvents && (
                                <div
                                    className="pointer-events-none absolute inset-0 z-[2] flex items-center justify-center text-sm text-[--muted]"
                                    data-schedule-calendar-desktop-empty
                                >
                                    <div className="rounded-md border bg-[--background]/90 px-4 py-2 shadow-sm">
                                        No scheduled releases for this month
                                    </div>
                                </div>
                            )}
                        </div>
...
function releaseLabel(event: CalendarEvent, compact = false) {
    if (event.isMovie) return "Movie"
    const prefix = event.mediaType === "shows" ? "Show" : event.mediaType === "anime" ? "Anime" : "Episode"
    return compact ? `${prefix} ${event.episode}` : `${prefix} Episode ${event.episode}`
}

function mediaTypePreviewPriority(mediaType?: string) {
    switch (mediaType) {
    case "movies":
        return 0
    case "shows":
...
function MobileCalendarList({ days, isLoading }: MobileCalendarListProps) {
    ...
    if (relevantDays.length === 0) {
        return (
            <div className="p-6 text-center text-[--muted]">
                <p>{isLoading ? "Loading releases..." : "No scheduled releases for this month"}</p>
            </div>
        )
    }
        ...
        <SeaLink href={event.href} className="block" data-schedule-calendar-mobile-list-day-item-event-link>
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    >
                        <span className="font-medium" data-schedule-calendar-mobile-list-day-item-event-episode>
                            {releaseLabel(event)}
                        </span>
    ...
    return (
        <Modal
            ...
            description={hasEvents
                ? `${day.events.length} scheduled release${day.events.length !== 1 ? "s" : ""}`
        ...
        >
            ...
            >
                {hasEvents ? (
                    ...
                    >
                        No releases are scheduled for this day.
                    </div>
                )}
            </div>
        </Modal>
                        ...
                        >
                            <p className="text-xs font-medium text-gray-100 line-clamp-2 leading-tight" data-schedule-calendar-day-hovered-event-text>
                                ...
                                {hoveredEvent.isSeasonFinale &&
                                    ...
                                    <span className="ml-1" data-schedule-calendar-day-hovered-event-text-episode>
                                    {releaseLabel(hoveredEvent, true)}
                                </span>
                                {hoveredEvent.time &&
