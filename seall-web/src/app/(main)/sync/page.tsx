import { MediaAPI_BaseMedia, MediaAPI_BaseManga, Local_QueueState } from "@/api/generated/types"
import {
    useLocalGetHasLocalChanges,
    useLocalGetLocalStorageSize,
    useLocalGetTrackedMediaItems,
    useLocalSetHasLocalChanges,
    useLocalSyncMediaData,
    useLocalSyncData,
    useSetOfflineMode,
...
import { simkl_getListDataFromEntry } from "@/lib/helpers/media"
...
export default function Page() {
    ...
    const { mutate: syncSimkl, isPending: isSyncingSimkl } = useLocalSyncMediaData()
    ...
    const unsavedAnime = React.useMemo(() => {
        ...
        let unsavedAnime: MediaAPI_BaseMedia[] = []
    ...
    const unsavedManga = React.useMemo(() => {
        ...
        let unsavedManga: MediaAPI_BaseManga[] = []
        for (const entry of currentList?.entries ?? []) {
            if (!trackedIds.has(entry.mediaId)) {
                ...
                <div className="contents">
                    ...
                    >
                        <div className="space-y-4">
                            ...
                            <p className="text-sm">
                                Update your local snapshots with the data from SIMKL.
                            ...
                            <Button
                                ...
                                loading={isSyncingSimkl}
                                onClick={handleSyncMediaApi}
                            >
                                Upload local changes to SIMKL
                            </Button>
                            <p className="text-sm">
                                Update your SIMKL lists with the data from your local snapshots.
            ...
            {hasLocalChanges && <>
                <Alert
                    ...
                    description={<div className="space-y-2">
                        <p>
                            <span>You have local changes that have not been synced to SIMKL.</span>
                ...
                <MediaCardLazyGrid itemCount={trackedAnimeItems?.length}>
                    {trackedAnimeItems?.map((item) => (
                        <MediaEntryCard
                            ...
                            listData={simkl_getListDataFromEntry(item.animeEntry!)}
                ...
                <MediaCardLazyGrid itemCount={trackedMangaItems?.length}>
                    {trackedMangaItems?.map((item) => (
                        <MediaEntryCard
                            ...
                            listData={simkl_getListDataFromEntry(item.mangaEntry!)}
