import {
    MediaAPI_BaseMedia,
    Media_AutoDownloaderRule,
    Media_AutoDownloaderRuleEpisodeType,
    Media_AutoDownloaderRuleTitleComparisonType,
    Media_LibraryCollection,
} from "@/api/generated/types"
import { useCreateAutoDownloaderRule } from "@/api/hooks/auto_downloader.hooks"
import { __simkl_userAnimeListDataAtom } from "@/app/(main)/_atoms/media.atoms"
import { useSimklUserAnime } from "@/app/(main)/_hooks/media-collection-loader"
...
type AutoDownloaderBatchRuleFormProps = {
    onRuleCreated: () => void
    rules: Media_AutoDownloaderRule[]
}
...
export function AutoDownloaderBatchRuleForm(props: AutoDownloaderBatchRuleFormProps) {
    ...
    const userMedia = useSimklUserAnime()
    ...
    function handleSave(data: InferType<typeof schema>) {
        for (const entry of data.entries) {
            ...
            createRule({
                rule: {
                    dbId: 0,
                    titleComparisonType: data.titleComparisonType as Media_AutoDownloaderRuleTitleComparisonType,
                    episodeType: "recent" as Media_AutoDownloaderRuleEpisodeType,
...
type RuleFormFieldsProps = {
    ...
    allMedia: MediaAPI_BaseMedia[]
    isPending: boolean
    mediaList: MediaAPI_BaseMedia[]
    libraryCollection?: Media_LibraryCollection | undefined
    rules: Media_AutoDownloaderRule[]
}
...
export function MediaArrayField(props: MediaArrayFieldProps) {
    ...
    function handleAddCurrentlyWatching() {
        ...
        const currentlyWatchingMedia = userMedia?.filter(media => {
            const listData = simklListData[String(media.id)]
