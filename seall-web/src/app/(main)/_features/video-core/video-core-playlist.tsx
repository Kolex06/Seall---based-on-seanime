import { Media_Entry, Media_Episode } from "@/api/generated/types"
import { useGetMediaEpisodeCollection } from "@/api/hooks/media_playback.hooks"
...
export type VideoCorePlaylistState = {
    ...
    episodes: Media_Episode[]
    previousEpisode: Media_Episode | null
    nextEpisode: Media_Episode | null
    currentEpisode: Media_Episode
    animeEntry: Media_Entry | null
...
export function useVideoCorePlaylistSetup(providedState: VideoCoreLifecycleState,
    ...
    const { data: animeEntry } = useGetMediaEntry(mediaId)
    const { data: episodeCollection, isLoading, refetch } = useGetMediaEpisodeCollection(mediaId)
...
export function useVideoCorePlaylist() {
    ...
    function startStream(episode: Media_Episode) {
    ...
    const playEpisode = (which: "previous" | "next" | string) => {
        ...
        let episode: Media_Episode | null = null
        switch (which) {
            case "previous":
...
function PlaylistEpisodeHoverCard({ episode, children }: { episode?: Media_Episode, children: React.ReactNode }) {
    return (
        <HoverCard
...
export function VideoCorePlaylistControl() {
    ...
    return (
        <>
            ...
            />}
            {animeEntry && <TorrentSearchDrawer entry={animeEntry as Media_Entry} />}
        </>
    )
}
