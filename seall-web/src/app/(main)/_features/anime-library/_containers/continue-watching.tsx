import { Media_Episode, Continuity_WatchHistory } from "@/api/generated/types"
...
import { simkl_animeIsMovie } from "@/lib/helpers/media"
...
export const __libraryHeaderEpisodeAtom = atom<Media_Episode | null>(null)
...
export function ContinueWatching({ episodes, isLoading, linkTemplate, withTitle }: {
    ...
    const prevSelectedEpisodeRef = React.useRef<Media_Episode | null>(null)
                ...
                render: () => (
                    <>
                        ...
                        <div className="flex gap-1 items-center w-full">
                            ...
                            {!simkl_animeIsMovie(episode.baseAnime) ? <>
                ...
                if (!overrideLink) {
                    setPlayNext(episode.baseAnime?.id, () => {
                        if (!serverStatus?.isOffline) {
                            ...
                            router.push(`/offline/entry/media?id=${episode.baseAnime?.id}`)
                        }
                    })
                } else {
                    setPlayNext(episode.baseAnime?.id, () => {
