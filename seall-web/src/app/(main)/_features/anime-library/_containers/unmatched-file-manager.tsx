import { Media_UnmatchedGroup } from "@/api/generated/types"
import { useMediaEntryManualMatch, useFetchMediaEntrySuggestions } from "@/api/hooks/media_entries.hooks"
...
type UnmatchedFileManagerProps = {
    unmatchedGroups: Media_UnmatchedGroup[]
}
...
export function UnmatchedFileManager(props: UnmatchedFileManagerProps) {
    ...
    const [simklId, setMediaId] = React.useState(0)
    ...
    const { mutate: manualMatch, isPending: isMatching } = useMediaEntryManualMatch()
    ...
    /**
     * Manually match the current group with the specified Simkl ID.
    ...
    function handleMatchSelected() {
        if (!!currentGroup && simklId > 0 && selectedPaths.length > 0) {
            manualMatch({
                paths: selectedPaths,
                mediaId: simklId,
            }, {
            ...
            <AppLayoutStack className="mt-4">
                ...
                <div className="flex items-center flex-wrap gap-2">
                    <div className="flex gap-2 items-center w-full">
                        <p className="flex-none text-lg mr-2 font-semibold">{!hasCustomSources ? "SIMKL" : "Media"} ID</p>
                        <NumberInput
                            // key={String(_r) + (currentGroup?.dir || "")}
                            value={simklId}
                            onValueChange={v => setMediaId(v)}
                            formatOptions={{
                                useGrouping: false,
                            }}
                        />
                ...
                <div className="flex flex-wrap items-center gap-1">
                    ...
                    {/*<SeaLink*/}
                    {/*    target="_blank"*/}
                    {/*    href={`https://simkl.com/search/?q=${encodeURIComponent(currentGroup?.localFiles?.[0]?.parsedInfo?.title || currentGroup?.localFiles?.[0]?.parsedFolderInfo?.[0]?.title || "")}`}*/}
                    {/*>*/}
                    <Button
                        intent="white-subtle"
                            ...
                            label: <div>
                                ...
                                <div className="mt-2 flex w-full gap-4">
                                    ...
                                    <div className="text-[--muted]">
                                        ...
                                        <SeaLink
                                            href={media.siteUrl || `https://simkl.com/search/?q=${encodeURIComponent(media.title?.userPreferred || media.title?.english || media.title?.romaji || "")}`}
                                            target="_blank"
                                        >
                                            <Button
                                                intent="primary-link"
