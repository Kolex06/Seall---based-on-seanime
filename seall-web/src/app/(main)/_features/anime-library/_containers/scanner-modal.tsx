...
export function ScannerModal() {
    ...
    const [userMedia] = useAtom(__media_userMediaAtom)
    const simklDataOnly = useBoolean(true)
    ...
    React.useEffect(() => {
        if (!userMedia?.length) simklDataOnly.off()
        else simklDataOnly.on()
    }, [userMedia])
        ...
        <>
            ...
            >
                ...
                {serverStatus?.user?.isSimulated && <div className="border border-dashed rounded-md py-2 px-4 !mt-5">
                    Using this feature without an SIMKL account is not recommended if you have a large library, as it may lead to rate limits and
                ...
                <div className="space-y-4" data-scanner-modal-content>
                    ...
                    <AppLayoutStack className="space-y-2">
                        ...
                        <AppLayoutStack className="space-y-2">
                            ...
                            <Switch
                                side="right"
                                label="My SIMKL Collection only"
                                moreHelp="This is faster but generally less accurate if your collection does not contain all anime in the library."
                                help={simklDataOnly.active ? "Matches local files against your SIMKL collection." : ""}
                                value={simklDataOnly.active}
                                onValueChange={v => simklDataOnly.set(v as boolean)}
                                // className="data-[state=checked]:bg-amber-700 dark:data-[state=checked]:bg-amber-700"
                                // size="lg"

                                disabled={!userMedia?.length}
                            />
                            {!simklDataOnly.active && <RadioGroup
                                label="Enhanced matching method"
                                options={[
                                    { value: "database", label: "Use Anime Offline Database" },
                                    { value: "simkl", label: "Use SIMKL API" },
                                ]}
                                size="lg"
                                stackClass="space-y-2 py-1"
                                value={enhanceWithOfflineDatabase.active ? "database" : "simkl"}
                                onValueChange={v => enhanceWithOfflineDatabase.set(v === "database")}
                                help={enhanceWithOfflineDatabase.active
                                    ? <span>Matches local files against the entire SIMKL catalog. Scanning will be slower.</span>
                                    : <span><span className="text-[--orange]">Slower for large libraries</span>. Seall will send an API request for
