import { Media_LibraryCollectionEntry, Media_LibraryCollectionList } from "@/api/generated/types"
...
import { CollectionParams, DEFAULT_MEDIA_COLLECTION_PARAMS, filterAnimeCollectionEntries, filterEntriesByTitle } from "@/lib/helpers/filtering"
...
export const DETAILED_LIBRARY_DEFAULT_PARAMS: CollectionParams<"anime"> = {
    ...DEFAULT_MEDIA_COLLECTION_PARAMS,
    sorting: "TITLE",
}
...
export function useHandleDetailedLibraryCollection({ enabled = true }: { enabled?: boolean } = { enabled: true }) {
     ...
     */
    const _filteredCollection: Media_LibraryCollectionList[] = React.useMemo(() => {
    ...
    }, [animeTagMap, data, debouncedParams, serverStatus?.settings?.simkl?.enableAdultContent, watchHistory])

    const filteredCollection: Media_LibraryCollectionList[] = React.useMemo(() => {
    ...
    const _filteredEntries: Media_LibraryCollectionEntry[] = React.useMemo(() => {
        if (!data || !data.lists) return []
    ...
    const filteredEntries: Media_LibraryCollectionEntry[] = React.useMemo(() => {
    ...
    const continueWatchingList = React.useMemo(() => {
        ...
        if (!serverStatus?.settings?.simkl?.enableAdultContent || serverStatus?.settings?.simkl?.blurAdultContent) {
        ...
        data?.continueWatchingList,
        serverStatus?.settings?.simkl?.enableAdultContent,
        serverStatus?.settings?.simkl?.blurAdultContent,
    ])
