...
import { useCheckSimklPinLogin, useLogout, useSaveSimklClientConfig, useStartSimklPinLogin } from "@/api/hooks/auth.hooks"
...
import { SIMKL_DEVELOPER_URL, SIMKL_PIN_URL } from "@/lib/server/config"
...
import { LuCalendar, LuCompass, LuRefreshCw, LuRss, LuSettings } from "react-icons/lu"
...
function SidebarNavigation({ isCollapsed, containerRef }: { isCollapsed: boolean, containerRef: React.RefObject<HTMLDivElement | null> }) {
    ...
    const hasTorrentProvider = !!serverStatus?.settings?.library?.torrentProvider
    ...
    const { mutate: refreshAC, isPending: isRefreshingAC } = useRefreshMediaCollection()
    ...
    const items = React.useMemo(() => [
        ...
        ...(hasTorrentProvider && !!serverStatus?.settings?.library?.libraryPath) ? [{
            id: "auto-downloader",
            iconType: LuRss,
...
function SimklPinLogin() {
    const [pin, setPin] = React.useState<{ user_code?: string, verification_url?: string, interval?: number } | null>(null)
    const [message, setMessage] = React.useState("")
    const serverStatus = useServerStatus()
    const hasSimklClientId = !!serverStatus?.simklClientId?.trim()

    const { mutate: startPinLogin, isPending: isStarting } = useStartSimklPinLogin()
    const { mutate: checkPinLogin, isPending: isChecking } = useCheckSimklPinLogin()
    const { mutate: saveClientConfig, isPending: isSavingClientConfig } = useSaveSimklClientConfig()

    React.useEffect(() => {
        if (!pin?.user_code) return
        const id = window.setInterval(() => {
            checkPinLogin({ userCode: pin.user_code! }, {
                onSuccess: data => {
        ...
        }, Math.max(pin.interval || 5, 5) * 1000)
        return () => window.clearInterval(id)
    }, [checkPinLogin, pin?.interval, pin?.user_code])

    return (
        <div className="space-y-3">
            {!hasSimklClientId && <div className="rounded-md border border-orange-500/40 bg-orange-950/20 p-3 text-left space-y-3">
                <div className="space-y-1">
                    <p className="font-medium">SIMKL client ID needed</p>
                    <p className="text-sm text-[--muted]">
                        SIMKL requires each app to have its own API key before login can start.
                    </p>
                    <a
                        className="text-sm text-[--brand] hover:underline"
                        href={SIMKL_DEVELOPER_URL}
                        target="_blank"
                ...
                <Form
                    schema={defineSchema(({ z }) => z.object({
                        clientId: z.string().min(1, "SIMKL client ID is required"),
                    }))}
                    onSubmit={data => {
                        saveClientConfig({ clientId: data.clientId.trim() })
                    }}
                >
                    <Field.Text
                        name="clientId"
                        label="SIMKL client ID"
                        placeholder="Paste your SIMKL API key"
                        fieldClass="px-0"
                    />
                    <Field.Submit loading={isSavingClientConfig}>Save client ID</Field.Submit>
                </Form>
            </div>}
            ...
            <Button
                ...
                size="md"
                loading={isStarting}
                disabled={!hasSimklClientId}
                onClick={() => {
                    startPinLogin(undefined, {
                        onSuccess: data => {
                            setPin(data || null)
                            setMessage("")
                            const url = data?.verification_url || SIMKL_PIN_URL
            ...
            {!!pin?.user_code && <div className="rounded-md border p-3 text-sm space-y-2">
                <p className="text-[--muted]">Enter this code on SIMKL:</p>
                <p className="text-2xl font-semibold tracking-widest">{pin.user_code}</p>
                <Button
                    intent="gray-subtle"
                    size="sm"
                    loading={isChecking}
                    onClick={() => checkPinLogin({ userCode: pin.user_code! })}
                >
                    Check approval
                </Button>
            ...
            {!!user && <div className="flex w-full gap-2 flex-col">
                ...
                >
                    {!user.isSimulated ? <DropdownMenuItem onClick={confirmSignOut.open}>
                        ...
                        <BiLogIn /> Log in with SIMKL
                    </DropdownMenuItem>}
                </DropdownMenu>
            </div>}

            <Modal
                title="Log in with SIMKL"
                description="Use SIMKL to sync your movies, shows, and watchlist."
            ...
            >
                <div className="mt-5 text-center space-y-4">
                    ...
                    >
                        <Field.Textarea
                            name="token"
                            label="Enter a SIMKL access token"
                            fieldClass="px-4"
                        />
