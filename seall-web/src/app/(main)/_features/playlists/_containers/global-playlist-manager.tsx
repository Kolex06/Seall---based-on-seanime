import { Media_Entry, Media_Playlist, Media_PlaylistEpisode, HibikeTorrent_MediaTorrent } from "@/api/generated/types"
...
const pm_currentPlaylist = atom<Media_Playlist | null>(null)
const pm_currentPlaylistEpisode = atom<Media_PlaylistEpisode | null>(null)
...
export function usePlaylistManager() {
    ...
    function startPlaylist(playlist: Media_Playlist) {
...
export function GlobalPlaylistManager() {
    ...
    const { data: animeEntry } = useGetMediaEntry(torrentSearchStreamEpisode?.baseAnime?.id)

    // The torrent to continue playing from
    const { autoPlayTorrent } = useAutoPlaySelectedTorrent()

    function sameTorrent(autoPlayTorrent: { entry: Media_Entry, torrent: HibikeTorrent_MediaTorrent } | null, episode: Media_PlaylistEpisode) {
        ...
        onMessage: (data: { type: ServerEvents, payload: unknown }) => {
            switch (data.type) {
                case "current-playlist":
                    let payload = data.payload as { playlist: Media_Playlist | null, playlistEpisode: Media_PlaylistEpisode | null }
                ...
                case "play-episode":
                    ...
                    const payload2 = data.payload as { playlistEpisode: Media_PlaylistEpisode }
...
function EpisodeItem({ episode }: { episode: Media_PlaylistEpisode }) {
