...
interface RefreshSimklButtonProps {
    children?: React.ReactNode
}

export const RefreshSimklButton: React.FC<RefreshSimklButtonProps> = (props) => {

    const { children, ...rest } = props

    /**
     * @description
     * - Asks the server to fetch an up-to-date version of the user's SIMKL collection.
     */
    const { mutate, isPending } = useRefreshMediaCollection()

    return (
        <>
            <Tooltip
                data-refresh-simkl-button-tooltip
                trigger={
                    <Button
                        data-refresh-simkl-button
                        onClick={() => mutate()}
