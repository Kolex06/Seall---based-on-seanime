import { Media_LibraryCollectionList, Media_LocalFile, Media_UnknownGroup } from "@/api/generated/types"
import { useOpenInExplorer } from "@/api/hooks/explorer.hooks"
import { useGetAllExtensions } from "@/api/hooks/extensions.hooks"
import { __bulkAction_modalAtomIsOpen } from "@/app/(main)/_features/anime-library/_containers/bulk-action-modal"
import { __ignoredFileManagerIsOpen } from "@/app/(main)/_features/anime-library/_containers/ignored-file-manager"
...
import { HomeSettingsButton } from "@/app/(main)/_features/home/home-settings-button"
...
import { DEFAULT_TORRENT_PROVIDER, TORRENT_PROVIDER } from "@/lib/server/settings"
...
export type HomeToolbarProps = {
    collectionList: Media_LibraryCollectionList[]
    ignoredLocalFiles: Media_LocalFile[]
    unmatchedLocalFiles: Media_LocalFile[]
    unknownGroups: Media_UnknownGroup[]
    isLoading: boolean
    hasEntries: boolean
...
export function HomeToolbar(props: HomeToolbarProps) {
    ...
    const selectedTorrentProvider = serverStatus?.settings?.library?.torrentProvider ?? DEFAULT_TORRENT_PROVIDER
    const hasSelectedTorrentProvider = !!selectedTorrentProvider && selectedTorrentProvider !== TORRENT_PROVIDER.NONE
    const hasTorrentProviderExtension = allExtensions?.extensions?.some(n => n.type === "media-torrent-provider") ?? false
    ...
    return (
        <>
            <div className={cn("flex flex-wrap w-full justify-end gap-1 p-4 relative z-[10]", className)} data-home-toolbar-container>
                ...
                {(!isExtensionsLoading && !hasTorrentProviderExtension && hasSelectedTorrentProvider && hasLibraryPath) &&
                    <SeaLink href="/extensions?tab=marketplace&type=media-torrent-provider">
                        <span>
                            <Tooltip
                                trigger={<Button
                ...
                {(hasEntries) && (
                    <>
                        {(!isStreamingOnly && !isNakamaLibrary) && <Tooltip
                            ...
                            />}
                        >
                            {homeView === "base" ? "Local Media Library" : "Home"}
                        </Tooltip>}
                ...
                >
                    {hasEntries ? "Refresh Media Library" : "Scan Media Library"}
                </Tooltip>}
