import { MediaAPI_ListRecentMedia_Page_AiringSchedules, Media_ScheduleItem, Models_HomeItem } from "@/api/generated/types"
import { useListMedia, useListRecentAiringMedia } from "@/api/hooks/simkl.hooks"
...
import { __media_userMediaAtom } from "@/app/(main)/_atoms/media.atoms"
import { LibraryHeader } from "@/app/(main)/_features/anime-library/_components/library-header"
import { BulkActionModal } from "@/app/(main)/_features/anime-library/_containers/bulk-action-modal"
import { ContinueWatching } from "@/app/(main)/_features/anime-library/_containers/continue-watching"
import { ContinueWatchingHeader } from "@/app/(main)/_features/anime-library/_containers/continue-watching-header"
import { CustomLibraryBanner } from "@/app/(main)/_features/anime-library/_containers/custom-library-banner"
import { IgnoredFileManager } from "@/app/(main)/_features/anime-library/_containers/ignored-file-manager"
import { __scanner_modalIsOpen } from "@/app/(main)/_features/anime-library/_containers/scanner-modal"
import { UnknownMediaManager } from "@/app/(main)/_features/anime-library/_containers/unknown-media-manager"
import { UnmatchedFileManager } from "@/app/(main)/_features/anime-library/_containers/unmatched-file-manager"
import { HandleLibraryCollectionProps, useHandleLibraryCollection } from "@/app/(main)/_features/anime-library/_lib/handle-library-collection"
import { DetailedLibraryView } from "@/app/(main)/_features/anime-library/_screens/detailed-library-view"
import { LibraryView } from "@/app/(main)/_features/anime-library/_screens/library-view"
import { SimklMediaEntryList } from "@/app/(main)/_features/anime/_components/media-entry-list"
import { DEFAULT_HOME_ITEMS, HOME_ITEMS, isAnimeLibraryItemsOnly } from "@/app/(main)/_features/home/home-items.utils"
import { __home_settingsModalOpen, HomeSettingsModal } from "@/app/(main)/_features/home/home-settings-modal"
import { HomeToolbar } from "@/app/(main)/_features/home/home-toolbar"
...
import { useHandleUserMediaLists } from "@/app/(main)/lists/_lib/handle-user-media-lists"
...
export function HomeScreen() {
    ...
    const allUserMedia = useAtomValue(__media_userMediaAtom)
    ...
    const homeItems = React.useMemo(() => {
        ...
        ret = ret.filter(item => !["manga-carousel", "manga-library", "manga-continue-reading"].includes(item.type))
...
export function HomeScreenItem(props: HomeScreenItemProps) {
    ...
    if (item.type === "anime-schedule-calendar") {
        return (
            <>
                {item.options?.type !== "global" && <MediaScheduleCalendar libraryCollectionProps={props.libraryCollectionProps} item={item} />}
                {item.options?.type === "global" && <GlobalMediaScheduleCalendar libraryCollectionProps={props.libraryCollectionProps} item={item} />}
            </>
        )
    }
...
function MediaScheduleCalendar(props: { libraryCollectionProps: HandleLibraryCollectionProps, item: Models_HomeItem }) {
    const { libraryCollectionProps, item } = props
    const {} = libraryCollectionProps
...
function GlobalMediaScheduleCalendar(props: { libraryCollectionProps: HandleLibraryCollectionProps, item: Models_HomeItem }) {
    const { libraryCollectionProps, item } = props
    const {} = libraryCollectionProps
    ...
    const { data: previous1, isLoading: loadingPrevious1 } = useListRecentAiringMedia({
        page: 1,
        perPage: 100,
    ...
    const { data: previous2, isLoading: loadingPrevious2 } = useListRecentAiringMedia({
        page: 2,
        perPage: 100,
        airingAt_lesser: Math.floor(weekBefore.getTime() / 1000),
        airingAt_greater: Math.floor(twoWeeksBefore.getTime() / 1000),
    ...
    const { data: previous3, isLoading: loadingPrevious3 } = useListRecentAiringMedia({
        page: 1,
        perPage: 200,
    ...
    const { data: previous4, isLoading: loadingPrevious4 } = useListRecentAiringMedia({
        page: 2,
        perPage: 200,
        airingAt_lesser: Math.floor(now.getTime() / 1000),
        airingAt_greater: Math.floor(weekBefore.getTime() / 1000),
    ...
    const { data: next1, isLoading: loadingNext1 } = useListRecentAiringMedia({
        page: 1,
        perPage: 100,
    ...
    const { data: next2, isLoading: loadingNext2 } = useListRecentAiringMedia({
        page: 2,
        perPage: 100,
        airingAt_lesser: Math.floor(weekAfter.getTime() / 1000),
        airingAt_greater: Math.floor(now.getTime() / 1000),
    ...
    const { data: next3, isLoading: loadingNext3 } = useListRecentAiringMedia({
        page: 1,
        perPage: 100,
    ...
    const { data: next4, isLoading: loadingNext4 } = useListRecentAiringMedia({
        page: 2,
        perPage: 100,
        airingAt_lesser: Math.floor(twoWeeksAfter.getTime() / 1000),
        airingAt_greater: Math.floor(weekAfter.getTime() / 1000),
    ...
    const items = React.useMemo<Media_ScheduleItem[]>(() => {
        let airingSchedules: MediaAPI_ListRecentMedia_Page_AiringSchedules[] = []
...
function AnimeCarousel(props: { libraryCollectionProps: HandleLibraryCollectionProps, item: Models_HomeItem }) {
    ...
    return (
        <PageWrapper className="space-y-0 px-4" ref={ref}>
            <h2>{options?.name || "Media Carousel"}</h2>
...
function MyLists(props: { item: Models_HomeItem }) {
    ...
    const {
        ...
        droppedList,
        customLists,
    } = useHandleUserMediaLists("", item.options?.type)
    ...
    return (
        <PageWrapper className="space-y-6 px-4">
            {(!!currentList?.entries?.length && !isCustomList && (!item.options?.statuses?.length || item.options?.statuses?.includes("CURRENT"))) && <>
                ...
                <SimklMediaEntryList type={item.options?.type ?? "anime"} layout={item.options?.layout} list={currentList} />
            </>}
            {(!!repeatingList?.entries?.length && !isCustomList && (!item.options?.statuses?.length || item.options?.statuses?.includes("REPEATING"))) && <>
                ...
                <SimklMediaEntryList type={item.options?.type ?? "anime"} layout={item.options?.layout} list={repeatingList} />
            </>}
            {(!!planningList?.entries?.length && !isCustomList && (!item.options?.statuses?.length || item.options?.statuses?.includes("PLANNING"))) && <>
                ...
                <SimklMediaEntryList type={item.options?.type ?? "anime"} layout={item.options?.layout} list={planningList} />
            </>}
            {(!!pausedList?.entries?.length && !isCustomList && (!item.options?.statuses?.length || item.options?.statuses?.includes("PAUSED"))) && <>
                ...
                <SimklMediaEntryList type={item.options?.type ?? "anime"} layout={item.options?.layout} list={pausedList} />
            </>}
            {(!!completedList?.entries?.length && !isCustomList && (!item.options?.statuses?.length || item.options?.statuses?.includes("COMPLETED"))) && <>
                ...
                <SimklMediaEntryList type={item.options?.type ?? "anime"} layout={item.options?.layout} list={completedList} />
            </>}
            {(!!droppedList?.entries?.length && !isCustomList && (!item.options?.statuses?.length || item.options?.statuses?.includes("DROPPED"))) && <>
                ...
                <SimklMediaEntryList type={item.options?.type ?? "anime"} layout={item.options?.layout} list={droppedList} />
            </>}
            {customLists?.map(list => {
                ...
                >
                    <h2>{list.name}</h2>
                    <SimklMediaEntryList type={item.options?.type ?? "anime"} layout={item.options?.layout} list={list} />
                </div> : null
            })}
        </PageWrapper>
    )
