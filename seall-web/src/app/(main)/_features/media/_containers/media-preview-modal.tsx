import { MediaAPI_MediaDetailsById_Media, MediaAPI_BaseMedia, MediaAPI_MangaDetailsById_Media, Media_Entry, Manga_Entry, Nullish } from "@/api/generated/types"
import { useGetMediaDetails } from "@/api/hooks/simkl.hooks"
import { useGetMediaEntry } from "@/api/hooks/media_entries.hooks"
...
function Anime({ mediaId }: { mediaId: number }) {
    const { data: entry, isLoading: entryLoading } = useGetMediaEntry(mediaId)
    const { data: details, isLoading: detailsLoading } = useGetMediaDetails(mediaId)
...
function Content({ entry, entryLoading, detailsLoading, details, type }: {
    entry: Nullish<Media_Entry | Manga_Entry>,
    entryLoading: boolean,
    detailsLoading: boolean,
    details: Nullish<MediaAPI_MediaDetailsById_Media | MediaAPI_MangaDetailsById_Media>
    type: "anime" | "manga"
}) {
    ...
    const simklTitle = media?.title?.userPreferred || media?.title?.english || media?.title?.romaji || ""
    const simklUrl = media?.siteUrl || details?.siteUrl || (simklTitle ? `https://simkl.com/search/?q=${encodeURIComponent(simklTitle)}` : "https://simkl.com")
    ...
    return (
        <ScopeProvider atoms={[__torrentSearch_selectionAtom, __torrentSearch_selectionEpisodeAtom, __torrentSearch_selectedTorrentsAtom]}>
            ...
            {(!entryLoading && entry) && <>
                ...
                <div className="z-[5] relative">
                    <MediaPageHeaderEntryDetails
                        ...
                        progressTotal={(entry.media as MediaAPI_BaseMedia)?.episodes}
                    ...
                    >
                        ...
                        >
                            ...
                            {(details as MediaAPI_MediaDetailsById_Media)?.studios &&
                                <AnimeEntryStudio studios={(details as MediaAPI_MediaDetailsById_Media)?.studios} />}
                    ...
                    <div className="mt-6 flex gap-3 items-center">
                        ...
                        <SeaLink href={type === "anime" ? `/entry?id=${media?.id}` : `/reading/entry?id=${media?.id}`}>
                            <Button className="px-0" intent="gray-link">
                                Open page
                            </Button>
                        </SeaLink>

                        {type === "anime" && !!(entry?.media as MediaAPI_BaseMedia)?.trailer?.id && <TrailerModal
                            trailerId={(entry?.media as MediaAPI_BaseMedia)?.trailer?.id} trigger={
                            <Button intent="gray-link" className="px-0">
                                Trailer
                            </Button>}
                        />}

                        {!isCustomSource(media?.id) && <SeaLink href={simklUrl} target="_blank">
                            <IconButton intent="gray-link" className="px-0" icon={<SiSimkl className="text-lg" />} />
                        </SeaLink>}
                        ...
                        {(
                            ...
                            <TorrentSearchButton
                                entry={entry as Media_Entry}
                            />
                        )}
                    </div>

                    {detailsLoading ? <LoadingSpinner /> : <div className="space-y-6 pt-6">
                        {(type === "anime" && (entry as Media_Entry).libraryData) && <EpisodeSection
                            entry={entry as Media_Entry}
                            details={details as MediaAPI_MediaDetailsById_Media}
                            bottomSection={<></>}
                            hideCarousel
                            maxCol={2}
                        />}
                        {type === "anime" && <RelationsRecommendationsSection entry={entry as Media_Entry} details={details} maxCol={5} />}
            ...
            {(type === "anime" && !!entry) && <TorrentSearchDrawer entry={entry as Media_Entry} />}
        </ScopeProvider>
    )
}
