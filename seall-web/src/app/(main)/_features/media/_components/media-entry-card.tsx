import {
    MediaAPI_BaseMedia,
    MediaAPI_BaseManga,
    Media_EntryLibraryData,
    Media_EntryListData,
    Media_LibraryCollectionEntry,
    Media_NakamaEntryLibraryData,
    Manga_EntryListData,
...
import { MediaEntryEditModal } from "@/app/(main)/_features/media/_containers/media-entry-edit-modal"
import { useMediaPreviewModal } from "@/app/(main)/_features/media/_containers/media-preview-modal"
import { usePlaylistEditorManager } from "@/app/(main)/_features/playlists/lib/playlist-editor-manager"
import { useMediaUserListData } from "@/app/(main)/_hooks/media-collection-loader"
...
type MediaEntryCardListData = Media_EntryListData | Manga_EntryListData

type MediaEntryCardProps<T extends "anime" | "manga"> = {
    type: T
    media: T extends "anime" ? MediaAPI_BaseMedia : T extends "manga" ? MediaAPI_BaseManga : never
    // Anime-only
    listData?: T extends "anime" ? Media_EntryListData : T extends "manga" ? Manga_EntryListData : never
    showLibraryBadge?: T extends "anime" ? boolean : never
    showTrailer?: T extends "anime" ? boolean : never
    libraryData?: T extends "anime" ? Media_EntryLibraryData : never
    nakamaLibraryData?: T extends "anime" ? Media_NakamaEntryLibraryData : never
    hideUnseenCountBadge?: boolean
    hideSimklEntryEditButton?: boolean
    onClick?: () => void
...
export function MediaEntryCard<T extends "anime" | "manga">(props: MediaEntryCardProps<T>) {
    ...
    const {
        ...
        hideSimklEntryEditButton = false,
        onClick,
    ...
    const mediaEpisodes = (media as MediaAPI_BaseMedia)?.episodes
    const mediaChapters = (media as MediaAPI_BaseManga)?.chapters
    const mediaIsAdult = media?.isAdult
    const collectionEntry = useMediaCollectionEntry(type, mediaId)
    const animeListDataFromCollection = useMediaUserListData(mediaId, type === "anime" && !_listData)
    const animeCollectionEntry = type === "anime" ? collectionEntry as Media_LibraryCollectionEntry | undefined : undefined
    ...
    const MANGA_LINK = React.useMemo(() =>
            serverStatus?.isOffline ? `/offline/entry/reading?id=${mediaId}` : `/reading/entry?id=${mediaId}`,
        [serverStatus?.isOffline, mediaId],
    )
    const ANIME_LINK = React.useMemo(() =>
            serverStatus?.isOffline ? `/offline/entry/media?id=${mediaId}` : `/entry?id=${mediaId}`,
        [serverStatus?.isOffline, mediaId],
    )
    ...
    const progressTotal = type === "anime" ? (media as MediaAPI_BaseMedia)?.episodes : (media as MediaAPI_BaseManga)?.chapters
                    ...
                    >
                        ...
                        <MediaEntryCardHoverPopupBody>
                            ...
                            <MediaEntryCardHoverPopupBanner
                                ...
                                blurAdultContent={serverStatus?.settings?.simkl?.blurAdultContent}
                                link={link}
                            ...
                            <MediaEntryCardHoverPopupTitleSection
                                ...
                                year={(media as MediaAPI_BaseMedia).seasonYear ?? media.startDate?.year}
                                season={media.season}
                                format={media.format}
                                link={link}
                                bypassEntryPreloadBudget={shouldBypassPreloadBudget}
                            ...
                            {type === "anime" && (
                                <AnimeEntryCardNextAiring nextAiring={(media as MediaAPI_BaseMedia).nextAiringEpisode} />
                            )}
                        ...
                        <MediaEntryCardHoverPopupFooter>
                            ...
                            {!hideSimklEntryEditButton && <MediaEntryEditModal listData={listData} media={media} type={type} forceModal />}
