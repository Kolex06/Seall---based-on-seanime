...
export function PlaybackManagerProgressTracking() {
    ...
    useWebsocketMessageListener<PlaybackManager_PlaybackState | null>({
        ...
        onMessage: data => {
            if (data) {
                queryClient.invalidateQueries({ queryKey: [API_ENDPOINTS.MEDIA_ENTRIES.GetMediaEntry.key, String(data.mediaId)] })
                queryClient.invalidateQueries({ queryKey: [API_ENDPOINTS.MEDIA_LIBRARY.GetLibraryCollection.key] })
                queryClient.invalidateQueries({ queryKey: [API_ENDPOINTS.SIMKL.GetMediaCollection.key] })
