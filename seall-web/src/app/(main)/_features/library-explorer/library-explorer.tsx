...
import { MediaAPI_BaseMedia, Media_LocalFile, Media_LocalFileType, LibraryExplorer_FileTreeNodeJSON } from "@/api/generated/types"
...
function hasMatchingFiles(userMedia: MediaAPI_BaseMedia[] | undefined, node: LibraryExplorer_FileTreeNodeJSON, filter: LibraryExplorer_Filter): boolean {
    if (node.kind === "file") {
        switch (filter) {
            case "UNMATCHED":
...
function flattenTreeNodes(
    userMedia: MediaAPI_BaseMedia[] | undefined,
    node: LibraryExplorer_FileTreeNodeJSON,
...
export function LibraryExplorer() {
    ...
    const userMedia = useAtomValue(__media_userMediaAtom)
    ...
    const handleResolveFileMedia = (lfs: Media_LocalFile[]) => {
...
interface VirtualizedTreeNodeProps {
    ...
    selectedPath?: string
    localFiles: Record<string, Media_LocalFile> | undefined
    ...
    onResolveMedia: (lfs: Media_LocalFile[]) => void
                        ...
                        if (node.localFile) {
                            updateLocalFile(node.localFile, {
                                metadata: {
                                    ...
                                    type: data.type as Media_LocalFileType,
...
function LibraryInfoPanel({}: { localFiles: Record<string, Media_LocalFile> | undefined }) {
