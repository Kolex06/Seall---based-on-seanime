...
import { Media_Entry, Manga_Entry, Nullish } from "@/api/generated/types"
...
function getEntryPreloadTarget(href: Nullish<string>): EntryPreloadTarget | undefined {
    ...
    if (pathname === "/reading/entry") return { type: "manga", id }

    return undefined
}

function getAnimeEntryQueryKey(id: string) {
    return [API_ENDPOINTS.MEDIA_ENTRIES.GetMediaEntry.key, id]
}
...
async function runEntryPreload(target: EntryPreloadTarget) {
    ...
    try {
        ...
        if (target.type === "anime") {
            await preloaderQueryClient.prefetchQuery<Media_Entry | undefined>({
                queryKey: getAnimeEntryQueryKey(target.id),
                queryFn: () => buildSeaQuery<Media_Entry>({
                    endpoint: API_ENDPOINTS.MEDIA_ENTRIES.GetMediaEntry.endpoint.replace("{id}", target.id),
