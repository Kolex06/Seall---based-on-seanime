import {
    MediaAPI_MediaCollection_MediaListCollection_Lists_Entries,
    MediaAPI_BaseMedia,
    MediaAPI_BaseManga,
    MediaAPI_MangaCollection_MediaListCollection_Lists_Entries,
    MediaAPI_MediaFormat,
    MediaAPI_MediaSeason,
    MediaAPI_MediaStatus,
    Media_Episode,
    Media_LibraryCollectionEntry,
...
import { simkl_getUnwatchedCount } from "./media"
...
export const MEDIA_COLLECTION_SORTING_OPTIONS = [
...
export const DEFAULT_MEDIA_COLLECTION_PARAMS: CollectionParams<"anime"> = {
    sorting: "SCORE_DESC",
    genre: null,
    tags: null,
    status: null,
...
export function filterEntriesByTitle<T extends { media?: MediaAPI_BaseMedia | MediaAPI_BaseManga }[] | null | undefined>(arr: T, input: string): T {
    // @ts-expect-error
    if (!arr) return []
...
export function filterListEntries<T extends MediaAPI_MangaCollection_MediaListCollection_Lists_Entries[] | MediaAPI_MediaCollection_MediaListCollection_Lists_Entries[], V extends CollectionType>(
    type: V,
    ...
    // Filter by year
    if (!!arr && !!params.year) arr = arr.filter(n => (n.media as MediaAPI_BaseMedia)?.seasonYear ?
...
export function filterCollectionEntries<T extends Media_LibraryCollectionEntry[], V extends CollectionType>(
    type: V,
    entries: T | null | undefined,
    params: CollectionParams<V>,
    showAdultContent: boolean | undefined,
...
/** */
export function filterAnimeCollectionEntries<T extends Media_LibraryCollectionEntry[]>(
    entries: T | null | undefined,
    params: CollectionParams<"anime">,
    showAdultContent: boolean | undefined,
    continueWatchingList: Media_Episode[] | null | undefined,
    ...
    if (getParamValue(params.sorting) === "UNWATCHED_EPISODES") {
        // arr = sortBy(arr,
        //     n => !!n.libraryData?.mainFileCount ? n.libraryData?.unwatchedCount : (simkl_getUnwatchedCount(n.media, n.listData?.progress) ||
        // 99999))
        arr = sortBy(arr,
            n => !!n.libraryData?.mainFileCount ? n.libraryData?.unwatchedCount : (
                !!n.nakamaLibraryData?.mainFileCount ? n.nakamaLibraryData?.unwatchedCount : (simkl_getUnwatchedCount(n.media,
                    n.listData?.progress) || 99999)
            ))
    }
    if (getParamValue(params.sorting) === "UNWATCHED_EPISODES_DESC") {
        // arr = sortBy(arr,
        //     n => !!n.libraryData?.mainFileCount ? n.libraryData?.unwatchedCount : simkl_getUnwatchedCount(n.media,
...
/** */
export function filterMangaCollectionEntries<T extends Media_LibraryCollectionEntry[]>(
    entries: T | null | undefined,
...
export function sortContinueWatchingEntries(
    entries: Media_Episode[] | null | undefined,
    sorting: ContinueWatchingSorting,
    libraryEntries: Media_LibraryCollectionEntry[] | null | undefined,
    watchHistory: Continuity_WatchHistory | null | undefined,
) {
    if (!entries) return []
    let arr = [...entries]
    ...
    if (sorting === "UNWATCHED_EPISODES")
        arr = sortBy(arr,
            n => libraryEntries?.find(e => e.media?.id === n.baseAnime?.id)?.libraryData?.unwatchedCount ?? (simkl_getUnwatchedCount(n.baseAnime,
