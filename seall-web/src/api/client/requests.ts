...
export async function buildSeaQuery<T, D extends any = any>(
    ...
    if (password) {
        headers["X-Seall-Token"] = password
    }

    const clientId = getClientId()
    const clientIdProof = getClientIdProof()
    if (clientId) {
        headers["X-Seall-Client-Id"] = clientId
    }
    if (clientIdProof) {
        headers["X-Seall-Client-Id-Proof"] = clientIdProof
    }
    if (__clientPlatform__) {
        headers["X-Seall-Client-Platform"] = __clientPlatform__
    }
...
function syncClientIdFromHeader(headers: unknown) {
    ...
    const clientId = getHeaderValue(headers, "x-seall-client-id")
    const clientIdProof = getHeaderValue(headers, "x-seall-client-id-proof")
...
function _handleSeaError(data: any): string {
    ...
    if (err.includes("Too many requests"))
        return "SIMKL: Too many requests, please wait a moment and try again."

    try {
        const graphqlErr = JSON.parse(err) as any
        console.log("SIMKL error", graphqlErr)
        if (graphqlErr.graphqlErrors && graphqlErr.graphqlErrors.length > 0 && !!graphqlErr.graphqlErrors[0]?.message) {
            return "SIMKL error: " + graphqlErr.graphqlErrors[0]?.message
        }
        return "SIMKL error"
    }
    catch (e) {
