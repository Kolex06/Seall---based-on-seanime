...
const DESKTOP_SERVER_DEFAULT_HOST = "127.0.0.2"
const DESKTOP_SERVER_DEV_HOST = "127.0.0.1"
const DESKTOP_SERVER_DEFAULT_PORT = 43221
const DESKTOP_SERVER_DEV_PORT = 43000
const DEFAULT_UPDATE_FEED_URL = "https://github.com/Kolex06/Seall---based-on-seanime/releases/latest/download"
...
function getDesktopServerHost() {
    if (_development) {
        return DESKTOP_SERVER_DEV_HOST
    }

    return process.env.SEALL_SERVER_HOST || DESKTOP_SERVER_DEFAULT_HOST
}
...
function getDesktopServerPort() {
    ...
    const envPort = Number.parseInt(process.env.SEALL_SERVER_PORT || "", 10)
...
function getDesktopServerBaseUrl() {
    return `http://${getDesktopServerHost()}:${getDesktopServerPort()}`
}
...
function createTray() {
    ...
    const contextMenu = Menu.buildFromTemplate([{
        ...
        id: "quit", label: "Quit Seall", click: () => {
            cleanupAndExit()
        }
    }
    ])

    tray.setToolTip("Seall")
...
async function launchSeallServer(isRestart) {
    return new Promise((resolve, reject) => {
        ...
        if (process.platform === "win32") {
            binaryName = "seall-server-windows.exe"
        } else if (process.platform === "darwin") {
            const arch = process.arch === "arm64" ? "arm64" : "amd64"
            binaryName = `seall-server-darwin-${arch}`
        } else if (process.platform === "linux") {
            const arch = process.arch === "arm64" ? "arm64" : "amd64"
            binaryName = `seall-server-linux-${arch}`
        }

        let binaryPath

        if (_development || !app.isPackaged) {
        ...
        args.push("-host", getDesktopServerHost())
        args.push("-port", String(getDesktopServerPort()))
        ...
        if (_development) {
            if (process.env.TEST_DATADIR) {
                ...
                const devDataDir = path.join(app.getPath("appData"), "Seall-dev")
        ...
        serverProcess.on("close", (code) => {
            ...
            if (!startupResolved && !isShutdown) {
                ...
                if (crashScreen && !crashScreen.isDestroyed()) {
                    crashScreen.show()
                    crashScreen.webContents.send("crash", `Seall server process terminated with status: ${code}. Closing in 10 seconds.`)
...
async function restartSeallServer() {
...
app.whenReady().then(async () => {
    ...
    if (currentUpdateChannel === "seall_nightly") {
        updateConfig.url = DEFAULT_UPDATE_FEED_URL
        updateConfig.allowPrerelease = true
    } else if (currentUpdateChannel === "seall") {
        updateConfig.url = DEFAULT_UPDATE_FEED_URL
    ...
    try {
        ...
        autoUpdater.checkForUpdatesAndNotify().catch(error => {
            log.warn("[Denshi] Auto-update check failed:", error)
        })
    } catch (error) {
    ...
    ipcMain.on("restart-server", () => {
        ...
        restartSeallServer().catch(console.error)
    })
